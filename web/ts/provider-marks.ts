...
// The door is deliberately colourless — brushed metal and one green fingerprint.
// A way in is the exception: you recognise Mastodon by its shape and its purple
// long before you read the word, and making it read as the word is slower.

/** The official Mastodon mark, and the purple it is drawn in. */
const MASTODON = {
    colour: '#6364FF',
...
export function providerMark(provider: string): SVGSVGElement | null {
    if (provider !== 'mastodon') return null;
    ...
    svg.classList.add('door-mark');

    const path = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    path.setAttribute('d', MASTODON.path);
    path.setAttribute('fill', MASTODON.colour);
    svg.append(path);
    return svg;
}
