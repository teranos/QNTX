...
import { openUsersGlyph } from './users-glyph';
...
export interface Line {
    ...
    actors: string[];
    by: string;
    // The writer's token id when the writer is a token: the way to its glyph.
    by_token?: string;
    at: string;
}
...
export function renderList(container: HTMLElement, lines: Line[]): void {
    ...
    for (const line of lines) {
        ...
        // The writer is a way in: a token to its own glyph, a person to Users.
        ...
        if (line.by_token) {
            ...
            by.addEventListener('click', () => { openTokenGlyph(line.by_token as string, line.by); });
        } else if (line.by) {
            by.title = `open Users: ${line.by}`;
            by.addEventListener('click', () => { openUsersGlyph(); });
        }
        tr.appendChild(by);
