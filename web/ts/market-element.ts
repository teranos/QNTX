...
// hit. This element lists every stand across all namespaces as one row each;
...
const ELEMENT_ID = 'market-element';
...
// One font scale, one margin: the element sets these once and every part reads
...
// element, copyable, so the operator reads exactly what the node said.
...
export function createMarketElement(): Element {
    return {
        id: ELEMENT_ID,
        title: 'Stands',
        symbol: '⛬',
        renderContent: () => {
            ...
            content.className = 'stands-element-content';
...
/** Opens the Stands element. */
export function openMarketElement(): void {
