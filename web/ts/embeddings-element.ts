...
import { isSigmaAttestation, spawnSigmaAsWindow } from './components/element/sigma-element';
...
function renderEmbeddings(): void {
    ...
    if (!info) {
        if (sectionInfo) sectionInfo.innerHTML = '<div class="element-loading">Loading...</div>';
    ...
    if (sectionInfo) {
        const modelRows = (models ?? []).map(m => {
            ...
            return `
            <div class="element-row emb-model-row" data-model="${escapeHtml(m.name)}" style="cursor:pointer">
                <span class="label" style="padding-left:12px">${escapeHtml(m.name)}${dimLabel}</span>
                <span class="element-value" style="display:flex;align-items:center;gap:6px">
                    ${m.count}
        ...
        sectionInfo.innerHTML = `
            <div class="element-row">
                ...
                <span class="element-value">${available ? '<span style="color:#4ade80">Active</span>' : '<span style="color:#fbbf24">Unavailable</span>'}</span>
            </div>
            ${(models ?? []).length > 0 ? `
            <div class="element-row">
                <span class="label">Models:</span>
                <span class="element-value">${models.length}</span>
            </div>
            ${modelRows}
            ` : ''}
            <div class="element-row">
                <span class="label">Embedded:</span>
                <span class="element-value">${embedding_count} / ${attestation_count}</span>
            </div>
            ${info.lag ? (() => {
                ...
                <div class="element-row">
                    ...
                    <span class="element-value">${lagLabel}${oldestDate ? ` (from ${oldestDate})` : ''}</span>
                </div>`;
            })() : ''}
        `;
        ...
        for (const btn of sectionInfo.querySelectorAll('.emb-model-cluster-btn')) {
            btn.addEventListener('click', (e) => {
                ...
                recluster(modelName).catch((err: unknown) => log.error(SEG.ELEMENT, `Recluster with ${modelName} failed:`, err));
            });
        }
    }
    ...
    if (sectionReembed) {
        if (available && unembedded > 0) {
            sectionReembed.innerHTML = `
                <div class="element-section" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border-color, #333)">
    ...
    if (sectionCluster) {
        ...
        if (available && embedding_count >= 2) {
            ...
            if (ci && ci.n_clusters > 0) {
                ...
                clusterRows = `
                    <div class="element-row">
                        ...
                        <span class="element-value">${ci.n_clusters}</span>
                    </div>
                    <div class="element-row">
                        <span class="label">Noise:</span>
                        <span class="element-value">${ci.n_noise}</span>
                    </div>
                ...
                clusterRows = `
                    <div class="element-row">
                        <span class="label">Clusters:</span>
                        <span class="element-value" style="color:#6b7280">not computed</span>
                    </div>
                `;
            }
            ...
            sectionCluster.innerHTML = `
                <div class="element-section" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border-color, #333)">
                    <h3 class="element-section-title">HDBSCAN Clustering</h3>
                    ${clusterRows}
    ...
    if (sectionScatter) {
        ...
        if (available && embedding_count >= 2) {
            ...
            if (view3d && regl3d) {
                ...
                sectionScatter.innerHTML = `
                    <div class="element-section" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border-color, #333)">
                        <div style="display:flex;align-items:center;justify-content:space-between">
                            <h3 class="element-section-title" style="margin:0">Projections</h3>
                ...
                sectionScatter.innerHTML = `
                    <div class="element-section" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border-color, #333)">
                        <h3 class="element-section-title">Projections</h3>
                        <div class="element-row">
                            <span class="label">Status:</span>
                            <span class="element-value" style="color:#6b7280">not computed</span>
                        </div>
    ...
    if (sectionTimeline) {
        ...
        if (available && runIDs.size >= 2) {
            sectionTimeline.innerHTML = `
                <div class="element-section" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border-color, #333)">
                    <h3 class="element-section-title">Cluster Timeline</h3>
...
async function renderClusterDetail(clusterID: number): Promise<void> {
    ...
    const mainContent = embeddingsElement.querySelector('.element-content') as HTMLElement | null;
    if (mainContent) mainContent.style.display = 'none';

    // Remove any existing detail container
    embeddingsElement.querySelector('.emb-detail-view')?.remove();

    const detailView = el('div', { class: 'emb-detail-view element-content' });
    detailView.innerHTML = `
            ...
            <div style="display:flex;gap:12px;margin-top:4px">
                <div style="flex:1;min-width:0">
                    <h3 class="element-section-title">Projection</h3>
                    <div class="emb-detail-scatters" style="display:flex;gap:6px"></div>
                    <div class="element-section" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--border-color, #333)">
                        <h3 class="element-section-title">Cluster History</h3>
                ...
                <div style="flex:1;min-width:0;border-left:1px solid var(--border-color, #333);padding-left:12px">
                    <h3 class="element-section-title">Recent Attestations</h3>
    ...
    clusterDetailKeyHandler = (e: KeyboardEvent) => {
        if (e.key === 'ArrowLeft' && prevID !== null) {
            e.stopPropagation();
            renderClusterDetail(prevID).catch((err: unknown) => log.error(SEG.ELEMENT, `Cluster ${prevID} failed to render:`, err));
        } else if (e.key === 'ArrowRight' && nextID !== null) {
            e.stopPropagation();
            renderClusterDetail(nextID).catch((err: unknown) => log.error(SEG.ELEMENT, `Cluster ${nextID} failed to render:`, err));
...
export function createEmbeddingsElement() {
    return {
        id: 'embeddings-element',
        title: 'Embeddings',
        symbol: '\u29C9',
        ...
        renderContent: () => {
            ...
            fetchEmbeddingsInfo().catch((err: unknown) => log.error(SEG.ELEMENT, 'Embeddings info fetch failed:', err));
            return content;
        },
    };
}
