/**
 * Plugin Element Renderer — generic renderer for all plugin-provided elements.
 *
...
/** Create a plugin element element */
export async function createPluginElement(
    item: Element,
    ...
    const { element } = canvasPlaced({
        ...
        resizable: true,
        logLabel: 'PluginElement',
    });

    // Custom title bar with expand button — .symbol via the package so
    // thread-line snapping and spine anchoring can find this element
    ...
    // Content container
    const content = el('div', { class: 'plugin-element-content content-area' });
    element.appendChild(content);

    // Fetch and render plugin content
    await fetchPluginContent(content, def.content_url, item.id, item.content ?? '');
    ...
    wireExpandToWindow({
        ...
        symbol: def.symbol,
        renderContent: () => {
            const trayContent = el('div', { class: 'plugin-element-content content-area' });
            fetchPluginContent(trayContent, def.content_url, item.id, item.content ?? '').catch((err: unknown) => log.error(SEG.ELEMENT, `[PluginElement] content for ${item.id} failed:`, err));
            return trayContent;
        },
        logLabel: 'PluginElement',
    });
    ...
    try {
        ...
        if (resp.status === 503) {
            ...
            if (retryCount < maxRetries) {
                setTimeout(() => {
                    void fetchPluginContent(container, baseURL, itemId, content, retryCount + 1);
                }, delay);
            } else {
                container.innerHTML = `
                    <div class="plugin-error">
        ...
        // without triggering element drag. Runs AFTER scripts execute so
        ...
        log.error(SEG.ELEMENT, '[PluginElement] Fetch error:', err);
        container.innerHTML = `
            <div class="plugin-error">
...
/**
 * Create a placeholder element for unavailable plugin
 *
 ...
 */
export function createPluginPlaceholderElement(
    item: Element,
    pluginName: string
): HTMLElement {
    ...
    // Custom title bar (same style as working plugin element)
    const symbol = item.symbolElement ? settleSymbolSpan(item.symbolElement) : createSymbolSpan(item.symbol ?? '?');
    ...
    // Content area with instructions (like SE element degraded state)
    ...
    if (isOffline) {
        content.innerHTML = `
            ...
            <div style="font-size: 11px; color: #666;">
                Plugin elements are only available when online
            </div>
        `;
    } else {
    ...
    log.debug(SEG.ELEMENT, `[PluginPlaceholder] Created placeholder for ${pluginName} plugin`);
...
function fetchPluginState(pluginName: string, content: HTMLElement, displayName: string): void {
    apiFetch('/api/plugins')
        ...
        .then((data: { plugins?: Array<{ name: string; state: string; message?: string }> } | null) => {
            ...
            if (info.state === 'loading') {
                ...
                content.append(
                    ...
                    el('div', {
                        text: 'Plugin is starting up, element will appear when ready',
        ...
        .catch((err: unknown) => {
            ...
            log.warn(SEG.ELEMENT, 'Could not read plugin status; showing "not enabled" by default:', err);
        });
}
