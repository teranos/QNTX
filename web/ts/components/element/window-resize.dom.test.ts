 ...
 * Tests ResizeObserver integration for database/self window elements
 *
...
describe('Window ResizeObserver', () => {
    ...
    test('Window element stores ResizeObserver reference on element', async () => {
        // Create a mock window element with element content
        ...
        innerContent.className = 'element-content';
    ...
    test('Multiple render calls cleanup old observer before creating new one', () => {
        ...
        elementElement.className = 'canvas-ax-element';
        document.body.appendChild(elementElement);

        // First render - create observer
        const observer1 = new ResizeObserver(() => {});
        (elementElement as any).__resizeObserver = observer1;

        // Second render - cleanup and create new (simulates re-render)
        const existingObserver = (elementElement as any).__resizeObserver;
        if (existingObserver && typeof existingObserver.disconnect === 'function') {
            existingObserver.disconnect();
            delete (elementElement as any).__resizeObserver;
        }

        const observer2 = new ResizeObserver(() => {});
        (elementElement as any).__resizeObserver = observer2;

        // Verify new observer replaced old one
        expect((elementElement as any).__resizeObserver).toBe(observer2);
        expect((elementElement as any).__resizeObserver).not.toBe(observer1);
    });
});
