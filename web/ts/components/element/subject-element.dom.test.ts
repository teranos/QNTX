...
import { subjectElementId, subjectSegment, renderSubjectStats } from './subject-element';
import { AS } from '../../sym';
...
describe('What the subject element is', () => {
    ...
    test('the id says which subject it is about', () => {
        expect(subjectElementId('batch')).toBe('subject-batch');
    });

    test('the same subject is the same id, so it reopens rather than duplicates', () => {
        expect(subjectElementId('batch')).toBe(subjectElementId('batch'));
    });
});
