...
/**
 * Apply the placed-thread-element state to `element`. Mutates in place — the
 * same DOM node now represents a pinned 〽 instead of a cursor (or freshly
 * created div).
 */
function applyPlacedState(element: HTMLElement, item: Element): void {
    const color = item.color ?? THREAD_COLORS[0];
    ...
    if (item.symbol) element.dataset.symbol = item.symbol;
 ...
 */
export function pinThreadElement(element: HTMLElement, canvas: HTMLElement, x: number, y: number, color: string, itemId: string): void {
    if (element.parentElement !== canvas) canvas.appendChild(element);
    applyPlacedState(element, { id: itemId, title: 'Thread', symbol: '〽', x, y, color, renderContent: () => element });
 ...
 */
export function unpinThreadElement(element: HTMLElement): void {
...
/**
 * Initial creation of a thread element for canvas placement.
 *
 * If `element.cursorElement` is provided (the cursor handed off from build
 * mode), it is reused — preserving DOM identity. Otherwise a new div is
 * created (e.g., during canvas restore from persistence).
 */
export function createThreadElement(item: Element): HTMLElement {
    const element = item.cursorElement ?? document.createElement('div');
    applyPlacedState(element, item);
    wireProximityReveal(element);
    log.debug(SEG.ELEMENT, `[ThreadElement] Created ${item.id} with color ${item.color ?? THREAD_COLORS[0]}`);
    return element;
}
