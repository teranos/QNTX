 ...
 *
 * Draws flowing Bezier curves between connected element symbols.
 * Always visible at ~12% opacity (whisper-quiet). Hover brightens.
 * Updates positions when elements move via MutationObserver on style changes.
 */

import { log, SEG } from '../../../logger';

/** Bright opacity for the under-element layer (visible over canvas, occluded by elements) */
const UNDER_OPACITY = 0.55;

/** Whisper-quiet opacity for the over-element layer (subtle hint where line crosses an element) */
...
interface RenderedSpine {
    spine: Spine;
    /** Bright path drawn under the elements — visible over canvas, occluded by element wrappers */
    underPath: SVGPathElement;
    /** Faint baseline drawn above everything — subtle hint where the thread crosses an element */
    overPath: SVGPathElement;
}

/** Per-canvas spine renderer state */
const renderers = new Map<string, {
    /** SVG behind the elements — element wrappers occlude this layer */
    svgUnder: SVGSVGElement;
    /** SVG above the elements — drawn on top of everything */
...
/** Find the .symbol center within a canvas element, in content-layer coordinates */
function getSymbolCenter(canvas: HTMLElement, elementId: string): { x: number; y: number } | null {
    const elementEl = canvas.querySelector(`[data-element-id="${elementId}"]`) as HTMLElement | null;
    if (!elementEl) return null;

    const symbolEl = elementEl.querySelector('.symbol') as HTMLElement | null;
    const target = symbolEl ?? elementEl;
...
function ensureRenderer(canvasId: string, canvas: HTMLElement): { svgUnder: SVGSVGElement; svgOver: SVGSVGElement; spines: RenderedSpine[]; animId: number } {
    ...
    // Under-layer: behind elements. Insert as first child so element wrappers stack above it.
...
export function addSpine(canvasId: string, canvas: HTMLElement, spine: Spine): void {
    ...
    log.debug(SEG.ELEMENT, `[Spine] Added spine ${spine.id} with ${spine.nodes.length} nodes, color ${spine.color}`);
}
...
export function removeSpine(canvasId: string, spineId: string): void {
    ...
    log.debug(SEG.ELEMENT, `[Spine] Removed spine ${spineId}`);
}

/** Find a spine that contains a given element ID */
export function getSpineByNode(canvasId: string, elementId: string): Spine | null {
    const renderer = renderers.get(canvasId);
    if (!renderer) return null;

    const rs = renderer.spines.find(rs => rs.spine.nodes.includes(elementId));
    return rs?.spine ?? null;
}

/** All spines that contain a given element ID, in renderer (≈ creation) order */
export function getSpinesByNode(canvasId: string, elementId: string): Spine[] {
    const renderer = renderers.get(canvasId);
    if (!renderer) return [];
    return renderer.spines.filter(rs => rs.spine.nodes.includes(elementId)).map(rs => rs.spine);
}
