 ...
 * element is on a thread.
 *
 * - ←/→: step prev/next along the active spine, skipping the 〽 end marker.
 *   No-op at the ends (no wrap).
 * - ↑/↓: rotate which of the current element's spines is active. Wraps.
 *   No effect when the element is on only one spine.
 *
...
export interface ThreadNavContext {
    ...
    /** Per-element "active thread" state — mutated by this function. */
    activeSpinePerElement: Map<string, string>;
}
...
export interface ThreadNavResult {
     ...
     * applicable (no selection, element not on a thread).
     */
    handled: boolean;
    /** When set, caller should select this element and pan to it. */
    targetElementId?: string;
    /** When set, the active spine for the current element changed to this id. */
    activatedSpineId?: string;
}

export function navigateThread(direction: Direction, ctx: ThreadNavContext): ThreadNavResult {
    if (!ctx.currentElementId) return { handled: false };
    const spines = getSpinesByNode(ctx.canvasId, ctx.currentElementId);
    if (spines.length === 0) return { handled: false };

    if (direction === 'up' || direction === 'down') {
        if (spines.length <= 1) return { handled: true };
        const activeId = ctx.activeSpinePerElement.get(ctx.currentElementId) ?? spines[0].id;
        ...
        ctx.activeSpinePerElement.set(ctx.currentElementId, newSpineId);
    ...
    let currentIdx = navNodes.indexOf(ctx.currentElementId);
    if (currentIdx === -1) {
        // Current is the 〽 itself — treat as one-past-the-end (← lands on last real element)
    ...
    ctx.activeSpinePerElement.set(targetId, activeSpine.id);
    return { handled: true, targetElementId: targetId };
}
