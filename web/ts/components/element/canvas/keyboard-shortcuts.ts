 ...
 * - h/j/k/l: directional element selection (left/down/up/right)
 * - ENTER: focus the follow-up textarea of the selected element
 * - ESC: deselect all elements (or blur follow-up textarea)
...
/**
 * Callback to check if any elements are selected
 */
...
export function setupKeyboardShortcuts(
    ...
    const handleKeydown = (e: KeyboardEvent) => {
        ...
        // Arrow keys — thread navigation when selected element is on a spine;
        ...
        // h/j/k/l — directional element navigation (spatial only)
        ...
        // ENTER — focus the selected element's follow-up textarea
        ...
        if (e.key === 'Escape') {
            if (hasSelection()) {
                e.preventDefault();
                onDeselect();
                log.debug(SEG.ELEMENT, '[Canvas] ESC pressed - deselecting all elements');
            }
            return;
        }
        ...
        if (e.key === '0' && !e.ctrlKey && !e.metaKey && !e.altKey && !e.shiftKey) {
            e.preventDefault();
            onResetView();
            log.debug(SEG.ELEMENT, '[Canvas] 0 pressed - resetting zoom/pan to origin');
            return;
        }
        ...
        if (e.key === 'Delete' || e.key === 'Backspace') {
            e.preventDefault();
            onDelete();
            log.debug(SEG.ELEMENT, '[Canvas] DELETE/BACKSPACE pressed - deleting selected elements');
            return;
        }
        ...
        if (e.key === 'u' || e.key === 'U') {
            e.preventDefault();
            onUnmeld();
            log.debug(SEG.ELEMENT, '[Canvas] U pressed - unmelding selected composition');
            return;
        }
    };
