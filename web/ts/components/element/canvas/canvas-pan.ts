...
function applyTransform(container: HTMLElement, canvasId: string): void {
    ...
    if (contentLayer) {
        ...
        contentLayer.style.setProperty('--element-border-opacity', String(borderOpacity.toFixed(3)));
    }
}
...
function loadTransformState(canvasId: string): void {
    ...
    if (saved) {
        ...
        log.debug(SEG.ELEMENT, '[CanvasPan] Loaded transform state', {
            canvasId,
            panX: state.panX,
            panY: state.panY,
            scale: state.scale
...
export function setupCanvasPan(container: HTMLElement, canvasId: string): AbortController {
    ...
    // Always register — user may resize browser between mobile/desktop widths
    {
        ...
        document.addEventListener('mouseup', (e: MouseEvent) => {
            ...
            saveTransformState(canvasId);
            log.debug(SEG.ELEMENT, '[CanvasPan] Middle mouse pan end', { panX: state.panX, panY: state.panY });
        }, { signal });
    }
...
/**
 * Pan the canvas so an element element is visible.
 * If the element center is already within the viewport, no movement occurs.
 * Otherwise, smoothly pans to bring the element center into the viewport center.
 */
export function panToElement(container: HTMLElement, canvasId: string, elementEl: HTMLElement): void {
    const state = getState(canvasId);
    const vw = container.clientWidth;
    const vh = container.clientHeight;

    // Element position in canvas coords (from CSS left/top)
    const gx = elementEl.offsetLeft + elementEl.offsetWidth / 2;
    const gy = elementEl.offsetTop + elementEl.offsetHeight / 2;
    ...
    // Pan so element center maps to viewport center
    const contentLayer = container.querySelector('.canvas-content-layer') as HTMLElement;
    if (contentLayer) {
...
/**
 * Frame an element for thread navigation.
 *
 * - Horizontal: center on the element wrapper (the element reads visually balanced
 ...
 *   upper portion of the screen and the element body extends downward into
 *   visible space.
 *
 * Always pans, no "already visible" check.
 */
export function centerOnElementSymbol(container: HTMLElement, canvasId: string, elementEl: HTMLElement): void {
    const state = getState(canvasId);
    const vw = container.clientWidth;
    const vh = container.clientHeight;
    ...
    let wrapperCx = elementEl.offsetWidth / 2;
    let wEl: HTMLElement | null = elementEl;
    ...
    const symbolEl = (elementEl.querySelector('.symbol') as HTMLElement | null) ?? elementEl;
