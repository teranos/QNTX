...
describe('Canvas Pan', () => {
    ...
    test('touch pan works regardless of target (no element check)', () => {
        const container = document.createElement('div');
        const contentLayer = document.createElement('div');
        contentLayer.className = 'canvas-content-layer';
        container.appendChild(contentLayer);

        // Create a mock element inside the content layer
        const item = document.createElement('div');
        item.setAttribute('data-element-id', 'test-element');
        item.className = 'canvas-py-element';
        ...
        // Touch events on container should work even with element present
        const touchStart = createTouchEvent('touchstart', 100, 100);
        container.dispatchEvent(touchStart);
...
// Mock ResizeObserver (used by some element code paths)
...
function buildCanvasWithElement(canvasId: string) {
    const container = document.createElement('div');
    ...
    return { container, contentLayer, el, element: item };
}
...
describe('Drag respects canvas zoom - Tim (Happy Path)', () => {
    ...
    test('Tim drags element at scale 1.0 — delta matches mouse', () => {
        const { container, el, element: item } = buildCanvasWithElement('drag-zoom');
        setupCanvasPan(container, 'drag-zoom');

        const handle = el;
        makeDraggable(el, handle, item);
    ...
    test('Tim drags element at scale 0.5 — element moves 2x mouse delta in CSS space', () => {
        const { container, el, element: item } = buildCanvasWithElement('drag-zoom');
        setupCanvasPan(container, 'drag-zoom');
    ...
    test('Tim drags element at scale 2.0 — element moves 0.5x mouse delta in CSS space', () => {
        const { container, el, element: item } = buildCanvasWithElement('drag-zoom');
        setupCanvasPan(container, 'drag-zoom');
...
describe('Resize respects canvas zoom - Tim (Happy Path)', () => {
    ...
    test('Tim resizes element at scale 0.5 — size grows 2x mouse delta', () => {
        const { container, el, element: item } = buildCanvasWithElement('resize-zoom');
