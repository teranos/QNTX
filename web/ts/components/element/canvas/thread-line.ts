/**
 * Thread Building Mode — interactive path construction through element symbols.
 *
 * Flow:
 * 1. Enter mode: scrim dims, 〽 cursor follows mouse, symbols glow
 * 2. Click an element symbol: adds it to the thread path, Bezier extends
 * 3. Click empty canvas: places the 〽 element, thread is finished
 ...
 * @param existingNodeIds Pre-existing element IDs when extending a thread (excludes 〽)
 ...
 *                        the element axiom; when omitted, a fresh cursor element is created.
 */
export function enterThreadBuildingMode(
    ...
    if (existingNodeIds && existingNodeIds.length > 0) {
        for (const id of existingNodeIds) {
            const elementEl = document.querySelector(`[data-element-id="${id}"]`) as HTMLElement | null;
            const sym = elementEl?.querySelector('.symbol') as HTMLElement | null;
    ...
    const onMouseDown = (e: MouseEvent) => {
        ...
        // Check if clicking on an element symbol
        const symbolTarget = target?.closest('.symbol') as HTMLElement | null;
        const elementTarget = target?.closest('.canvas-element') as HTMLElement | null;

        if (symbolTarget && elementTarget) {
            // Add this symbol to the thread path (skip if already added)
            const itemId = elementTarget.dataset.elementId;
            if (itemId && !nodes.includes(symbolTarget)) {
                nodes.push(symbolTarget);
                // Redraw with new node
                path.setAttribute('d', buildFullPath(nodes, mouseX, mouseY));
                log.debug(SEG.ELEMENT, `[Thread] Added node ${itemId} (${nodes.length} nodes)`);
            }
        } else {
            // No direct hit — snap to nearest symbol within radius
            const snapped = findNearestSymbol(e.clientX, e.clientY, nodes);
            if (snapped) {
                const itemId = snapped.closest('.canvas-element')?.getAttribute('data-element-id');
                if (itemId) {
                    nodes.push(snapped);
                    path.setAttribute('d', buildFullPath(nodes, mouseX, mouseY));
                    log.debug(SEG.ELEMENT, `[Thread] Snapped to node ${itemId} (${nodes.length} nodes)`);
                    return;
                }
            }
            ...
            for (const node of nodes) {
                const item = node.closest('.canvas-element') as HTMLElement | null;
                if (item?.dataset.elementId) nodeIds.push(item.dataset.elementId);
            }
            ...
            log.debug(SEG.ELEMENT, `[Thread] Completed with ${nodeIds.length} nodes`);
        }
    };
    ...
    log.debug(SEG.ELEMENT, `[Thread] Entered building mode from ${originSymbol.textContent}`);
}
