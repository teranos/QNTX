 ...
 * Canvas Action Bar
 *
 * Floating toolbar that appears at top center when elements are selected.
...
export function showActionBar(
    ...
    // Check if any selected elements are in a meld
    // Uses .closest() to handle elements inside sub-containers within compositions
    let meldedComposition: HTMLElement | null = null;
    for (const elementId of selectedElementIds) {
        const elementEl = container.querySelector(`[data-element-id="${elementId}"]`) as HTMLElement | null;
        const comp = elementEl?.closest('.melded-composition') as HTMLElement | null;
        if (comp) {
    ...
    // Check single-element selection type for conversion buttons
    let selectedSymbol: string | undefined;
    if (selectedElementIds.length === 1) {
        const elementEl = container.querySelector(`[data-element-id="${selectedElementIds[0]}"]`) as HTMLElement | null;
        selectedSymbol = elementEl?.dataset.symbol;
    }
    ...
    if (selectedSymbol === Prose && onConvertToPrompt) {
        ...
        convertBtn.dataset.tooltip = 'Convert to prompt element';
    ...
    // Add unmeld button if elements are in a meld
    if (meldedComposition) {
        const isSingleInsideComposition = selectedElementIds.length === 1;
    ...
    deleteBtn.dataset.tooltip = `Delete ${selectedElementIds.length} element${selectedElementIds.length > 1 ? 's' : ''}`;
