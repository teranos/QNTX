...
import { actorElementId, actorSegment, renderActorStats } from './actor-element';
import { BY } from '../../sym';
...
describe('What the actor element is', () => {
    ...
    test('the id says which actor it is about', () => {
        expect(actorElementId('did:key:z6Mkalice')).toBe('actor-did:key:z6Mkalice');
    });

    test('the same actor is the same id, so it reopens rather than duplicates', () => {
        expect(actorElementId('ground')).toBe(actorElementId('ground'));
    });
