/**
 * Attestation attribute rendering — shared by attestation element (+),
 * triplet element (⫶), and any future container that displays attestation attributes.
 *
...
// Muted azure palette — shared with attestation-element.ts
...
function buildArrayPager(items: unknown[]): HTMLElement {
    ...
    // Arrow key navigation when pager or its parent element has focus
    wrapper.tabIndex = 0;
 ...
 * Reusable by triplet-element and any other container that shows attestation attributes.
 */
...
export function parseAttributes(attestation: Attestation): Record<string, unknown> | null {
    ...
    try {
        ...
        log.warn(SEG.ELEMENT, `Unparseable attributes on ${attestation.id}; rendering without them:`, err);
    }
    return null;
}
