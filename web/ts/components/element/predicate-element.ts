 ...
 *
 * Clicking a predicate spawns the predicate element, the same way clicking an
 * attestation spawns the attestation element.
 *
...
/** One element per predicate, so its id says which predicate it is about. */
export function predicateElementId(predicate: string): string {
    return segmentElementId(predicateSegment(), predicate);
}
...
/** Opens one predicate as its own element. */
export function openPredicateElement(predicate: string): void {
    log.debug(SEG.ELEMENT, `[PredicateElement] opening ${predicate}`);
    openSegmentElement(predicateSegment(), predicate);
}
