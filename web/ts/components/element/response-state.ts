/**
 * The element's background answers for its last response: error red, warning
...
export function setResponseState(element: HTMLElement, state: ResponseState | null): void {
    ...
    element.style.backgroundColor = `var(--element-status-${state}-bg)`;
    element.style.outlineColor = `var(--element-status-${state}-text)`;
    if (titleBar) titleBar.style.backgroundColor = `var(--element-status-${state}-section-bg)`;
}
