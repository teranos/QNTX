 ...
 *
 * Clicking a subject spawns the subject element, the same way clicking an
...
/** One element per subject, so its id says which subject it is about. */
export function subjectElementId(subject: string): string {
    return segmentElementId(subjectSegment(), subject);
}
...
/** Opens one subject as its own element. */
export function openSubjectElement(subject: string): void {
    log.debug(SEG.ELEMENT, `[SubjectElement] opening ${subject}`);
    openSegmentElement(subjectSegment(), subject);
}
