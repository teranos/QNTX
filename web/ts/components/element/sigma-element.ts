 ...
 * The element renders as a report: big observation count, time range,
 * proportional bars for string distributions, ranges for number aggregates.
 *
 * Opened via double-click on sigma result items in AX or SE elements.
 */
...
/** Create a Sigma element for canvas placement */
export function createSigmaElement(item: Element): HTMLElement {
    let attestation: Attestation | null = null;
    try {
        if (item.content) attestation = JSON.parse(item.content);
    } catch (err) {
        log.warn(SEG.ELEMENT, `[SigmaElement] Failed to parse content for ${item.id}:`, err);
    }
    ...
    const symbolEl = item.symbolElement ? settleSymbolSpan(item.symbolElement) : createSymbolSpan(Sigma);
    ...
    log.debug(SEG.ELEMENT, `[SigmaElement] Created ${item.id}`);
    return element;
}
...
export function spawnSigmaElement(attestation: Attestation, mouseX?: number, mouseY?: number): void {
...
export function spawnSigmaAsWindow(attestation: Attestation): void {
    const itemId = `sigma-${attestation.id || crypto.randomUUID()}`;
    ...
    tray.open(itemId);
    log.debug(SEG.ELEMENT, `[SigmaElement] Spawned ${itemId} as window`);
}
...
export function renderSigmaResultLine(attestation: Attestation): HTMLElement {
    ...
    item.addEventListener('dblclick', (e) => {
        e.stopPropagation();
        spawnSigmaElement(attestation, e.clientX, e.clientY);
    });
