...
export function parseTypeAttrs(attestation: Attestation): Record<string, unknown> | null {
    ...
    if (typeof attestation.attributes === 'string') {
        try {
            ...
            log.warn(SEG.ELEMENT, `Unparseable type attributes on ${attestation.id}; line renders without them:`, err);
            return null;
        }
    }
...
export function renderTypeResultLine(group: TypeGroup): HTMLElement {
    ...
    // Double-click spawns type element on canvas (lazy import to avoid circular dep)
    item.addEventListener('dblclick', (e) => {
        e.stopPropagation();
        import('./type-element')
            .then(m => m.spawnTypeElement(group.attestations, e.clientX, e.clientY))
            .catch((err: unknown) => log.error(SEG.ELEMENT, `Type element for ${group.label} failed to spawn:`, err));
    });
