 ...
 *
 * A reusable element for displaying time-series data with D3 charts.
...
/**
 * Chart element state manager
 */
export class ChartElementState {
    ...
    /**
     * Create render function for element registration
     */
    createRenderer(): () => HTMLElement {
        return () => {
            const content = document.createElement('div');
            this.element = content;
            this.loadAndRender().catch((err: unknown) => log.error(SEG.ELEMENT, '[ChartElement] initial render failed:', err));
            return content;
        };
    }
    ...
    private async loadAndRender(): Promise<void> {
        ...
        this.element.innerHTML = `
            <div class="element-content" style="position: relative;">
                <div class="chart-container" id="chart-${this.id}">
                    <div class="element-loading">Loading chart data...</div>
                </div>
            </div>
        `;
        ...
        try {
            ...
            log.warn(SEG.UI, `[ChartElement ${this.id}] Failed to load data from ${this.dataSource} (${this.currentRange}):`, error);
            if (this.element) {
    ...
    private render(): void {
        ...
        if (!container || !this.data || this.data.length === 0) {
            if (container) {
                container.innerHTML = '<div class="element-loading">No data available</div>';
            }
            return;
        }
    ...
    private addViewToggle(): void {
        ...
        const elementContent = this.element.querySelector('.element-content');
        if (!elementContent) return;

        // Remove existing toggle if present
        const existing = elementContent.querySelector('.chart-view-toggle');
        if (existing) {
            existing.remove();
        }
        ...
        elementContent.appendChild(toggle);
    }
    ...
    private toggleRange(): void {
        ...
        this.loadAndRender().catch((err: unknown) => log.error(SEG.ELEMENT, `[ChartElement] render for ${this.currentRange} failed:`, err));
    }
}

/**
 * Create a chart element instance
 */
export function createChartElement(
    ...
    const state = new ChartElementState(id, dataSource, config);
