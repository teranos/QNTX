/**
 * @jest-environment jsdom
 *
 * Critical path tests for Element morphing system
 * Focus: Single element axiom, state transitions, invariant enforcement
 *
 * These tests run only with USE_JSDOM=1 (CI environment)
 */

import { describe, test, expect, beforeEach, mock } from 'bun:test';
import type { Element } from './element.ts';

// Only run these tests when USE_JSDOM=1 (CI environment)
const USE_JSDOM = process.env.USE_JSDOM === '1';

// Mock Element.animate (Web Animations API, not in JSDOM)
// Calls the "finish" listener synchronously so morph completes in tests
if (USE_JSDOM) {
    (globalThis.window as any).HTMLElement.prototype.animate = function() {
        const listeners: Record<string, Function[]> = {};
        return {
            finished: Promise.resolve(),
            cancel: () => {},
            finish: () => {},
            play: () => {},
            pause: () => {},
            addEventListener: (type: string, cb: Function) => {
                (listeners[type] ??= []).push(cb);
                // Fire finish immediately for test determinism
                if (type === 'finish') queueMicrotask(() => cb());
            },
            removeEventListener: () => {},
        };
    };
}

// Mock uiState — process-global, must be superset-complete (see test/mock-ui-state.ts)
import { createMockUiState } from '../../test/mock-ui-state';
const { uiState } = createMockUiState();
mock.module('../../state/ui', () => ({ uiState }));

// Import after JSDOM + mock setup
const { tray } = await import('@teranos/elements');
const { findPeakedElement } = await import('@teranos/elements');

describe('Element Single Element Axiom', () => {
    if (!USE_JSDOM) {
        test.skip('Skipped locally (run with USE_JSDOM=1 to enable)', () => {});
        return;
    }

    beforeEach(() => {
        // Clear the element run state
        document.body.innerHTML = '';
        // Reset the singleton (this is a bit hacky but needed for testing)
        (tray as any).element = null;
        (tray as any).indicatorContainer = null;
        (tray as any).items.clear();
        (tray as any).elements.clear();
        (tray as any).deferredItems = [];
    });

    test('Single element axiom: Each element is exactly ONE DOM element', () => {
        tray.init();

        const testElement: Element = {
            id: 'test-element-1',
            title: 'Test Element',
            renderContent: () => {
                const content = document.createElement('div');
                content.textContent = 'Test Content';
                return content;
            }
        };

        // Add element
        tray.add(testElement);

        // Verify exactly one element exists
        const elements = document.querySelectorAll('[data-element-id="test-element-1"]');
        expect(elements.length).toBe(1);

        // Verify it's tracked
        expect(tray.has('test-element-1')).toBe(true);

        // Attempting to add the same element again should be a no-op
        tray.add(testElement);
        const elementsAfter = document.querySelectorAll('[data-element-id="test-element-1"]');
        expect(elementsAfter.length).toBe(1); // Still exactly one

        // The invariant should pass
        expect(() => tray.verifyInvariant()).not.toThrow();
    });

    test('Axiom violation: Creating duplicate elements throws error', () => {
        tray.init();

        const testElement: Element = {
            id: 'test-element-2',
            title: 'Test Element 2',
            renderContent: () => document.createElement('div')
        };

        // Add element properly
        tray.add(testElement);

        // Manually create a duplicate element (violating axiom)
        const duplicate = document.createElement('div');
        duplicate.setAttribute('data-element-id', 'test-element-2');
        document.body.appendChild(duplicate);

        // Verify invariant catches this violation
        expect(() => tray.verifyInvariant()).toThrow(/INVARIANT VIOLATION.*2 elements/);
    });

    test('Axiom violation: Untracked elements are detected', () => {
        tray.init();

        // Create an element outside the factory (violating axiom)
        const rogue = document.createElement('div');
        rogue.setAttribute('data-element-id', 'rogue-element');
        document.body.appendChild(rogue);

        // Verify invariant catches this violation
        expect(() => tray.verifyInvariant()).toThrow(/INVARIANT VIOLATION.*not tracked/);
    });

    test('Element persistence: Same element through add/remove from tray', () => {
        tray.init();

        const testElement: Element = {
            id: 'test-element-3',
            title: 'Test Element 3',
            renderContent: () => document.createElement('div')
        };

        // Add element
        tray.add(testElement);
        const element = document.querySelector('[data-element-id="test-element-3"]');
        expect(element).not.toBeNull();

        // Store a reference to verify it's the same element later
        const elementRef = element;

        // The element should be in the indicator container
        const indicatorContainer = document.querySelector('.tray-dots');
        expect(indicatorContainer?.contains(element!)).toBe(true);

        // Remove the element
        tray.remove('test-element-3');

        // Element should be removed from DOM
        const removedElement = document.querySelector('[data-element-id="test-element-3"]');
        expect(removedElement).toBeNull();

        // Adding again would create a new element (since we removed it)
        tray.add(testElement);
        const newElement = document.querySelector('[data-element-id="test-element-3"]');
        expect(newElement).not.toBeNull();

        // Note: After removal, a new element is created - this is allowed
        // The axiom is about no duplicates existing simultaneously
    });

    test('Click handler persists with element', () => {
        tray.init();

        const testElement: Element = {
            id: 'test-element-4',
            title: 'Test Element 4',
            renderContent: () => {
                const content = document.createElement('div');
                content.textContent = 'Content';
                return content;
            }
        };

        tray.add(testElement);
        const element = document.querySelector('[data-element-id="test-element-4"]') as HTMLElement;
        expect(element).not.toBeNull();

        // Verify click handler works by checking that clicking changes state
        expect(element.dataset.windowState).toBeUndefined();

        // Note: We can't directly test the handler anymore since it's in a WeakMap
        // which is the correct encapsulation. The handler will work when clicked.
    });

    test('Auto-initialization: Elements added before explicit init() auto-initialize', () => {
        // tray.element is null (beforeEach resets it), but add() calls init() internally
        const testElement: Element = {
            id: 'deferred-element',
            title: 'Deferred',
            renderContent: () => document.createElement('div')
        };

        // Add element before explicit init — add() auto-initializes via document.body
        tray.add(testElement);

        // Should be in DOM immediately (auto-init succeeds because body always exists)
        const element = document.querySelector('[data-element-id="deferred-element"]');
        expect(element).not.toBeNull();

        // Verify invariant holds
        expect(() => tray.verifyInvariant()).not.toThrow();
    });

    test('Element tracking: Tracked elements match DOM elements', () => {
        tray.init();

        // Add multiple elements
        const items: Element[] = [
            {
                id: 'track-1',
                title: 'Track 1',
                renderContent: () => document.createElement('div')
            },
            {
                id: 'track-2',
                title: 'Track 2',
                renderContent: () => document.createElement('div')
            },
            {
                id: 'track-3',
                title: 'Track 3',
                renderContent: () => document.createElement('div')
            }
        ];

        items.forEach(g => tray.add(g));

        // All should be tracked
        expect(tray.has('track-1')).toBe(true);
        expect(tray.has('track-2')).toBe(true);
        expect(tray.has('track-3')).toBe(true);
        expect(tray.count).toBe(3);

        // All should be in DOM
        const elements = document.querySelectorAll('[data-element-id]');
        expect(elements.length).toBe(3);

        // Invariant should pass
        expect(() => tray.verifyInvariant()).not.toThrow();

        // Remove one
        tray.remove('track-2');
        expect(tray.count).toBe(2);
        const remainingElements = document.querySelectorAll('[data-element-id]');
        expect(remainingElements.length).toBe(2);

        // Invariant should still pass
        expect(() => tray.verifyInvariant()).not.toThrow();
    });
});

/**
 * Helper: create a fake TouchEvent since JSDOM doesn't support TouchEvent.
 * Must use JSDOM's Event constructor — JSDOM rejects events from other realms.
 */
function createTouchEvent(type: string, clientX: number, clientY: number): Event {
    const Win = globalThis.window as any;
    const event = new Win.Event(type, { bubbles: true, cancelable: true });
    event.touches = [{ clientX, clientY, identifier: 0 }];
    return event;
}

/**
 * Helper: mock getBoundingClientRect on an element.
 * JSDOM returns all zeros by default — we need real geometry for hit testing.
 */
function mockRect(element: HTMLElement, rect: { left: number; right: number; top: number; bottom: number }) {
    element.getBoundingClientRect = () => ({
        left: rect.left,
        right: rect.right,
        top: rect.top,
        bottom: rect.bottom,
        width: rect.right - rect.left,
        height: rect.bottom - rect.top,
        x: rect.left,
        y: rect.top,
        toJSON: () => ({})
    });
}

describe('Touch Browse', () => {
    if (!USE_JSDOM) {
        test.skip('Skipped locally (run with USE_JSDOM=1 to enable)', () => {});
        return;
    }

    // Tray positioned at right edge: x 350-360, y 200-260
    const TRAY_RECT = { left: 350, right: 360, top: 200, bottom: 260 };

    beforeEach(() => {
        document.body.innerHTML = '';
        (tray as any).element = null;
        (tray as any).indicatorContainer = null;
        (tray as any).items.clear();
        (tray as any).elements.clear();
        (tray as any).deferredItems = [];

        // Reset touch browse state from previous test (singleton leaks state)
        (tray as any).proximity.isTouchBrowsing = false;

        tray.init();

        // Mock geometry on the tray container
        const trayEl = (tray as any).element as HTMLElement;
        if (trayEl) mockRect(trayEl, TRAY_RECT);
    });

    function addTestElements(count: number): Element[] {
        const items: Element[] = [];
        for (let i = 0; i < count; i++) {
            const g: Element = {
                id: `touch-element-${i}`,
                title: `Element ${i}`,
                renderContent: () => document.createElement('div')
            };
            items.push(g);
            tray.add(g);
        }
        return items;
    }

    test('touchstart near tray enters browse mode', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;
        expect(proximity.isTouchBrowsing).toBe(false);

        // Touch within activation margin of tray
        document.dispatchEvent(createTouchEvent('touchstart', 355, 230));

        expect(proximity.isTouchBrowsing).toBe(true);
    });

    test('touchstart far from tray does NOT enter browse mode', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;

        // Touch on the opposite side of the screen
        document.dispatchEvent(createTouchEvent('touchstart', 50, 230));

        expect(proximity.isTouchBrowsing).toBe(false);
    });

    test('touchstart with empty tray does NOT enter browse mode', () => {
        // No elements added
        const proximity = (tray as any).proximity;

        document.dispatchEvent(createTouchEvent('touchstart', 355, 230));

        expect(proximity.isTouchBrowsing).toBe(false);
    });

    test('touchmove updates pointer position during browse', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;

        // Enter browse
        document.dispatchEvent(createTouchEvent('touchstart', 355, 230));
        expect(proximity.isTouchBrowsing).toBe(true);

        // Slide thumb down
        document.dispatchEvent(createTouchEvent('touchmove', 355, 250));

        const pos = proximity.getMousePosition();
        expect(pos.x).toBe(355);
        expect(pos.y).toBe(250);
    });

    test('touchmove outside browse mode is ignored', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;

        // Move without starting browse (isTouchBrowsing is false)
        document.dispatchEvent(createTouchEvent('touchmove', 355, 250));

        // Position should still be at default (0,0) or wherever mouse left it
        // The key assertion: isTouchBrowsing should remain false
        expect(proximity.isTouchBrowsing).toBe(false);
    });

    test('touchend exits browse mode', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;

        // Enter browse
        document.dispatchEvent(createTouchEvent('touchstart', 355, 230));
        expect(proximity.isTouchBrowsing).toBe(true);

        // Release
        document.dispatchEvent(createTouchEvent('touchend', 355, 230));

        expect(proximity.isTouchBrowsing).toBe(false);
    });

    test('touchend collapses pointer to offscreen', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;

        // Enter and browse
        document.dispatchEvent(createTouchEvent('touchstart', 355, 230));
        document.dispatchEvent(createTouchEvent('touchmove', 355, 240));

        // Release
        document.dispatchEvent(createTouchEvent('touchend', 355, 240));

        // Pointer should be moved far offscreen to collapse all elements
        const pos = proximity.getMousePosition();
        expect(pos.x).toBe(-9999);
        expect(pos.y).toBe(-9999);
    });

    test('findPeakedElement returns null when no elements are close', () => {
        addTestElements(3);

        const proximity = (tray as any).proximity;
        // Set pointer far from everything
        proximity.setPointerPosition(-9999, -9999);

        const peaked = findPeakedElement(tray as any);
        expect(peaked).toBeNull();
    });

    test('findPeakedElement returns the element with highest proximity', () => {
        const testElements = addTestElements(3);

        // Give elements real geometry so proximity calculations work.
        // Stack them vertically at x=350-360
        const indicatorContainer = (tray as any).indicatorContainer as HTMLElement;
        const dots = indicatorContainer.querySelectorAll('.dot') as NodeListOf<HTMLElement>;

        mockRect(dots[0], { left: 350, right: 360, top: 200, bottom: 212 });
        mockRect(dots[1], { left: 350, right: 360, top: 218, bottom: 230 });
        mockRect(dots[2], { left: 350, right: 360, top: 236, bottom: 248 });

        const proximity = (tray as any).proximity;

        // Place pointer right on top of the second element (y=224 is center of 218-230)
        proximity.setPointerPosition(355, 224);

        const peaked = findPeakedElement(tray as any);
        expect(peaked).not.toBeNull();
        expect(peaked.item.id).toBe('touch-element-1');
    });

    test('touchend on peaked element triggers morph', () => {
        addTestElements(1);

        // Give the dot real geometry on the tray
        const indicatorContainer = (tray as any).indicatorContainer as HTMLElement;
        const dot = indicatorContainer.querySelector('.dot') as HTMLElement;
        mockRect(dot, { left: 350, right: 360, top: 220, bottom: 232 });

        // Element should be in dot state before browse
        expect(dot.dataset.windowState).toBeUndefined();

        // Enter browse near element
        document.dispatchEvent(createTouchEvent('touchstart', 355, 226));
        expect((tray as any).proximity.isTouchBrowsing).toBe(true);

        // Release on element — triggers morphElement which sets isRestoring
        document.dispatchEvent(createTouchEvent('touchend', 355, 226));

        // morphElement was called: isRestoring is set during the morph animation
        expect((tray as any).isRestoring).toBe(true);
    });
});

describe('Tray Identity', () => {
    if (!USE_JSDOM) {
        test.skip('Skipped locally (run with USE_JSDOM=1 to enable)', () => {});
        return;
    }

    beforeEach(() => {
        document.body.innerHTML = '';
        (tray as any).element = null;
        (tray as any).indicatorContainer = null;
        (tray as any).items.clear();
        (tray as any).elements.clear();
        (tray as any).deferredItems = [];
    });

    test('Tim clicks a tray item and gets that item, not a different one', () => {
        tray.init();

        const elementA: Element = { id: 'element-a', title: 'Alpha', renderContent: () => document.createElement('div') };
        const elementB: Element = { id: 'element-b', title: 'Beta', renderContent: () => document.createElement('div') };
        const elementC: Element = { id: 'element-c', title: 'Gamma', renderContent: () => document.createElement('div') };

        tray.add(elementA);
        tray.add(elementB);
        tray.add(elementC);

        const dots = document.querySelectorAll('.dot') as NodeListOf<HTMLElement>;
        expect(dots.length).toBe(3);

        // Each dot's identity matches its element — DOM order matches registration order
        expect(dots[0].dataset.elementId).toBe('element-a');
        expect(dots[1].dataset.elementId).toBe('element-b');
        expect(dots[2].dataset.elementId).toBe('element-c');

        // The items map has the correct element for each ID
        const items = (tray as any).items as Map<string, Element>;
        expect(items.get('element-a')!.title).toBe('Alpha');
        expect(items.get('element-b')!.title).toBe('Beta');
        expect(items.get('element-c')!.title).toBe('Gamma');

        // Click the second dot — isRestoring flag proves morphElement was called for this element
        dots[1].click();
        expect((tray as any).isRestoring).toBe(true);
    });

    test('Tim adds elements out of order via adopt — tray labels still match', () => {
        tray.init();

        const elementA: Element = { id: 'element-a', title: 'Alpha', renderContent: () => document.createElement('div') };
        const elementB: Element = { id: 'element-b', title: 'Beta', renderContent: () => document.createElement('div') };

        tray.add(elementA);

        // Adopt an existing element (simulates canvas-placed element minimized to tray)
        const adoptedEl = document.createElement('div');
        tray.adopt(adoptedEl, elementB);

        // Both dots exist
        const dots = document.querySelectorAll('.dot') as NodeListOf<HTMLElement>;
        expect(dots.length).toBe(2);

        // Each dot's data-element-id matches its item
        expect(dots[0].dataset.elementId).toBe('element-a');
        expect(dots[1].dataset.elementId).toBe('element-b');
    });

    test('Tim: system element is not overwritten by persistence restore', () => {
        tray.init();

        let systemRenderCalled = false;
        const systemElement: Element = {
            id: 'llm-provider',
            title: 'LLM Provider',
            renderContent: () => { systemRenderCalled = true; return document.createElement('div'); },
        };

        // System element registers first
        tray.add(systemElement);

        // Persistence restore tries to add an element with the same ID
        const imposterElement: Element = {
            id: 'llm-provider',
            title: 'Result',
            renderContent: () => document.createElement('div'),
        };
        tray.add(imposterElement);

        // Only one element exists
        const dots = document.querySelectorAll('[data-element-id="llm-provider"]');
        expect(dots.length).toBe(1);

        // The stored item is the system element, not the imposter
        const storedItem = (tray as any).items.get('llm-provider');
        expect(storedItem.title).toBe('LLM Provider');
    });
});

describe('Element State Transitions', () => {
    if (!USE_JSDOM) {
        test.skip('Skipped locally (run with USE_JSDOM=1 to enable)', () => {});
        return;
    }

    beforeEach(() => {
        document.body.innerHTML = '';
        (tray as any).element = null;
        (tray as any).indicatorContainer = null;
        (tray as any).items.clear();
        (tray as any).elements.clear();
        (tray as any).deferredItems = [];
    });

    test('Element starts in dot state', () => {
        tray.init();

        const testElement: Element = {
            id: 'state-test-1',
            title: 'State Test',
            renderContent: () => document.createElement('div')
        };

        tray.add(testElement);
        const element = document.querySelector('[data-element-id="state-test-1"]') as HTMLElement;

        // Should have element class, not window state
        expect(element.className).toBe('dot');
        expect(element.dataset.windowState).toBeUndefined();
        expect(element.dataset.hasText).toBeUndefined();
    });

    test('Window state flag is set/cleared correctly', () => {
        tray.init();

        const testElement: Element = {
            id: 'state-test-2',
            title: 'Window State Test',
            renderContent: () => document.createElement('div')
        };

        tray.add(testElement);
        const element = document.querySelector('[data-element-id="state-test-2"]') as HTMLElement;

        // Initially no window state
        expect(element.dataset.windowState).toBeUndefined();

        // Simulate recording the manifestation (what morphDotToWindow does)
        element.dataset.windowState = 'true';
        expect(element.dataset.windowState).toBe('true');

        // Simulate clearing window state (what morphToElement does)
        delete element.dataset.windowState;
        expect(element.dataset.windowState).toBeUndefined();
    });
});