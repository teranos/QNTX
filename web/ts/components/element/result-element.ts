 ...
 * results (execution output, non-streaming providers). A single element
 * transitions from streaming → complete, gaining copy/expand buttons
 * when the stream finishes.
 *
 * Replaces the former stream-element.ts and result-element.ts.
 *
...
import { createFollowUpZone, type FollowUpRequest, type FollowUpControls } from './element-followup';
...
export interface ResultElementContent {
...
function dispatch(msg: LLMStreamMessage): void {
    ...
    if (cb) {
        ...
        log.debug(SEG.ELEMENT, `[ResultElement] No subscriber for job_id ${msg.job_id}`);
    }
}
...
export function subscribeStream(jobId: string, callback: StreamCallback): void {
    if (subscribers.size === 0) {
        ...
        log.debug(SEG.ELEMENT, '[ResultElement] Registered llm_stream handler');
    }
...
export function unsubscribeStream(jobId: string): void {
    ...
    if (subscribers.size === 0) {
        ...
        log.debug(SEG.ELEMENT, '[ResultElement] Unregistered llm_stream handler');
    }
}
...
function renderOutput(container: HTMLElement, result: ExecutionResult): void {
    ...
    if (result.stderr) {
        const stderrSpan = el('span', {
            text: result.stderr,
            style: { color: 'var(--element-status-error-text)' },
        });
    ...
    if (result.error) {
        const errorSpan = el('span', {
            text: `\nError: ${result.error}`,
            style: { color: 'var(--element-status-error-text)', fontWeight: 'bold' },
        });
        container.appendChild(document.createTextNode(outputText));
...
/**
 * Create a response element — handles both streaming and static results.
 *
...
export function createResultElement(
    ...
    const header = el('div', { class: 'title-bar title-bar--auto result-element-header' });
    ...
    copyBtn.addEventListener('click', () => {
        ...
        navigator.clipboard.writeText(text).then(() => {
            ...
            log.warn(SEG.ELEMENT, 'Copy to clipboard failed:', err);
            copyBtn.textContent = '✗';
            setTimeout(() => { copyBtn.textContent = '⎘'; }, 1500);
        });
    });
    ...
    closeBtn.addEventListener('click', () => {
        ...
        if (composition) {
            ...
            if (unmelded) {
                for (const elementElement of unmelded.members) {
                    const itemId = elementElement.getAttribute('data-element-id');
                    if (itemId && itemId !== item.id) {
                        const elementObj: Element = {
                            id: itemId,
                            title: elementElement.getAttribute('data-symbol') || 'Element',
                            symbol: elementElement.getAttribute('data-symbol') || undefined,
                            renderContent: () => elementElement
                        };
                        makeDraggable(elementElement, elementElement, elementObj, {
                            logLabel: 'RestoredElement'
                        });
                    }
                }
                log.debug(SEG.ELEMENT, `[ResultElement] Unmelded composition before closing ${item.id}`);
            }
        }

        element.remove();
        uiState.removeCanvasElement(item.id);
        log.debug(SEG.ELEMENT, `[ResultElement] Closed ${item.id}`);
    });
    ...
    item.border ??= '1px solid var(--border-on-dark)';
    ...
    wireExpandToWindow({
        ...
        onRestoreToCanvas: (el) => {
            const contentStr = uiState.getCanvasElement(item.id)?.content;
    ...
    const output = el('div', {
        class: 'result-element-output content-area',
        style: {
    ...
    // Nebula renders behind the text as the element's background
    ...
    function connectNebula(): void {
        ...
        nebulaWs.onopen = () => {
            ...
            log.debug(SEG.ELEMENT, '[ResultElement] Nebula WebSocket connected');
        };
        ...
        nebulaWs.onmessage = (event: MessageEvent) => {
            ...
            try {
                ...
                if (decoded.indexOf('forked:') === 0) {
                    ...
                    log.debug(SEG.ELEMENT, `[ResultElement] Fork started: branch ${branchId}`);
                ...
                if (decoded.indexOf('fork_done:') === 0) {
                    ...
                    log.debug(SEG.ELEMENT, `[ResultElement] Fork complete: branch ${branchId}`);
                    return;
                }
                ...
                }).catch((err: unknown) => log.warn(SEG.ELEMENT, 'Nebula frame could not be decoded; the canvas keeps the last frame:', err));
            } catch (e) {
                log.error(SEG.ELEMENT, '[ResultElement] Nebula frame error', e);
            }
        };
    ...
    function sendNebulaMessage(data: string): void {
        // Track scrub state — enable frame drawing during scrub on completed elements
    ...
    if (saved?.content && !result && !streamJobId) {
        try {
            const content = JSON.parse(saved.content) as ResultElementContent;
            if (content.tokens && content.tokens.length > 0) {
                ...
                log.debug(SEG.ELEMENT, `[ResultElement] Restored ${tokens.length} tokens for ${item.id}`);
            ...
            log.error(SEG.ELEMENT, `[ResultElement] Failed to parse saved content for ${item.id}:`, e);
        }
    }
    ...
    if (tokens.length > 0) {
        ...
        if (saved?.content) {
            try {
                const restored = JSON.parse(saved.content) as ResultElementContent;
                if (restored.nebulaFrame) {
                    ...
                    createImageBitmap(new Blob([arr], { type: 'image/png' })).catch((err: unknown) => {
                        log.warn(SEG.ELEMENT, 'Saved nebula frame could not be decoded; the canvas stays blank:', err);
                        return null;
                    }).then((bmp) => {
                        if (!bmp) return;
                        if (!nebulaCtx) return;
                        ...
                        nebulaCtx.clearRect(0, 0, nebulaCanvas.width, nebulaCanvas.height);
                        nebulaCtx.drawImage(bmp, 0, 0, nebulaCanvas.width, nebulaCanvas.height);
                    }).catch((err: unknown) => log.warn(SEG.ELEMENT, 'Restored nebula frame could not be drawn:', err));
                }
    ...
    if (streamJobId) {
        ...
        subscribeStream(streamJobId, (msg: LLMStreamMessage) => {
            ...
            if (msg.done) {
                ...
                persistContent();
                log.debug(SEG.ELEMENT, `[ResultElement] Stream complete for ${streamJobId}, ${tokens.length} tokens`);
                return;
            }
    ...
    const followupZone = createFollowUpZone({
        ...
        onExecute: hasTokens
            ? (request: FollowUpRequest, controls: FollowUpControls) => {
                executeStreamFollowUp(element, item, request, controls).catch((err: unknown) => log.error(SEG.ELEMENT, 'Stream follow-up failed:', err));
            }
            : undefined,
    });
    ...
    function persistContent(): void {
        const content: ResultElementContent = {
        ...
        const existing = uiState.getCanvasElement(item.id);
        if (existing) {
            uiState.addCanvasElement({ ...existing, content: JSON.stringify(content) });
        }
    }

    // Persist static result immediately
    if (result && !streamJobId) {
        const contentPayload: ResultElementContent = { result, promptConfig, prompt };
        (item as any).content = JSON.stringify(contentPayload);
    }
...
/**
 * Check whether a response element received any tokens.
 */
export function getResponseTokenCount(elementEl: HTMLElement): number {
    const output = elementEl.querySelector('.result-element-output');
    if (!output) return 0;
    return output.children.length;
}

/**
 * Populate a streaming response element with static text
 * (for non-streaming providers that return a full response).
 */
export function populateStaticContent(elementEl: HTMLElement, text: string): void {
    const output = elementEl.querySelector('.result-element-output');
    if (!output) return;
...
/**
 * Update an existing response element's content in place.
 */
export function updateResultElementContent(resultElement: HTMLElement, result: ExecutionResult): boolean {
    const output = resultElement.querySelector('.result-element-output') as HTMLElement | null;
    if (!output) return false;

    renderOutput(output, result);

    const itemId = resultElement.getAttribute('data-element-id');
    if (itemId) {
        const existing = uiState.getCanvasElement(itemId);
        if (existing) {
            ...
            try {
                ...
                // The write below replaces the element's content: losing the
                // prompt config here loses it for good, so it is said.
                log.warn(SEG.ELEMENT, `Previous content of ${itemId} unparseable; its prompt config is not carried over:`, err);
            }
            const contentPayload: ResultElementContent = { result, promptConfig };
            uiState.addCanvasElement({ ...existing, content: JSON.stringify(contentPayload) });
        }
    }
...
async function executeStreamFollowUp(
    ...
    const responseElement = createResultElement(responseItem, undefined, undefined, request.text, responseElementId);
    canvas.appendChild(responseElement);

    const parentElementId = parentElement.dataset.elementId;
    if (parentElementId) {
        autoMeldResultBelow(
            parentElement, parentElementId, 'result',
            'ResultElement', responseElement, responseElementId, 'ResponseFollowUp',
        );
    }
    ...
    const body: Record<string, unknown> = {
        ...
        element_id: responseElementId,
    };
    if (parentElementId) body.parent_element_id = parentElementId;
    ...
    apiFetch('/api/prompt/direct', jsonBody('POST', body))
        ...
        .then((data: any) => {
            ...
            if (data.error) {
                unsubscribeStream(responseElementId);
                responseElement.remove();
                uiState.removeCanvasElement(responseElementId);
            ...
            log.debug(SEG.ELEMENT, `[ResultElement] Follow-up complete, ${tokenCount} streamed tokens`);
        })
        .catch((err) => {
            unsubscribeStream(request.itemId);
            responseElement.remove();
            uiState.removeCanvasElement(responseElementId);

            const errMsg = err instanceof Error ? err.message : String(err);
            controls.error(`Failed: ${errMsg}`);
            log.error(SEG.ELEMENT, `[ResultElement] Follow-up failed for ${parentItem.id}: ${errMsg}`);
        });
}
