/**
 * Token Popup — hover overlay for stream element tokens.
 *
    ...
    function show(span: HTMLSpanElement): void {
        ...
        if (topKRaw) {
            try {
                ...
                log.warn(SEG.ELEMENT, 'Token candidates unparseable; the popup omits them:', err);
            }
        }
        ...
        if (stagesRaw) {
            try {
                ...
                log.warn(SEG.ELEMENT, 'Sampler stages unparseable; the popup omits them:', err);
            }
        }
