...
import { createAxElement, updateAxElementResults } from '../ax-element';
...
describe('AX Query Meld - Jenny (Complex Scenarios)', () => {
    ...
    test('Jenny types "of qntx", gets results, drags AX element, melds with prompt', async () => {
        ...
        // 2. Jenny creates AX element and renders it
        const axItem: Element = {
            id: 'ax-jenny-qntx',
            title: 'AX Query',
            symbol: AX,
            x: 100,
        ...
        const axElement = createAxElement(axItem);
        ...
        expect(axElement.style.backgroundColor).toBe('var(--element-status-pending-bg)'); // Pending state
        ...
        updateAxElementResults('ax-jenny-qntx', attestation1);
        updateAxElementResults('ax-jenny-qntx', attestation2);
        updateAxElementResults('ax-jenny-qntx', attestation3);

        // 6. Verify results are displayed inside AX element (not separate result elements)
        const resultsContainer = axElement.querySelector('.ax-element-results') as HTMLElement;
        expect(resultsContainer).toBeTruthy();

        const resultItems = resultsContainer.querySelectorAll('.ax-element-result-item');
        ...
        // 7. Jenny drags AX element to the right
        // Mock getBoundingClientRect for AX element at starting position
        ...
        // Simulate drag (dragstart → dragend updates position via element-interaction.ts)
        ...
        // 8. Jenny creates a prompt element positioned for melding (within MELD_THRESHOLD)
        const promptItem = {
            id: 'prompt-jenny-meld',
            title: 'Prompt',
            symbol: SO,
            x: 970, // 20px gap from AX element right edge (950 + 20 = 970)
            y: 100,
            width: 420,
            height: 340,
        ...
        // Verify AX element detected prompt as meld target
        ...
        if (meldResult.target) {
            performMeld(axElement as HTMLElement, meldResult.target, axItem, promptItem);
        }
        ...
        // Verify both elements are inside composition
        const elementsInComposition = composition?.querySelectorAll('.canvas-element');
        expect(elementsInComposition?.length).toBe(2);

        // Verify AX element is first (left), prompt is second (right)
        const firstElement = elementsInComposition?.[0] as HTMLElement;
        const secondElement = elementsInComposition?.[1] as HTMLElement;
        expect(firstElement.dataset.elementId).toBe('ax-jenny-qntx');
        expect(secondElement.dataset.elementId).toBe('prompt-jenny-meld');

        // Verify AX results are still displayed inside the melded AX element
        const meldedResultsContainer = firstElement.querySelector('.ax-element-results');
        expect(meldedResultsContainer).toBeTruthy();
        const meldedResults = meldedResultsContainer?.querySelectorAll('.ax-element-result-item');
