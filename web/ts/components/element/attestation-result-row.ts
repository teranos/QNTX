 ...
 *
 * ax-element and semantic-element each grew their own copy — same tint, same
...
/**
 * Build one result row. Double-click spawns the attestation element, which is
...
export function attestationResultRow(attestation: Attestation, options: ResultRowOptions): HTMLElement {
    ...
    item.addEventListener('dblclick', (e) => {
        e.stopPropagation();
        spawnAttestationElement(attestation, e.clientX, e.clientY);
    });
