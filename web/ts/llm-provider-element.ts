...
export function createLlmProviderElement(): Element {
    return {
        id: 'llm-provider-element',
        title: 'LLM Provider',
        symbol: BY,
        renderContent: () => {
...
async function setupLlmProviderContent(content: HTMLElement): Promise<void> {
    content.innerHTML = `
        <div class="element-content">
            <div class="element-section">
                <h3 class="element-section-title">AI Inference Provider</h3>
    ...
    function saySo(message: string, well: boolean): void {
        ...
        statusEl.className = `llm-provider-status-line ${well ? 'status-well' : 'status-unwell'}`;
    }
    ...
    try {
        ...
        listEl.innerHTML = providers.map(name => `
                <div class="element-row llm-provider-row">
