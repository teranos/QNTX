...
/** A compact attestation for cartesian expansion */
export interface ExpandAttestation {
    id: string;
    subjects: string[];
    predicates: string[];
    contexts: string[];
...
/** A single claim from cartesian expansion */
...
/** Result of claim grouping */
export interface GroupResult {
    groups: ClaimGroup[];
    total_groups: number;
}

/** Result of source ID deduplication */
 ...
 * Each attestation with S subjects, P predicates, C contexts, A actors
 * produces S x P x C x A individual claims.
 */
export function expandCartesianClaims(attestations: ExpandAttestation[]): ExpandResult {
    const json = wasm.expand_cartesian_claims(JSON.stringify({ attestations }));
    const parsed = JSON.parse(json);
    if ('error' in parsed) {
        throw new Error(`expand_cartesian_claims: ${parsed.error}`);
    }
    return parsed;
}

/**
 * Group individual claims by their (subject, predicate, context) key.
 * Returns groups in deterministic sorted order.
 */
export function groupClaims(claims: IndividualClaim[]): GroupResult {
    const json = wasm.group_claims(JSON.stringify({ claims }));
    const parsed = JSON.parse(json);
    if ('error' in parsed) {
        throw new Error(`group_claims: ${parsed.error}`);
    }
    return parsed;
}
 ...
 */
export function dedupSourceIds(claims: IndividualClaim[]): DedupResult {
    const json = wasm.dedup_source_ids(JSON.stringify({ claims }));
    const parsed = JSON.parse(json);
    if ('error' in parsed) {
        throw new Error(`dedup_source_ids: ${parsed.error}`);
    }
    return parsed;
}
