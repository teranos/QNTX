...
describe('Canvas Sync - Spike (Backoff)', () => {
    beforeEach(() => {
        ...
        mockElements.push({ id: 'g-1', symbol: 'ax', x: 100, y: 200 });
    ...
    test('item skipped during flush if backoff not expired', async () => {
        localStorage.setItem(STORAGE_KEY, JSON.stringify([
            { id: 'g-1', op: 'element_upsert', retryCount: 1, nextRetryAt: Date.now() + 60000 }
        ]));
    ...
    test('item retried after backoff expired', async () => {
        localStorage.setItem(STORAGE_KEY, JSON.stringify([
            { id: 'g-1', op: 'element_upsert', retryCount: 1, nextRetryAt: Date.now() - 1000 }
        ]));
    ...
    test('new add() for same entity resets retryCount', async () => {
        ...
        // User edits element again → fresh entry, no backoff
        canvasSyncQueue.add({ id: 'g-1', op: 'element_upsert' });
        stored = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]');
