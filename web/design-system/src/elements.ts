...
export function renderElementSpecimens(root: HTMLElement) {
...
function morphSpecimen(section: HTMLElement) {
    section.appendChild(specimenCard(
        ...
        (body) => {
            ...
            elementEl.className = 'dot'
            elementEl.style.position = 'relative'
            elementEl.style.marginBottom = '10px'
            ...
            elementEl.style.right = '10px'
            elementEl.style.bottom = '10px'
            stage.appendChild(elementEl)
            ...
            const maximize = async () => {
                ...
                const fromRect = elementEl.getBoundingClientRect()

                // Detach from stage, go fixed
                elementEl.remove()
                elementEl.style.position = 'fixed'
                elementEl.style.left = `${fromRect.left}px`
                elementEl.style.top = `${fromRect.top}px`
                elementEl.style.width = `${fromRect.width}px`
                elementEl.style.height = `${fromRect.height}px`
                document.body.appendChild(elementEl)
                ...
                try {
                    ...
                    elementEl.className = ''
                    elementEl.style.left = `${targetX}px`
                    elementEl.style.top = `${targetY}px`
                    elementEl.style.width = `${targetW}px`
                    elementEl.style.height = `${targetH}px`
                    elementEl.style.borderRadius = WINDOW_BORDER_RADIUS
                    elementEl.style.boxShadow = WINDOW_BOX_SHADOW
                    elementEl.style.backgroundColor = 'var(--bg-almost-black)'
                    elementEl.style.border = '1px solid var(--border-on-dark)'
                    elementEl.style.display = 'flex'
                    elementEl.style.flexDirection = 'column'
