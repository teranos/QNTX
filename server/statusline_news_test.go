...
func tokenCaller(did string) auth.Admission {
	...
	a.Grant = &auth.Grant{DID: did}
	return a
}

// News is addressed. Two tokens polling the same node each see what was left
// for them and nothing left for the other.
func TestNewsReachesOnlyWhoItIsFor(t *testing.T) {
	l := newNewsLog()
	now := time.Now()
	l.leave(News{ID: "n-alice", For: "did:key:alice", Item: StatusItem{Name: "ci", Note: "success main", Symbol: SymbolWell}, UntilMs: now.Add(time.Minute).UnixMilli()})
	l.leave(News{ID: "n-bob", For: "did:key:bob", Item: StatusItem{Name: "ci", Note: "failure main", Symbol: SymbolUnwell}, UntilMs: now.Add(time.Minute).UnixMilli()})
	h := newsRow(l)
...
func TestNewsCarriesItsID(t *testing.T) {
	l := newNewsLog()
	l.leave(News{ID: "ground:payload:x", For: "did:key:alice", Item: StatusItem{Name: "ci", Symbol: SymbolWell}, UntilMs: time.Now().Add(time.Minute).UnixMilli()})
...
func TestNewsLeavesTheRowAfterItsHold(t *testing.T) {
	l := newNewsLog()
	l.leave(News{ID: "n-old", For: "did:key:alice", Item: StatusItem{Name: "ci", Symbol: SymbolWell}, UntilMs: time.Now().Add(-time.Second).UnixMilli()})
	body := rowAs(t, newsRow(l), tokenCaller("did:key:alice"))
	for _, it := range body.Items {
		if it.ID == "n-old" {
...
func TestNewsItemAnswersByID(t *testing.T) {
	l := newNewsLog()
	l.leave(News{ID: "n-1", For: "did:key:alice", Item: StatusItem{Name: "ci", Note: "success main", Symbol: SymbolWell},
