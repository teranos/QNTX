...
func (s *QNTXServer) setupHTTPRoutes() {
	...
	s.answer("/am/version", s.HandleVersion)   // Which build is running (GET)
	s.answer("/am/syscap", s.HandleSyscap)     // What the build can do: storage and parser backends (GET)
	s.answer("/openapi.json", s.HandleOpenAPI) // What this build serves, generated by cmd/openapi (GET)
	s.answer("/logs/download", s.HandleLogDownload)
	s.answer("/api/timeseries/usage", s.HandleUsageTimeSeries)
	s.answer("/am/config", s.HandleConfig)
