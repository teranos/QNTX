...
what the node serves is written down by `make openapi` (cmd/openapi), which reads the reach table and the handlers' own prose into server/openapi/openapi.json. the gRPC surface is the .proto files in plugin/grpc/protocol/.
