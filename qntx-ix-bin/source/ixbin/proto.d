...
struct Attestation {
    ...
    // attributes (field 8) is google.protobuf.Struct — encode as raw bytes
...
//
// Struct is: { map<string, Value> fields = 1; }
// Value is: oneof { null_value=1, number_value=2, string_value=3, bool_value=4, struct_value=5, list_value=6 }
// ---------------------------------------------------------------------------
...
ubyte[] encodeStructFromStringMap(string[string] map) {
    ubyte[] result;
    foreach (k, v; map) {
        // Each map entry is: field 1 (fields map) → key-value pair
        // Map entry message: key=1 (string), value=2 (Value message)
        // Value message for string: string_value=3 (string)
        ubyte[] valueMsg;
...
unittest {
    ...
    meta.description = "Binary data ingestion";
    meta.author = "QNTX";
    meta.license = "MIT";

    auto encoded = encode(meta);
    auto decoded = decode!MetadataResponse(encoded);
    assert(decoded.name == "ix-bin");
    assert(decoded.version_ == "0.1.0");
    assert(decoded.qntxVersion == ">= 0.1.0");
    assert(decoded.description == "Binary data ingestion");
    assert(decoded.author == "QNTX");
    assert(decoded.license == "MIT");

    // Test InitializeRequest with map
    InitializeRequest req;
    req.atsStoreEndpoint = "localhost:50051";
    req.authToken = "test-token";
    req.config["debug"] = "true";
    req.config["workers"] = "4";

    auto encReq = encode(req);
    auto decReq = decode!InitializeRequest(encReq);
    assert(decReq.atsStoreEndpoint == "localhost:50051");
    assert(decReq.authToken == "test-token");
    assert(decReq.config["debug"] == "true");
    assert(decReq.config["workers"] == "4");

    // Test repeated strings
    InitializeResponse resp;
    resp.handlerNames = ["ix-bin.ingest", "ix-bin.scan"];

    auto encResp = encode(resp);
    auto decResp = decode!InitializeResponse(encResp);
    assert(decResp.handlerNames.length == 2);
    assert(decResp.handlerNames[0] == "ix-bin.ingest");
    assert(decResp.handlerNames[1] == "ix-bin.scan");
}
