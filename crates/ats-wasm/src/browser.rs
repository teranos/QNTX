...
//! - Converted to ats::Attestation for internal storage operations
...
pub fn cosine_similarity_f32(query: &[f32], candidate: &[f32]) -> Result<f32, JsValue> {
    ats::similarity::cosine_similarity(query, candidate).map_err(|e| JsValue::from_str(&e))
}
