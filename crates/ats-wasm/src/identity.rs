...
//!
//! Wraps ats-id functions for use through the WASM bridge.
...
/// JSON input for compact ASUID generation (type IDs).
#[derive(serde::Deserialize)]
...
pub(crate) fn generate_compact_asuid_impl(input: &str) -> String {
    ...
    match ats_id::Asuid::compact(&parsed.prefix, &parsed.name, &parsed.random_bytes) {
        Some(id) => format!(r#"{{"full":"{}","short":"{}"}}"#, id.full(), id.short()),
...
pub(crate) fn generate_random_id_impl(input: &str) -> String {
    ...
    let id = ats_id::random_id_from_bytes(parsed.length, &parsed.random_bytes);
    format!(r#"{{"id":"{}"}}"#, id)
}
