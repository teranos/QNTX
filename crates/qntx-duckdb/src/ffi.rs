...
pub extern "C" fn duckdb_watchers_new(location: *const c_char) -> *mut WatcherStore {
    let loc = match unsafe { cstr_to_str(location) } {
        Ok(s) => s,
        Err(e) => {
            eprintln!("qntx-duckdb: invalid watcher location string: {}", e);
            return ptr::null_mut();
        }
    };
    match WatcherStore::open(loc) {
        Ok(store) => Box::into_raw(Box::new(store)),
        Err(e) => {
            eprintln!("qntx-duckdb: failed to open watchers at {}: {}", loc, e);
            ptr::null_mut()
        }
    }
}
...
pub extern "C" fn duckdb_watchers_free(store: *mut WatcherStore) {
    if !store.is_null() {
        if let Err(e) = unsafe { (*store).flush() } {
            eprintln!("qntx-duckdb: failed to flush watcher fires on close: {}", e);
        }
    }
    unsafe { free_boxed(store) };
}
...
pub extern "C" fn duckdb_schedules_new(location: *const c_char) -> *mut ScheduleStore {
    let loc = match unsafe { cstr_to_str(location) } {
        Ok(s) => s,
        Err(e) => {
            eprintln!("qntx-duckdb: invalid schedule location string: {}", e);
            return ptr::null_mut();
        }
    };
    match ScheduleStore::open(loc) {
        Ok(store) => Box::into_raw(Box::new(store)),
        Err(e) => {
            eprintln!("qntx-duckdb: failed to open schedules at {}: {}", loc, e);
            ptr::null_mut()
        }
    }
}
...
pub extern "C" fn duckdb_schedules_free(store: *mut ScheduleStore) {
    if !store.is_null() {
        if let Err(e) = unsafe { (*store).flush() } {
            eprintln!(
                "qntx-duckdb: failed to flush schedule ticks on close: {}",
                e
            );
        }
    }
