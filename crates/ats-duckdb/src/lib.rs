...
/// trip each, so this number is the read cost. Sixteen holds a query under a
/// second at the round trip time the deployment measures. The deployment writes
/// a file every several minutes, which puts a merge at about one an hour.
...
/// delete it, and every other read and write waits on that. This bounds the
/// wait to tens of seconds at the round trip time the deployment measures. A
/// namespace holding more files than this is worked off over several runs, each
/// one leaving fewer files than it found, so the count converges.
...
impl DuckdbStore {
    ...
    pub fn open(location: impl Into<String>, namespace: impl AsRef<str>) -> Result<Self> {
        ...
        // A process that died mid-compaction left its sources in place, and
        // every one of their rows is also in the merged file. Finishing here
        // is before anything has read them twice.
    ...
    ///
    /// Rows in the buffer reach a file through `flush`, the only writer of one.
...
mod tests {
    ...
    fn a_compaction_whose_merge_stopped_partway_drops_it() {
        ...
        first
            .objects
            .put(Object::ParquetFiles, &half, b"PAR1 and then nothing".to_vec())
            .unwrap();
        first
            .objects
            .put(
