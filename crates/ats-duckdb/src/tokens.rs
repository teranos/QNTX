...
impl From<&TokenRecord> for TokenSummary {
    fn from(record: &TokenRecord) -> Self {
        Self {
            ...
            minted_by_username: record.minted_by_username.clone(),
    ...
    fn load(&mut self) -> Result<()> {
        let sql = format!(
            "SELECT id, hash, label, did, minted_by, namespace, \
                    ...
                    minted_by_user, minted_by_username \
             FROM read_json('{}/*.json', columns = {{ \
                 ...
                 minted_by_user: 'VARCHAR', minted_by_username: 'VARCHAR' }})",
            self.prefix
        );
        ...
        let rows = match stmt.query_map([], |row| {
            Ok(TokenRecord {
                ...
                minted_by_username: row.get::<_, Option<String>>(13)?.unwrap_or_default(),
            })
        }) {
            Ok(rows) => rows,
            Err(_) => return Ok(()),
...
mod tests {
    ...
    fn record(id: &str, hash: &str) -> TokenRecord {
        TokenRecord {
            ...
            minted_by_username: "tim".to_string(),
