# ats-sqlite FFI Integration

CGO bindings for using Rust ats-sqlite from Go.
...
CGO Wrapper (ats/storage/sqlitecgo)
    ↓
C ABI (crates/ats-sqlite/ffi.rs)
    ↓
Rust SqliteStore (ats-sqlite)
    ↓
SQLite Database
```
...
```bash
cargo build --release --features ffi -p ats-sqlite
```

This creates `target/release/libats_sqlite.dylib` (macOS) or `.so` (Linux).
...
Go's `types.As` and Rust's `ats::Attestation` have slight serialization differences:
...
If fully integrated, ats-sqlite could replace the entire Go `db` package and `ats/storage` SQL implementation, consolidating all storage logic in the Rust core.
