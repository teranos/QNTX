...
use ats::storage::enforcement::{EnforcementEvent, EnforcementInput, EvictionDetails};
    ...
    pub(crate) fn enforce_limits_inner(
        ...
        for actor in &input.actors {
            for context in &input.contexts {
                match self.enforce_actor_context_limit(
                    ...
                    Err(e) => {
                        eprintln!(
                            "ats-sqlite: enforce_actor_context_limit failed for actor={} context={}: {}",
                            actor, context, e
                        );
                    }
                }
        ...
        for actor in &input.actors {
            match self.enforce_actor_contexts_limit(actor, input.config.actor_contexts_limit) {
                ...
                Err(e) => {
                    eprintln!(
                        "ats-sqlite: enforce_actor_contexts_limit failed for actor={}: {}",
                        actor, e
                    );
                }
            }
        ...
        for subject in &input.subjects {
            match self.enforce_entity_actors_limit(subject, input.config.entity_actors_limit) {
                ...
                Err(e) => {
                    eprintln!(
                        "ats-sqlite: enforce_entity_actors_limit failed for subject={}: {}",
                        subject, e
                    );
                }
            }
        ...
        for ev in &events {
            if let Err(e) = self.log_storage_event(ev) {
                eprintln!("ats-sqlite: failed to log storage event: {}", e);
            }
        }
        ...
        if total.as_millis() > 100 {
            eprintln!(
                "ats-sqlite: enforce_limits took {}ms (ac={}ms acs={}ms ea={}ms) evictions={}",
                total.as_millis(),
                d1.as_millis(),
                d2.as_millis(),
                d3.as_millis(),
    ...
    fn enforce_actor_context_limit(
        ...
        if !eviction_batch.is_empty() {
            ...
            if let Err(e) = insert_result {
                eprintln!(
                    "ats-sqlite: Σ failed to insert sigma for actor={} context={}: {}",
                    actor, context, e
                );
            }
        }
    ...
    fn enforce_entity_actors_limit(
        ...
        for actor in actors_to_delete.iter() {
            ...
            if !eviction_batch.is_empty() {
                ...
                if let Err(e) = insert_result {
                    eprintln!(
                        "ats-sqlite: Σ failed to insert sigma for entity={} actor={}: {}",
                        entity, actor, e
                    );
                }
            }
    ...
    fn load_eviction_batch(
        ...
        for row_result in rows {
            ...
            match crate::SqliteStore::row_to_attestation(row_data) {
                ...
                Err(e) => {
                    eprintln!("ats-sqlite: failed to deserialize eviction row: {}", e);
                }
            }
        }
        Ok(attestations)
...
mod tests {
    ...
    use ats::storage::enforcement::{EnforcementConfig, EnforcementInput};
    ...
    fn test_distill_attestation_created_on_eviction() {
        ...
        assert!(distill_att.actors.contains(&"system:distill".to_string()));
        assert!(distill_att.actors.contains(&"bot".to_string()));
