//! QNTX PTY Element Plugin - Main Entry Point
//!
//! A gRPC plugin that provides persistent terminal elements with full PTY support.
//!
//! Usage:
//!     qntx-pty-element --port 38701
//!     qntx-pty-element --address localhost:38701

use clap::Parser;
use qntx_pty_element::proto::domain_plugin_service_server::DomainPluginServiceServer;
use qntx_pty_element::PTYElementService;
...
#[derive(Parser, Debug)]
#[command(name = "qntx-pty-element")]
#[command(about = "QNTX persistent terminal element plugin")]
#[command(version)]
struct Args {
    /// gRPC server port
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    std::panic::set_hook(Box::new(|panic_info| {
        eprintln!("PANIC: PTY element plugin panicked during startup or execution");
        eprintln!(
            "  Location: {}",
            panic_info
                .location()
    ...
    if args.version {
        println!("qntx-pty-element {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }
    ...
    info!("Initializing QNTX PTY Element Plugin");
    ...
    let service = PTYElementService::new();
    ...
    info!("PTY element plugin shutdown complete");
    Ok(())
}
