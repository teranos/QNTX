//! QNTX PTY Element Plugin Library
//!
//! Provides persistent terminal elements with full PTY support for QNTX.
...
/// PTY Element Plugin Service
pub struct PTYElementService {
    pty_manager: Arc<RwLock<PTYManager>>,
}

impl Default for PTYElementService {
    fn default() -> Self {
        Self::new()
    }
}

impl PTYElementService {
    pub fn new() -> Self {
...
#[tonic::async_trait]
impl DomainPluginService for PTYElementService {
    ...
    async fn metadata(
        ...
        Ok(Response::new(MetadataResponse {
            name: "pty-element".to_string(),
            version: env!("CARGO_PKG_VERSION").to_string(),
            qntx_version: ">=0.1.0".to_string(),
            description: "Persistent terminal elements with full PTY support".to_string(),
    ...
    async fn initialize(
        ...
        info!("Initializing PTY element plugin");
    ...
    async fn shutdown(&self, _request: Request<Empty>) -> Result<Response<Empty>, Status> {
        info!("Shutting down PTY element plugin");
    ...
    async fn handle_http(
        ...
        match (req.method.as_str(), req.path.as_str()) {
            ("GET", "/api/pty-element/pty-element-module.js") => self.handle_element_module().await,
            ("GET", "/api/pty-element/xterm.css") => self.handle_xterm_css().await,
            ("POST", "/api/pty-element/create") => self.handle_create_pty(req).await,
            ("GET", path) if path.starts_with("/api/pty-element/session/") => {
                self.handle_get_session(req).await
            }
            ("DELETE", path) if path.starts_with("/api/pty-element/session/") => {
    ...
    async fn register_elements(
        ...
        debug!("RegisterElements request");
    ...
    async fn execute_job(
        ...
        // PTY element doesn't use background jobs
        Err(Status::unimplemented("Background jobs not supported"))
    }
}

impl PTYElementService {
    async fn handle_element_module(&self) -> Result<Response<HttpResponse>, Status> {
        let js = include_str!("../web/pty-element-module.js");
    ...
    async fn handle_create_pty(&self, req: HttpRequest) -> Result<Response<HttpResponse>, Status> {
        ...
        let element_id = body
            .get("element_id")
            .and_then(|v| v.as_str())
            .ok_or_else(|| Status::invalid_argument("Missing element_id"))?;

        // Create PTY session
        let session_id = self
            .pty_manager
            .write()
            .create_session(element_id)
    ...
    async fn handle_get_session(&self, req: HttpRequest) -> Result<Response<HttpResponse>, Status> {
        ...
        let id = req
            .path
            .strip_prefix("/api/pty-element/session/")
