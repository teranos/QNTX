...
- **PAT** — ~~Performance attestations.~~ Resolved. Each generation writes a `["Performance"]` attestation with timing breakdown (prompt_eval_ms, generation_ms, decode_ms, signal_ms, callback_ms, tok/s). Regressions are detectable from attestation history.

- **HYP** — Hyperbolic nebula (Poincaré ball). Current PCA projection maximizes variance but the axes carry no semantic meaning. Hyperbolic space is the natural geometry for the token vocabulary's tree structure: center = high-frequency common tokens, boundary = rare/specific tokens (boundary is at infinity), angular position = semantic domain. The Poincaré ball produces an infinite fractal structure within a finite sphere — probability mass flowing toward the periphery during uncertain generation reveals endlessly branching clusters of rare tokens. Visually: live generation on a hyperbolic tiling where common tokens glow at the center and uncertainty sends light spiraling into infinite depth at the edge. Computed via Riemannian SGD on a k-NN similarity graph (200 epochs, k=15). Cached as `.vocab3d.hyp`. Switchable at runtime via `POST /projection` or 'p' key in nebula. Additive to PCA — a second visualization mode, not a replacement.
