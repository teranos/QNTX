...
- **SIG** — Signal capture overhead. `capture_signal()` runs softmax + partial sort over the full 128K vocabulary every token (~54ms/token). Actual llama.cpp decode is ~2.8ms/token — signal extraction is 20x more expensive than inference itself. This caps throughput at ~16 tok/s on M1 regardless of model speed. Moving softmax to a Metal compute shader would keep exact results while running on the GPU where the logits already live.

- **PAT** — Performance attestations. `stream_chat()` already measures per-token decode/signal/callback timings but only logs them to stdout. These should be attested — one attestation per generation with timing breakdown — so performance regressions are visible in the attestation graph, not buried in logs.

- **HYP** — Hyperbolic nebula (Poincaré ball). Current PCA projection maximizes variance but the axes carry no semantic meaning. Hyperbolic space is the natural geometry for the token vocabulary's tree structure: center = high-frequency common tokens, boundary = rare/specific tokens (boundary is at infinity), angular position = semantic domain. The Poincaré ball produces an infinite fractal structure within a finite sphere — probability mass flowing toward the periphery during uncertain generation reveals endlessly branching clusters of rare tokens. Visually: live generation on a hyperbolic tiling where common tokens glow at the center and uncertainty sends light spiraling into infinite depth at the edge. Requires computing hyperbolic embeddings (iterative optimization, not a closed-form projection like PCA), but the cache format (`.vocab3d`) and renderer are reusable. Additive to PCA — a second visualization mode, not a replacement.
