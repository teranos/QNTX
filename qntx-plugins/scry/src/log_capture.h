...
public:
    ...
    void flush_summary(const std::string& log_level) {
        ...
        if (!device_name_.empty()) {
            std::cout << "[model] Metal: " << device_name_;
        ...
        if (kv_buffer_mib_ > 0 || compute_buffer_mib_ > 0) {
            std::cout << "[model] Buffers: KV " << kv_buffer_mib_ << " MiB ("
        ...
        if (!model_name_.empty()) {
            std::cout << "[model] Model: " << model_name_;
        ...
        if (metal_load_secs_ > 0) {
            std::cout << "[model] Metal library loaded in " << metal_load_secs_ << " sec" << std::endl;
        }
        if (graph_nodes_ > 0) {
            std::cout << "[model] Graph: " << graph_nodes_ << " nodes, "
                      << graph_splits_ << " splits" << std::endl;
        }
        if (reserve_ms_ > 0) {
            std::cout << "[model] Scheduler reserve took " << reserve_ms_ << " ms" << std::endl;
        }
        if (n_tensors_ > 0) {
            std::cout << "[model] Tensors: " << n_tensors_ << " total" << std::endl;
        }
    }
