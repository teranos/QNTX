...
public:
    ...
    void flush_summary(const std::string& log_level) {
        ...
        for (auto& line : error_lines_) {
            std::cout << "[scry] ERROR: " << line << std::endl;
        }
        ...
        if (!device_name_.empty()) {
            std::cout << "[scry] Metal: " << device_name_;
        ...
        if (kv_buffer_mib_ > 0 || compute_buffer_mib_ > 0) {
            std::cout << "[scry] Buffers: KV " << kv_buffer_mib_ << " MiB ("
        ...
        if (!model_name_.empty()) {
            std::cout << "[scry] Model: " << model_name_;
        ...
        if (metal_load_secs_ > 0) {
            std::cout << "[scry] Metal library loaded in " << metal_load_secs_ << " sec" << std::endl;
        }
        if (graph_nodes_ > 0) {
            std::cout << "[scry] Graph: " << graph_nodes_ << " nodes, "
                      << graph_splits_ << " splits" << std::endl;
        }
        if (reserve_ms_ > 0) {
            std::cout << "[scry] Scheduler reserve took " << reserve_ms_ << " ms" << std::endl;
        }
        if (n_tensors_ > 0) {
            std::cout << "[scry] Tensors: " << n_tensors_ << " total" << std::endl;
        }
    }
