...
private ubyte[] encodeField(FT)(int fieldNum, const FT value) {
    ...
    static if (is(FT == string)) {
        ...
        // Map<string,string>: each entry is an embedded message with fields 1,2
        foreach (k, v; value) {
            ubyte[] entry;
            if (k.length > 0) {
...
struct InitializeResponse {
    ...
    @Proto(3) bool llmProvider; // Registers this plugin as an LLM provider
}
...
// ---------------------------------------------------------------------------
// LLM protocol messages (mirrors llm.proto)
// ---------------------------------------------------------------------------

struct LLMAttachment {
    @Proto(1) string filename;
    @Proto(2) string dataUri;
}

struct LLMChatRequest {
    @Proto(1) string systemPrompt;
    @Proto(2) string userPrompt;
    @Proto(3) string model;
    @Proto(4) double temperature;
    @Proto(5) int maxTokens;
    @Proto(6) string provider;
    @Proto(7) LLMAttachment[] attachments;
}

struct LLMChatResponse {
    @Proto(1) string content;
    @Proto(2) string model;
    @Proto(3) int promptTokens;
    @Proto(4) int completionTokens;
    @Proto(5) int totalTokens;
}

// ---------------------------------------------------------------------------
// ATSStore protocol messages (atsstore.proto)
// ---------------------------------------------------------------------------
...
struct Attestation {
    ...
    @Proto(6) long timestamp;
    @Proto(7) string source;
    @Proto(8) ubyte[] attributes;
    @Proto(9) long createdAt;
    @Proto(10) ubyte[] signature;
    @Proto(11) string signerDid;
}
...
struct AttestationCommand {
    ...
    @Proto(5) long timestamp;
    @Proto(6) ubyte[] attributes; // google.protobuf.Struct as raw bytes
    @Proto(7) string source;
...
// Wrapper since we need to encode the command as embedded message
ubyte[] encodeGenerateAttestationRequest(string authToken, ref const AttestationCommand cmd) {
    ubyte[] result;
    // Field 1: auth_token
    ...
    // Field 2: command (embedded message)
    auto cmdBytes = encode(cmd);
    if (cmdBytes.length > 0) {
        result ~= encodeTag(2, WireType.LengthDelimited);
        result ~= encodeVarint(cmdBytes.length);
        result ~= cmdBytes;
    }
    return result;
}
...
struct GenerateAttestationResponse {
    @Proto(1) bool success;
    @Proto(2) string error;
    @Proto(3) Attestation attestation;
}

// Wrapper since we need to decode the attestation as embedded message
GenerateAttestationResponse decodeGenerateAttestationResponse(const ubyte[] data) {
    GenerateAttestationResponse result;
    size_t pos = 0;
    while (pos < data.length) {
        auto tag = decodeVarint(data, pos);
        int fieldNum = cast(int)(tag >> 3);
        WireType wt = cast(WireType)(tag & 0x7);
        if (fieldNum == 1 && wt == WireType.Varint) {
            result.success = decodeVarint(data, pos) != 0;
        } else if (fieldNum == 2 && wt == WireType.LengthDelimited) {
            auto len = cast(size_t)decodeVarint(data, pos);
            result.error = cast(string)data[pos .. pos + len].idup;
            pos += len;
        } else if (fieldNum == 3 && wt == WireType.LengthDelimited) {
            auto len = cast(size_t)decodeVarint(data, pos);
            result.attestation = decode!Attestation(data[pos .. pos + len]);
            pos += len;
        } else {
            skipField(data, pos, wt);
        }
...
/// Encode a flat string[string] map as a google.protobuf.Struct.
ubyte[] encodeStructFromStringMap(string[string] map) {
    ubyte[] result;
    foreach (k, v; map) {
        ubyte[] valueMsg;
        valueMsg ~= encodeTag(3, WireType.LengthDelimited);
        valueMsg ~= encodeVarint(v.length);
        valueMsg ~= cast(const(ubyte)[])v;

        ubyte[] entryMsg;
        entryMsg ~= encodeTag(1, WireType.LengthDelimited);
        entryMsg ~= encodeVarint(k.length);
        entryMsg ~= cast(const(ubyte)[])k;
        entryMsg ~= encodeTag(2, WireType.LengthDelimited);
        entryMsg ~= encodeVarint(valueMsg.length);
        entryMsg ~= valueMsg;

        result ~= encodeTag(1, WireType.LengthDelimited);
        result ~= encodeVarint(entryMsg.length);
        result ~= entryMsg;
    }
    return result;
}

/// Encode a google.protobuf.Struct with number values.
/// Each pair is: key (string) -> number_value (double, proto field 2 in Value).
ubyte[] encodeStructFromNumberMap(string[string] map) {
    ubyte[] result;
    foreach (k, v; map) {
        // Value message: number_value = field 2, Fixed64 (double)
        ubyte[] valueMsg;
        import std.conv : to;
        double num = 0.0;
        try { num = v.to!double; } catch (Exception) {}
        if (num != 0.0) {
            valueMsg ~= encodeTag(2, WireType.Fixed64);
            auto bits = *cast(const(ubyte)[8]*)&num;
            valueMsg ~= bits[];
        }
...
/// Encode a mixed Struct from string and double values.
ubyte[] encodeStructMixed(string[string] stringFields, string[string] numberFields) {
    ubyte[] result;

    foreach (k, v; stringFields) {
        ubyte[] valueMsg;
        valueMsg ~= encodeTag(3, WireType.LengthDelimited);
        valueMsg ~= encodeVarint(v.length);
        valueMsg ~= cast(const(ubyte)[])v;
...
unittest {
    ...
    foreach (val; [0UL, 1, 127, 128, 300, 16384, ulong.max]) {
        auto encoded = encodeVarint(val);
        size_t pos = 0;
        auto decoded = decodeVarint(encoded, pos);
        assert(decoded == val, "varint round-trip failed");
        assert(pos == encoded.length, "varint consumed wrong number of bytes");
    }

    // Test MetadataResponse encode/decode round-trip
    MetadataResponse meta;
    meta.name = "infer";
    meta.version_ = "0.1.0";
    meta.qntxVersion = ">= 0.1.0";
    meta.description = "Inference attestation";
    meta.author = "QNTX";
    meta.license = "MIT";

    auto encoded = encode(meta);
    auto decoded = decode!MetadataResponse(encoded);
    assert(decoded.name == "infer");
    assert(decoded.version_ == "0.1.0");

    // Test InitializeResponse with llmProvider
    InitializeResponse resp;
    resp.llmProvider = true;
    resp.handlerNames = ["infer.attest"];

    auto encResp = encode(resp);
    auto decResp = decode!InitializeResponse(encResp);
    assert(decResp.llmProvider == true);
    assert(decResp.handlerNames.length == 1);
    assert(decResp.handlerNames[0] == "infer.attest");

    // Test LLMChatRequest encode/decode
    LLMChatRequest chatReq;
    chatReq.systemPrompt = "You are helpful.";
    chatReq.userPrompt = "Hello";
    chatReq.temperature = 0.7;
    chatReq.maxTokens = 512;

    auto encChat = encode(chatReq);
    auto decChat = decode!LLMChatRequest(encChat);
    assert(decChat.systemPrompt == "You are helpful.");
    assert(decChat.userPrompt == "Hello");
    assert(decChat.maxTokens == 512);
}
