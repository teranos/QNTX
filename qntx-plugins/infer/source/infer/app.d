...
import infer.plugin;
import infer.proto;
import infer.version_ : PLUGIN_VERSION;
...
void main(string[] args) {
    ...
    if (actualPort == 0) {
        logError("[infer] failed to bind to port %d (tried 64 ports)", port);
        return;
    }
    ...
    logInfo("[infer] gRPC server listening on 127.0.0.1:%d", actualPort);
    logInfo("[infer] Inference attestation plugin v%s ready", PLUGIN_VERSION);
