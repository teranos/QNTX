...
let test_result_to_json_buffered () =
  let r = Stitcher.{ branch = "main"; context = "session:x"; buffered_words = 42; emitted = None; turn_count = 0; paths = []; timestamp = 0 } in
...
let test_result_to_json_emitted () =
  let r = Stitcher.{ branch = "main"; context = "session:x"; buffered_words = 0; emitted = Some "hello world"; turn_count = 1; paths = []; timestamp = 0 } in
  let json = Yojson.Safe.from_string (Stitcher.result_to_json r) in
  match json with
  | `Assoc fields ->
...
let test_result_to_json_escapes_branch () =
  let r = Stitcher.{ branch = {|feat/"quoted"|}; context = "session:x"; buffered_words = 10; emitted = None; turn_count = 0; paths = []; timestamp = 0 } in
