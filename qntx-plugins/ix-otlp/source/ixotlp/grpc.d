...
struct GrpcClient {
    ...
    bool connect(string endpoint) {
        ...
        if (colonIdx < 0) {
            logError("[ix-otlp] gRPC client: invalid endpoint %s (no port)", endpoint);
            return false;
        }
        ...
        try {
            ...
            logError("[ix-otlp] gRPC client: TCP connect to %s failed: %s", endpoint, e.msg);
            sock = null;
            return false;
        }
        ...
        if (serverSettings is null) {
            logError("[ix-otlp] gRPC client: no SETTINGS from %s (timeout or connection closed)", endpoint);
            sock.close();
            sock = null;
            return false;
        }
        ...
        if (maxReads == 0) {
            logError("[ix-otlp] gRPC client: SETTINGS ACK not received from %s after 10 frames", endpoint);
            sock.close();
            sock = null;
            return false;
        }

        logInfo("[ix-otlp] gRPC client: connected to %s", endpoint);
        return true;
    }
    ...
    ubyte[] call(string method, const ubyte[] requestProto) {
        if (sock is null) {
            logError("[ix-otlp] gRPC client: call %s failed — not connected", method);
            return [];
        }
