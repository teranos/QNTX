...
// Integration tests for temporal expressions in ax command context
func TestTemporalExpressionIntegration(t *testing.T) {
	// Mock time for deterministic testing
	mockNow := time.Date(2024, 6, 15, 14, 30, 0, 0, time.UTC) // Saturday
	originalTimeNow := timeNow
	timeNow = func() time.Time { return mockNow }
	...
	tests := []struct {
		...
		// Since expressions
		{
			...
			temporalTokens: []string{"since", "yesterday"},
			expectedStart: func() *time.Time {
				t := mockNow.AddDate(0, 0, -1)
				return &t
			}(),
		},
		{
			name:           "since last week",
			temporalTokens: []string{"since", "last", "week"},
			expectedStart: func() *time.Time {
				t := mockNow.AddDate(0, 0, -7)
				return &t
			}(),
		...
		{
			name:           "since ISO date",
			temporalTokens: []string{"since", "2024-01-01"},
			expectedStart: func() *time.Time {
				t := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
				return &t
			}(),
		...
		{
			name:           "until tomorrow",
			temporalTokens: []string{"until", "tomorrow"},
			expectedEnd: func() *time.Time {
				t := mockNow.AddDate(0, 0, 1)
				return &t
			}(),
		},
		{
			name:           "until next month",
			temporalTokens: []string{"until", "next", "month"},
			expectedEnd: func() *time.Time {
				t := mockNow.AddDate(0, 1, 0)
				return &t
			}(),
		...
		// On expressions (creates range)
		{
			name:           "on specific date",
			temporalTokens: []string{"on", "2024-05-15"},
			expectedStart: func() *time.Time {
				t := time.Date(2024, 5, 15, 0, 0, 0, 0, time.UTC)
				return &t
			}(),
			expectedEnd: func() *time.Time {
				t := time.Date(2024, 5, 16, 0, 0, 0, 0, time.UTC)
				return &t
			}(),
		},
		...
		// Between expressions
		{
			name:           "between yesterday and today",
			temporalTokens: []string{"between", "yesterday", "and", "today"},
			expectedStart: func() *time.Time {
				t := mockNow.AddDate(0, 0, -1)
				return &t
			}(),
		...
		{
			name:           "between dates",
			temporalTokens: []string{"between", "2024-01-01", "and", "2024-01-31"},
			expectedStart: func() *time.Time {
				t := time.Date(2024, 1, 1, 0, 0, 0, 0, time.UTC)
				return &t
			}(),
		...
		// Error cases
		{
			...
			temporalTokens: []string{"between", "yesterday", "tomorrow"},
			wantErr:        true,
			errContains:    "between' requires format 'between X and Y'",
		},
		{
			name:           "since invalid date",
			temporalTokens: []string{"since", "invalid-date"},
			wantErr:        true,
			errContains:    "invalid 'since' expression",
		},
		{
			name:           "until malformed date",
			temporalTokens: []string{"until", "2024-13-45"},
			wantErr:        true,
			errContains:    "invalid 'until' expression",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create a mock ask filter to test temporal parsing
			filter := &types.AxFilter{}

			// Create mock parser to test temporal segment parsing
			parser := &axParser{warnings: []string{}}
			err := parser.parseTemporalSegment(tt.temporalTokens, filter)

			if tt.wantErr {
				assert.Error(t, err, "Expected error for temporal tokens: %v", tt.temporalTokens)
				if tt.errContains != "" {
					assert.Contains(t, err.Error(), tt.errContains, "Error should contain expected message")
				}
				return
			...
			assert.NoError(t, err, "Unexpected error for temporal tokens: %v", tt.temporalTokens)

			// Validate start time
			if tt.expectedStart != nil {
				assert.NotNil(t, filter.TimeStart, "TimeStart should not be nil")
				assert.Equal(t, tt.expectedStart.Year(), filter.TimeStart.Year(), "Start year mismatch")
				assert.Equal(t, tt.expectedStart.Month(), filter.TimeStart.Month(), "Start month mismatch")
				assert.Equal(t, tt.expectedStart.Day(), filter.TimeStart.Day(), "Start day mismatch")
			} else {
				assert.Nil(t, filter.TimeStart, "TimeStart should be nil")
			}

			// Validate end time
			if tt.expectedEnd != nil {
				assert.NotNil(t, filter.TimeEnd, "TimeEnd should not be nil")
				assert.Equal(t, tt.expectedEnd.Year(), filter.TimeEnd.Year(), "End year mismatch")
				assert.Equal(t, tt.expectedEnd.Month(), filter.TimeEnd.Month(), "End month mismatch")
				assert.Equal(t, tt.expectedEnd.Day(), filter.TimeEnd.Day(), "End day mismatch")
			} else {
				assert.Nil(t, filter.TimeEnd, "TimeEnd should be nil")
			}
		})
	}
}
