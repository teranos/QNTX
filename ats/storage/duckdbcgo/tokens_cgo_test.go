...
func TestCreateReturnsAUsableToken(t *testing.T) {
	...
	raw, id, err := store.Create(auth.NewToken{Label: "laptop-cron", ExpiresAt: nil, MintedBy: "https://chaos.social/@onf", Namespace: NamespaceDefault, ScopeRead: []string{"noted"}, ScopeWrite: []string{"ingested"}})
	if err != nil {
...
func TestTokensSurviveReopen(t *testing.T) {
	...
	raw, _, err := first.Create(auth.NewToken{Label: "laptop-cron", ExpiresAt: nil, MintedBy: "https://chaos.social/@onf", Namespace: NamespaceDefault, ScopeRead: []string{"noted"}, ScopeWrite: []string{"ingested"}})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	first.Close()
...
func TestListKeepsRevokedTokens(t *testing.T) {
	store := newStore(t)
	_, id, err := store.Create(auth.NewToken{Label: "laptop-cron", ExpiresAt: nil, MintedBy: "https://chaos.social/@onf", Namespace: NamespaceDefault, ScopeRead: []string{"noted"}, ScopeWrite: []string{"ingested"}})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	if err := store.Revoke(id); err != nil {
...
func TestExpiredTokenDoesNotAuthenticate(t *testing.T) {
	...
	raw, _, err := store.Create(auth.NewToken{Label: "laptop-cron", ExpiresAt: &past, MintedBy: "https://chaos.social/@onf", Namespace: NamespaceDefault, ScopeRead: []string{"noted"}, ScopeWrite: []string{"ingested"}})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}
	if store.lookupOK(hashOf(raw)) {
...
func TestRevokeHitsOnlyItsOwnToken(t *testing.T) {
	store := newStore(t)
	rawA, idA, err := store.Create(auth.NewToken{Label: "a", ExpiresAt: nil, MintedBy: "https://chaos.social/@onf", Namespace: NamespaceDefault, ScopeRead: []string{"noted"}, ScopeWrite: []string{"ingested"}})
	if err != nil {
		t.Fatalf("Create a: %v", err)
	}
	rawB, _, err := store.Create(auth.NewToken{Label: "b", ExpiresAt: nil, MintedBy: "https://chaos.social/@onf", Namespace: NamespaceDefault, ScopeRead: []string{"noted"}, ScopeWrite: []string{"ingested"}})
	if err != nil {
