...
func ExpandCartesianClaims(attestations []types.As) []IndividualClaim {
	...
	if err != nil {
		panic("WASM engine unavailable for expand: " + err.Error() + " — run `make ats`")
	}
...
func GroupClaimsByKey(claims []IndividualClaim) map[ClaimKey][]IndividualClaim {
	engine, err := wasm.GetEngine()
	if err != nil {
		panic("WASM engine unavailable for group_claims: " + err.Error() + " — run `make ats`")
	}
...
func ConvertClaimsToAttestations(claims []IndividualClaim) []types.As {
	engine, err := wasm.GetEngine()
	if err != nil {
		panic("WASM engine unavailable for dedup_source_ids: " + err.Error() + " — run `make ats`")
	}
