(********************************************************)
(*           AUTOGENERATED FILE - DO NOT EDIT!          *)
(********************************************************)
(* Generated by: ocaml-protoc-plugin                    *)
(* https://github.com/andersfugmann/ocaml-protoc-plugin *)
(********************************************************)
(*
  Source: plugin/grpc/protocol/schedule.proto
  Syntax: proto3
  Parameters:
    debug=false
    annot=''
    opens=[]
    int64_as_int=true
    int32_as_int=true
    fixed_as_int=false
    singleton_record=false
    prefix_output_with_package=false
*)
[@@@ocaml.alert "-protobuf"] (* Disable deprecation warnings for protobuf*)
(**/**)
module Runtime' = Ocaml_protoc_plugin [@@warning "-33"]
module Imported'modules = struct
  module Struct = Struct
end
(**/**)
module rec Protocol : sig

  (**
{%html:
<p>Value sets, not field types — the fields below stay string because
declarations are already persisted with these exact bytes. Names are
lowercase so String() returns the wire value.</p>
%}
  *)
  module rec ScheduleState : sig
    type t =
      | Active
      | Paused
      | Stopping
      | Inactive
      | Deleted

    val name: unit -> string
    (** Fully qualified protobuf name of this enum *)

    (**/**)
    val to_int: t -> int
    val from_int: int -> t Runtime'.Result.t
    val from_int_exn: int -> t
    val to_string: t -> string
    val from_string_exn: string -> t
    (**/**)
  end

  and ExecutionStatus : sig
    type t =
      | Running
      | Completed
      | Failed

    val name: unit -> string
    (** Fully qualified protobuf name of this enum *)

    (**/**)
    val to_int: t -> int
    val from_int: int -> t Runtime'.Result.t
    val from_int_exn: int -> t
    val to_string: t -> string
    val from_string_exn: string -> t
    (**/**)
  end


  (**
{%html:
<p>A schedule as declared — what someone decided, and all of it (ADR-028).
Changes when a person or a plugin changes it, never on a tick.</p>
%}
  *)
  and ScheduleDeclaration : sig
    type t = {
      id:string;
      handler_name:string;
      payload:bytes;
      source_url:string;
      interval_seconds:int;
      state:string;
      (**
{%html:
<p>one of ScheduleState</p>
%}
      *)

      created_from_doc:string;
      metadata:string;
      created_at_ms:int;
      first_run_at_ms:int;
      (**
{%html:
<p>later runs come from the ticks</p>
%}
      *)

    }
    val make: ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at_ms:int -> ?first_run_at_ms:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at_ms:int -> ?first_run_at_ms:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>One thing that happened to a schedule. An empty execution_id means the next
run moved without a run happening — a force trigger.</p>
%}
  *)
  and ScheduleTick : sig
    type t = {
      schedule_id:string;
      at_ms:int;
      execution_id:string;
      next_run_at_ms:int;
    }
    val make: ?schedule_id:string -> ?at_ms:int -> ?execution_id:string -> ?next_run_at_ms:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?schedule_id:string -> ?at_ms:int -> ?execution_id:string -> ?next_run_at_ms:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>One run of a scheduled job: timing, status, output, and the async job it
linked to. Execution history for debugging and failure troubleshooting.
optional marks what Go holds as a pointer with omitempty, so absence stays
absence and the JSON the web reads does not change shape.</p>
%}
  *)
  and Execution : sig
    type t = {
      id:string;
      scheduled_job_id:string;
      async_job_id:string option;
      status:string;
      (**
{%html:
<p>one of ExecutionStatus</p>
%}
      *)

      started_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      completed_at:string option;
      duration_ms:int option;
      logs:string option;
      result_summary:string option;
      error_message:string option;
      created_at:string;
      updated_at:string;
    }
    val make: ?id:string -> ?scheduled_job_id:string -> ?async_job_id:string -> ?status:string -> ?started_at:string -> ?completed_at:string -> ?duration_ms:int -> ?logs:string -> ?result_summary:string -> ?error_message:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?scheduled_job_id:string -> ?async_job_id:string -> ?status:string -> ?started_at:string -> ?completed_at:string -> ?duration_ms:int -> ?logs:string -> ?result_summary:string -> ?error_message:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>A task within a stage, with its log count.</p>
%}
  *)
  and TaskInfo : sig
    type t = {
      task_id:string;
      log_count:int option;
    }
    val make: ?task_id:string -> ?log_count:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?task_id:string -> ?log_count:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>A stage with its tasks.</p>
%}
  *)
  and StageInfo : sig
    type t = {
      stage:string;
      tasks:TaskInfo.t list;
    }
    val make: ?stage:string -> ?tasks:TaskInfo.t list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?stage:string -> ?tasks:TaskInfo.t list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>GET /pulse/jobs/{id}/executions</p>
%}
  *)
  and ListExecutionsResponse : sig
    type t = {
      executions:Execution.t list;
      count:int;
      total:int;
      has_more:bool;
    }
    val make: ?executions:Execution.t list -> ?count:int -> ?total:int -> ?has_more:bool -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?executions:Execution.t list -> ?count:int -> ?total:int -> ?has_more:bool -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>GET /jobs/{job_id}/stages</p>
%}
  *)
  and JobStagesResponse : sig
    type t = {
      job_id:string;
      stages:StageInfo.t list;
      plugin_version:string option;
    }
    val make: ?job_id:string -> ?stages:StageInfo.t list -> ?plugin_version:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?job_id:string -> ?stages:StageInfo.t list -> ?plugin_version:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>GET /tasks/{task_id}/logs</p>
%}
  *)
  and TaskLogsResponse : sig
    type t = {
      task_id:string;
      logs:LogEntry.t list;
    }
    val make: ?task_id:string -> ?logs:LogEntry.t list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?task_id:string -> ?logs:LogEntry.t list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>A single log line from a task execution.</p>
%}
  *)
  and LogEntry : sig
    type t = {
      timestamp:string;
      level:string;
      message:string;
      metadata:Imported'modules.Struct.Google.Protobuf.Struct.t option;
    }
    val make: ?timestamp:string -> ?level:string -> ?message:string -> ?metadata:Imported'modules.Struct.Google.Protobuf.Struct.t -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?timestamp:string -> ?level:string -> ?message:string -> ?metadata:Imported'modules.Struct.Google.Protobuf.Struct.t -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>The inputs a force trigger needs.</p>
%}
  *)
  and ForceTriggerParams : sig
    type t = {
      handler_name:string;
      payload:bytes;
      source_url:string;
      async_job_id:string;
    }
    val make: ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?async_job_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?async_job_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>What a force trigger created.</p>
%}
  *)
  and ForceTriggerResult : sig
    type t = {
      scheduled_job_id:string;
      execution_id:string;
      created_new_job:bool;
    }
    val make: ?scheduled_job_id:string -> ?execution_id:string -> ?created_new_job:bool -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?scheduled_job_id:string -> ?execution_id:string -> ?created_new_job:bool -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>What the ticks derive. The mutable columns of scheduled_pulse_jobs, which
this shape computes rather than stores.</p>
%}
  *)
  and ScheduleProgress : sig
    type t = {
      run_count:int;
      last_run_at_ms:int;
      (**
{%html:
<p>0 if never run</p>
%}
      *)

      last_execution_id:string;
      next_run_at_ms:int;
      (**
{%html:
<p>0 if the declaration's first run stands</p>
%}
      *)

    }
    val make: ?run_count:int -> ?last_run_at_ms:int -> ?last_execution_id:string -> ?next_run_at_ms:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?run_count:int -> ?last_run_at_ms:int -> ?last_execution_id:string -> ?next_run_at_ms:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>ScheduledJob is the read view: the declaration plus what the ticks derive.
Storage holds the two above; this is what a caller is handed.</p>
%}
  *)
  and ScheduledJob : sig
    type t = {
      id:string;
      handler_name:string;
      payload:bytes;
      source_url:string;
      interval_seconds:int;
      next_run_at:string;
      (**
{%html:
<p>RFC3339 timestamp</p>
%}
      *)

      last_run_at:string;
      (**
{%html:
<p>RFC3339 timestamp (empty if never run)</p>
%}
      *)

      last_execution_id:string;
      state:string;
      (**
{%html:
<p>one of ScheduleState</p>
%}
      *)

      metadata:string;
      created_at:string;
      (**
{%html:
<p>RFC3339 timestamp</p>
%}
      *)

      updated_at:string;
      (**
{%html:
<p>RFC3339 timestamp</p>
%}
      *)

      created_from_doc:string;
    }
    val make: ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> ?created_from_doc:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> ?created_from_doc:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and CreateScheduleRequest : sig
    type t = {
      auth_token:string;
      handler_name:string;
      (**
{%html:
<p>Handler to invoke on each tick</p>
%}
      *)

      interval_seconds:int;
      (**
{%html:
<p>Execution interval</p>
%}
      *)

      payload:bytes;
      (**
{%html:
<p>Handler-specific data (JSON)</p>
%}
      *)

      metadata:(string * string) list;
      (**
{%html:
<p>Optional metadata (glyph_id, plugin, etc.)</p>
%}
      *)

    }
    val make: ?auth_token:string -> ?handler_name:string -> ?interval_seconds:int -> ?payload:bytes -> ?metadata:(string * string) list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?handler_name:string -> ?interval_seconds:int -> ?payload:bytes -> ?metadata:(string * string) list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and CreateScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
      schedule_id:string;
      (**
{%html:
<p>The created schedule ID</p>
%}
      *)

    }
    val make: ?success:bool -> ?error:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and PauseScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and PauseScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
    }
    val make: ?success:bool -> ?error:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and ResumeScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and ResumeScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
    }
    val make: ?success:bool -> ?error:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and DeleteScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and DeleteScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
    }
    val make: ?success:bool -> ?error:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and GetScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and GetScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
      job:ScheduledJob.t option;
    }
    val make: ?success:bool -> ?error:string -> ?job:ScheduledJob.t -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> ?job:ScheduledJob.t -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>What /api/pulse/schedules and /api/pulse/jobs answer with, as the browser
receives it. Mirrors the structs in server/pulse_types.go: Go keeps those for
the json tags (ADR-006) and the shape is declared here.</p>
<p>Distinct from ScheduledJob above, which is the same concept over gRPC. These
carry timestamps as RFC3339 strings, which is what the JSON API has always
sent, and say optional wherever Go says omitempty.</p>
%}
  *)
  and ScheduledJobResponse : sig
    type t = {
      id:string;
      handler_name:string;
      interval_seconds:int option;
      next_run_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      last_run_at:string option;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      last_execution_id:string option;
      (**
{%html:
<p>Last async job it started</p>
%}
      *)

      state:string;
      (**
{%html:
<p>active, paused, stopping, inactive</p>
%}
      *)

      created_from_doc:string option;
      (**
{%html:
<p>ProseMirror document it came from</p>
%}
      *)

      metadata:string option;
      (**
{%html:
<p>JSON</p>
%}
      *)

      created_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      updated_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

    }
    val make: ?id:string -> ?handler_name:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and CreateScheduledJobRequest : sig
    type t = {
      handler_name:string;
      interval_seconds:int;
      created_from_doc:string option;
      metadata:string option;
      (**
{%html:
<p>JSON</p>
%}
      *)

      force:bool option;
      (**
{%html:
<p>Bypass deduplication</p>
%}
      *)

    }
    val make: ?handler_name:string -> ?interval_seconds:int -> ?created_from_doc:string -> ?metadata:string -> ?force:bool -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?handler_name:string -> ?interval_seconds:int -> ?created_from_doc:string -> ?metadata:string -> ?force:bool -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and UpdateScheduledJobRequest : sig
    type t = {
      state:string option;
      (**
{%html:
<p>active, paused, stopping, inactive</p>
%}
      *)

      interval_seconds:int option;
    }
    val make: ?state:string -> ?interval_seconds:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?state:string -> ?interval_seconds:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and ListScheduledJobsResponse : sig
    type t = {
      jobs:ScheduledJobResponse.t list;
      count:int option;
    }
    val make: ?jobs:ScheduledJobResponse.t list -> ?count:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?jobs:ScheduledJobResponse.t list -> ?count:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>ChildJobInfo is one task under a parent job.</p>
%}
  *)
  and ChildJobInfo : sig
    type t = {
      id:string;
      handler_name:string;
      source:string;
      status:string;
      progress_pct:float option;
      cost_estimate:float option;
      cost_actual:float option;
      error:string option;
      created_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      started_at:string option;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      completed_at:string option;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

    }
    val make: ?id:string -> ?handler_name:string -> ?source:string -> ?status:string -> ?progress_pct:float -> ?cost_estimate:float -> ?cost_actual:float -> ?error:string -> ?created_at:string -> ?started_at:string -> ?completed_at:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?source:string -> ?status:string -> ?progress_pct:float -> ?cost_estimate:float -> ?cost_actual:float -> ?error:string -> ?created_at:string -> ?started_at:string -> ?completed_at:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  and JobChildrenResponse : sig
    type t = {
      parent_job_id:string;
      children:ChildJobInfo.t list;
    }
    val make: ?parent_job_id:string -> ?children:ChildJobInfo.t list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?parent_job_id:string -> ?children:ChildJobInfo.t list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end


  (**
{%html:
<p>ErrorResponse is an API error with the structured context behind it.</p>
%}
  *)
  and ErrorResponse : sig
    type t = {
      error:string;
      details:string list;
      (**
{%html:
<p>From the error chain</p>
%}
      *)

    }
    val make: ?error:string -> ?details:string list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?error:string -> ?details:string list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end

  module ScheduleService : sig
    module CreateSchedule : sig
      include Runtime'.Service.Rpc with type Request.t = CreateScheduleRequest.t and type Response.t = CreateScheduleResponse.t
      module Request : Runtime'.Spec.Message with type t = CreateScheduleRequest.t and type make_t = CreateScheduleRequest.make_t
      (** Module alias for the request message for this method call *)

      module Response : Runtime'.Spec.Message with type t = CreateScheduleResponse.t and type make_t = CreateScheduleResponse.make_t
      (** Module alias for the response message for this method call *)

    end

    val createSchedule : (module Runtime'.Spec.Message with type t = CreateScheduleRequest.t) * (module Runtime'.Spec.Message with type t = CreateScheduleResponse.t)
    module PauseSchedule : sig
      include Runtime'.Service.Rpc with type Request.t = PauseScheduleRequest.t and type Response.t = PauseScheduleResponse.t
      module Request : Runtime'.Spec.Message with type t = PauseScheduleRequest.t and type make_t = PauseScheduleRequest.make_t
      (** Module alias for the request message for this method call *)

      module Response : Runtime'.Spec.Message with type t = PauseScheduleResponse.t and type make_t = PauseScheduleResponse.make_t
      (** Module alias for the response message for this method call *)

    end

    val pauseSchedule : (module Runtime'.Spec.Message with type t = PauseScheduleRequest.t) * (module Runtime'.Spec.Message with type t = PauseScheduleResponse.t)
    module ResumeSchedule : sig
      include Runtime'.Service.Rpc with type Request.t = ResumeScheduleRequest.t and type Response.t = ResumeScheduleResponse.t
      module Request : Runtime'.Spec.Message with type t = ResumeScheduleRequest.t and type make_t = ResumeScheduleRequest.make_t
      (** Module alias for the request message for this method call *)

      module Response : Runtime'.Spec.Message with type t = ResumeScheduleResponse.t and type make_t = ResumeScheduleResponse.make_t
      (** Module alias for the response message for this method call *)

    end

    val resumeSchedule : (module Runtime'.Spec.Message with type t = ResumeScheduleRequest.t) * (module Runtime'.Spec.Message with type t = ResumeScheduleResponse.t)
    module DeleteSchedule : sig
      include Runtime'.Service.Rpc with type Request.t = DeleteScheduleRequest.t and type Response.t = DeleteScheduleResponse.t
      module Request : Runtime'.Spec.Message with type t = DeleteScheduleRequest.t and type make_t = DeleteScheduleRequest.make_t
      (** Module alias for the request message for this method call *)

      module Response : Runtime'.Spec.Message with type t = DeleteScheduleResponse.t and type make_t = DeleteScheduleResponse.make_t
      (** Module alias for the response message for this method call *)

    end

    val deleteSchedule : (module Runtime'.Spec.Message with type t = DeleteScheduleRequest.t) * (module Runtime'.Spec.Message with type t = DeleteScheduleResponse.t)
    module GetSchedule : sig
      include Runtime'.Service.Rpc with type Request.t = GetScheduleRequest.t and type Response.t = GetScheduleResponse.t
      module Request : Runtime'.Spec.Message with type t = GetScheduleRequest.t and type make_t = GetScheduleRequest.make_t
      (** Module alias for the request message for this method call *)

      module Response : Runtime'.Spec.Message with type t = GetScheduleResponse.t and type make_t = GetScheduleResponse.make_t
      (** Module alias for the response message for this method call *)

    end

    val getSchedule : (module Runtime'.Spec.Message with type t = GetScheduleRequest.t) * (module Runtime'.Spec.Message with type t = GetScheduleResponse.t)
  end

end = struct
  module rec ScheduleState : sig
    type t =
      | Active
      | Paused
      | Stopping
      | Inactive
      | Deleted

    val name: unit -> string
    (** Fully qualified protobuf name of this enum *)

    (**/**)
    val to_int: t -> int
    val from_int: int -> t Runtime'.Result.t
    val from_int_exn: int -> t
    val to_string: t -> string
    val from_string_exn: string -> t
    (**/**)
  end = struct
    module This'_ = ScheduleState
    type t =
      | Active
      | Paused
      | Stopping
      | Inactive
      | Deleted

    let name () = ".protocol.ScheduleState"
    let to_int = function
      | Active -> 0
      | Paused -> 1
      | Stopping -> 2
      | Inactive -> 3
      | Deleted -> 4
    let from_int_exn = function
      | 0 -> Active
      | 1 -> Paused
      | 2 -> Stopping
      | 3 -> Inactive
      | 4 -> Deleted
      | n -> Runtime'.Result.raise (`Unknown_enum_value n)
    let from_int e = Runtime'.Result.catch (fun () -> from_int_exn e)
    let to_string = function
      | Active -> "active"
      | Paused -> "paused"
      | Stopping -> "stopping"
      | Inactive -> "inactive"
      | Deleted -> "deleted"
    let from_string_exn = function
      | "active" -> Active
      | "paused" -> Paused
      | "stopping" -> Stopping
      | "inactive" -> Inactive
      | "deleted" -> Deleted
      | s -> Runtime'.Result.raise (`Unknown_enum_name s)

  end

  and ExecutionStatus : sig
    type t =
      | Running
      | Completed
      | Failed

    val name: unit -> string
    (** Fully qualified protobuf name of this enum *)

    (**/**)
    val to_int: t -> int
    val from_int: int -> t Runtime'.Result.t
    val from_int_exn: int -> t
    val to_string: t -> string
    val from_string_exn: string -> t
    (**/**)
  end = struct
    module This'_ = ExecutionStatus
    type t =
      | Running
      | Completed
      | Failed

    let name () = ".protocol.ExecutionStatus"
    let to_int = function
      | Running -> 0
      | Completed -> 1
      | Failed -> 2
    let from_int_exn = function
      | 0 -> Running
      | 1 -> Completed
      | 2 -> Failed
      | n -> Runtime'.Result.raise (`Unknown_enum_value n)
    let from_int e = Runtime'.Result.catch (fun () -> from_int_exn e)
    let to_string = function
      | Running -> "running"
      | Completed -> "completed"
      | Failed -> "failed"
    let from_string_exn = function
      | "running" -> Running
      | "completed" -> Completed
      | "failed" -> Failed
      | s -> Runtime'.Result.raise (`Unknown_enum_name s)

  end

  and ScheduleDeclaration : sig
    type t = {
      id:string;
      handler_name:string;
      payload:bytes;
      source_url:string;
      interval_seconds:int;
      state:string;
      (**
{%html:
<p>one of ScheduleState</p>
%}
      *)

      created_from_doc:string;
      metadata:string;
      created_at_ms:int;
      first_run_at_ms:int;
      (**
{%html:
<p>later runs come from the ticks</p>
%}
      *)

    }
    val make: ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at_ms:int -> ?first_run_at_ms:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at_ms:int -> ?first_run_at_ms:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ScheduleDeclaration
    let name () = ".protocol.ScheduleDeclaration"
    type t = {
      id:string;
      handler_name:string;
      payload:bytes;
      source_url:string;
      interval_seconds:int;
      state:string;
      created_from_doc:string;
      metadata:string;
      created_at_ms:int;
      first_run_at_ms:int;
    }
    type make_t = ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at_ms:int -> ?first_run_at_ms:int -> unit -> t
    let make ?(id = {||}) ?(handler_name = {||}) ?(payload = (Bytes.of_string {||})) ?(source_url = {||}) ?(interval_seconds = 0) ?(state = {||}) ?(created_from_doc = {||}) ?(metadata = {||}) ?(created_at_ms = 0) ?(first_run_at_ms = 0) () = { id; handler_name; payload; source_url; interval_seconds; state; created_from_doc; metadata; created_at_ms; first_run_at_ms }
    let merge =
    let merge_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ) in
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_payload = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ) in
    let merge_source_url = Runtime'.Merge.merge Runtime'.Spec.( basic ((5, "source_url", "sourceUrl"), string, ({||})) ) in
    let merge_interval_seconds = Runtime'.Merge.merge Runtime'.Spec.( basic ((6, "interval_seconds", "intervalSeconds"), int32_int, (0)) ) in
    let merge_state = Runtime'.Merge.merge Runtime'.Spec.( basic ((7, "state", "state"), string, ({||})) ) in
    let merge_created_from_doc = Runtime'.Merge.merge Runtime'.Spec.( basic ((8, "created_from_doc", "createdFromDoc"), string, ({||})) ) in
    let merge_metadata = Runtime'.Merge.merge Runtime'.Spec.( basic ((9, "metadata", "metadata"), string, ({||})) ) in
    let merge_created_at_ms = Runtime'.Merge.merge Runtime'.Spec.( basic ((10, "created_at_ms", "createdAtMs"), int64_int, (0)) ) in
    let merge_first_run_at_ms = Runtime'.Merge.merge Runtime'.Spec.( basic ((11, "first_run_at_ms", "firstRunAtMs"), int64_int, (0)) ) in
    fun t1 t2 -> {
    	id = (merge_id t1.id t2.id);
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	payload = (merge_payload t1.payload t2.payload);
    	source_url = (merge_source_url t1.source_url t2.source_url);
    	interval_seconds = (merge_interval_seconds t1.interval_seconds t2.interval_seconds);
    	state = (merge_state t1.state t2.state);
    	created_from_doc = (merge_created_from_doc t1.created_from_doc t2.created_from_doc);
    	metadata = (merge_metadata t1.metadata t2.metadata);
    	created_at_ms = (merge_created_at_ms t1.created_at_ms t2.created_at_ms);
    	first_run_at_ms = (merge_first_run_at_ms t1.first_run_at_ms t2.first_run_at_ms);
     }
    let spec () = Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ^:: basic ((3, "handler_name", "handlerName"), string, ({||})) ^:: basic ((4, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ^:: basic ((5, "source_url", "sourceUrl"), string, ({||})) ^:: basic ((6, "interval_seconds", "intervalSeconds"), int32_int, (0)) ^:: basic ((7, "state", "state"), string, ({||})) ^:: basic ((8, "created_from_doc", "createdFromDoc"), string, ({||})) ^:: basic ((9, "metadata", "metadata"), string, ({||})) ^:: basic ((10, "created_at_ms", "createdAtMs"), int64_int, (0)) ^:: basic ((11, "first_run_at_ms", "firstRunAtMs"), int64_int, (0)) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { id; handler_name; payload; source_url; interval_seconds; state; created_from_doc; metadata; created_at_ms; first_run_at_ms } -> serialize writer id handler_name payload source_url interval_seconds state created_from_doc metadata created_at_ms first_run_at_ms

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor id handler_name payload source_url interval_seconds state created_from_doc metadata created_at_ms first_run_at_ms = { id; handler_name; payload; source_url; interval_seconds; state; created_from_doc; metadata; created_at_ms; first_run_at_ms } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { id; handler_name; payload; source_url; interval_seconds; state; created_from_doc; metadata; created_at_ms; first_run_at_ms } -> serialize id handler_name payload source_url interval_seconds state created_from_doc metadata created_at_ms first_run_at_ms
    let from_json_exn =
      let constructor id handler_name payload source_url interval_seconds state created_from_doc metadata created_at_ms first_run_at_ms = { id; handler_name; payload; source_url; interval_seconds; state; created_from_doc; metadata; created_at_ms; first_run_at_ms } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ScheduleTick : sig
    type t = {
      schedule_id:string;
      at_ms:int;
      execution_id:string;
      next_run_at_ms:int;
    }
    val make: ?schedule_id:string -> ?at_ms:int -> ?execution_id:string -> ?next_run_at_ms:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?schedule_id:string -> ?at_ms:int -> ?execution_id:string -> ?next_run_at_ms:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ScheduleTick
    let name () = ".protocol.ScheduleTick"
    type t = {
      schedule_id:string;
      at_ms:int;
      execution_id:string;
      next_run_at_ms:int;
    }
    type make_t = ?schedule_id:string -> ?at_ms:int -> ?execution_id:string -> ?next_run_at_ms:int -> unit -> t
    let make ?(schedule_id = {||}) ?(at_ms = 0) ?(execution_id = {||}) ?(next_run_at_ms = 0) () = { schedule_id; at_ms; execution_id; next_run_at_ms }
    let merge =
    let merge_schedule_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "schedule_id", "scheduleId"), string, ({||})) ) in
    let merge_at_ms = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "at_ms", "atMs"), int64_int, (0)) ) in
    let merge_execution_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "execution_id", "executionId"), string, ({||})) ) in
    let merge_next_run_at_ms = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "next_run_at_ms", "nextRunAtMs"), int64_int, (0)) ) in
    fun t1 t2 -> {
    	schedule_id = (merge_schedule_id t1.schedule_id t2.schedule_id);
    	at_ms = (merge_at_ms t1.at_ms t2.at_ms);
    	execution_id = (merge_execution_id t1.execution_id t2.execution_id);
    	next_run_at_ms = (merge_next_run_at_ms t1.next_run_at_ms t2.next_run_at_ms);
     }
    let spec () = Runtime'.Spec.( basic ((1, "schedule_id", "scheduleId"), string, ({||})) ^:: basic ((2, "at_ms", "atMs"), int64_int, (0)) ^:: basic ((3, "execution_id", "executionId"), string, ({||})) ^:: basic ((4, "next_run_at_ms", "nextRunAtMs"), int64_int, (0)) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { schedule_id; at_ms; execution_id; next_run_at_ms } -> serialize writer schedule_id at_ms execution_id next_run_at_ms

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor schedule_id at_ms execution_id next_run_at_ms = { schedule_id; at_ms; execution_id; next_run_at_ms } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { schedule_id; at_ms; execution_id; next_run_at_ms } -> serialize schedule_id at_ms execution_id next_run_at_ms
    let from_json_exn =
      let constructor schedule_id at_ms execution_id next_run_at_ms = { schedule_id; at_ms; execution_id; next_run_at_ms } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and Execution : sig
    type t = {
      id:string;
      scheduled_job_id:string;
      async_job_id:string option;
      status:string;
      (**
{%html:
<p>one of ExecutionStatus</p>
%}
      *)

      started_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      completed_at:string option;
      duration_ms:int option;
      logs:string option;
      result_summary:string option;
      error_message:string option;
      created_at:string;
      updated_at:string;
    }
    val make: ?id:string -> ?scheduled_job_id:string -> ?async_job_id:string -> ?status:string -> ?started_at:string -> ?completed_at:string -> ?duration_ms:int -> ?logs:string -> ?result_summary:string -> ?error_message:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?scheduled_job_id:string -> ?async_job_id:string -> ?status:string -> ?started_at:string -> ?completed_at:string -> ?duration_ms:int -> ?logs:string -> ?result_summary:string -> ?error_message:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = Execution
    let name () = ".protocol.Execution"
    type t = {
      id:string;
      scheduled_job_id:string;
      async_job_id:string option;
      status:string;
      started_at:string;
      completed_at:string option;
      duration_ms:int option;
      logs:string option;
      result_summary:string option;
      error_message:string option;
      created_at:string;
      updated_at:string;
    }
    type make_t = ?id:string -> ?scheduled_job_id:string -> ?async_job_id:string -> ?status:string -> ?started_at:string -> ?completed_at:string -> ?duration_ms:int -> ?logs:string -> ?result_summary:string -> ?error_message:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    let make ?(id = {||}) ?(scheduled_job_id = {||}) ?async_job_id ?(status = {||}) ?(started_at = {||}) ?completed_at ?duration_ms ?logs ?result_summary ?error_message ?(created_at = {||}) ?(updated_at = {||}) () = { id; scheduled_job_id; async_job_id; status; started_at; completed_at; duration_ms; logs; result_summary; error_message; created_at; updated_at }
    let merge =
    let merge_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ) in
    let merge_scheduled_job_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "scheduled_job_id", "scheduledJobId"), string, ({||})) ) in
    let merge_async_job_id = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((3, "async_job_id", "asyncJobId"), string) ) in
    let merge_status = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "status", "status"), string, ({||})) ) in
    let merge_started_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((5, "started_at", "startedAt"), string, ({||})) ) in
    let merge_completed_at = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((6, "completed_at", "completedAt"), string) ) in
    let merge_duration_ms = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((7, "duration_ms", "durationMs"), int32_int) ) in
    let merge_logs = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((8, "logs", "logs"), string) ) in
    let merge_result_summary = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((9, "result_summary", "resultSummary"), string) ) in
    let merge_error_message = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((10, "error_message", "errorMessage"), string) ) in
    let merge_created_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((11, "created_at", "createdAt"), string, ({||})) ) in
    let merge_updated_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((12, "updated_at", "updatedAt"), string, ({||})) ) in
    fun t1 t2 -> {
    	id = (merge_id t1.id t2.id);
    	scheduled_job_id = (merge_scheduled_job_id t1.scheduled_job_id t2.scheduled_job_id);
    	async_job_id = (merge_async_job_id t1.async_job_id t2.async_job_id);
    	status = (merge_status t1.status t2.status);
    	started_at = (merge_started_at t1.started_at t2.started_at);
    	completed_at = (merge_completed_at t1.completed_at t2.completed_at);
    	duration_ms = (merge_duration_ms t1.duration_ms t2.duration_ms);
    	logs = (merge_logs t1.logs t2.logs);
    	result_summary = (merge_result_summary t1.result_summary t2.result_summary);
    	error_message = (merge_error_message t1.error_message t2.error_message);
    	created_at = (merge_created_at t1.created_at t2.created_at);
    	updated_at = (merge_updated_at t1.updated_at t2.updated_at);
     }
    let spec () = Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ^:: basic ((2, "scheduled_job_id", "scheduledJobId"), string, ({||})) ^:: basic_opt ((3, "async_job_id", "asyncJobId"), string) ^:: basic ((4, "status", "status"), string, ({||})) ^:: basic ((5, "started_at", "startedAt"), string, ({||})) ^:: basic_opt ((6, "completed_at", "completedAt"), string) ^:: basic_opt ((7, "duration_ms", "durationMs"), int32_int) ^:: basic_opt ((8, "logs", "logs"), string) ^:: basic_opt ((9, "result_summary", "resultSummary"), string) ^:: basic_opt ((10, "error_message", "errorMessage"), string) ^:: basic ((11, "created_at", "createdAt"), string, ({||})) ^:: basic ((12, "updated_at", "updatedAt"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { id; scheduled_job_id; async_job_id; status; started_at; completed_at; duration_ms; logs; result_summary; error_message; created_at; updated_at } -> serialize writer id scheduled_job_id async_job_id status started_at completed_at duration_ms logs result_summary error_message created_at updated_at

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor id scheduled_job_id async_job_id status started_at completed_at duration_ms logs result_summary error_message created_at updated_at = { id; scheduled_job_id; async_job_id; status; started_at; completed_at; duration_ms; logs; result_summary; error_message; created_at; updated_at } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { id; scheduled_job_id; async_job_id; status; started_at; completed_at; duration_ms; logs; result_summary; error_message; created_at; updated_at } -> serialize id scheduled_job_id async_job_id status started_at completed_at duration_ms logs result_summary error_message created_at updated_at
    let from_json_exn =
      let constructor id scheduled_job_id async_job_id status started_at completed_at duration_ms logs result_summary error_message created_at updated_at = { id; scheduled_job_id; async_job_id; status; started_at; completed_at; duration_ms; logs; result_summary; error_message; created_at; updated_at } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and TaskInfo : sig
    type t = {
      task_id:string;
      log_count:int option;
    }
    val make: ?task_id:string -> ?log_count:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?task_id:string -> ?log_count:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = TaskInfo
    let name () = ".protocol.TaskInfo"
    type t = {
      task_id:string;
      log_count:int option;
    }
    type make_t = ?task_id:string -> ?log_count:int -> unit -> t
    let make ?(task_id = {||}) ?log_count () = { task_id; log_count }
    let merge =
    let merge_task_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "task_id", "taskId"), string, ({||})) ) in
    let merge_log_count = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((2, "log_count", "logCount"), int32_int) ) in
    fun t1 t2 -> {
    	task_id = (merge_task_id t1.task_id t2.task_id);
    	log_count = (merge_log_count t1.log_count t2.log_count);
     }
    let spec () = Runtime'.Spec.( basic ((1, "task_id", "taskId"), string, ({||})) ^:: basic_opt ((2, "log_count", "logCount"), int32_int) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { task_id; log_count } -> serialize writer task_id log_count

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor task_id log_count = { task_id; log_count } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { task_id; log_count } -> serialize task_id log_count
    let from_json_exn =
      let constructor task_id log_count = { task_id; log_count } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and StageInfo : sig
    type t = {
      stage:string;
      tasks:TaskInfo.t list;
    }
    val make: ?stage:string -> ?tasks:TaskInfo.t list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?stage:string -> ?tasks:TaskInfo.t list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = StageInfo
    let name () = ".protocol.StageInfo"
    type t = {
      stage:string;
      tasks:TaskInfo.t list;
    }
    type make_t = ?stage:string -> ?tasks:TaskInfo.t list -> unit -> t
    let make ?(stage = {||}) ?(tasks = []) () = { stage; tasks }
    let merge =
    let merge_stage = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "stage", "stage"), string, ({||})) ) in
    let merge_tasks = Runtime'.Merge.merge Runtime'.Spec.( repeated ((2, "tasks", "tasks"), (message (module TaskInfo)), not_packed) ) in
    fun t1 t2 -> {
    	stage = (merge_stage t1.stage t2.stage);
    	tasks = (merge_tasks t1.tasks t2.tasks);
     }
    let spec () = Runtime'.Spec.( basic ((1, "stage", "stage"), string, ({||})) ^:: repeated ((2, "tasks", "tasks"), (message (module TaskInfo)), not_packed) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { stage; tasks } -> serialize writer stage tasks

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor stage tasks = { stage; tasks } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { stage; tasks } -> serialize stage tasks
    let from_json_exn =
      let constructor stage tasks = { stage; tasks } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ListExecutionsResponse : sig
    type t = {
      executions:Execution.t list;
      count:int;
      total:int;
      has_more:bool;
    }
    val make: ?executions:Execution.t list -> ?count:int -> ?total:int -> ?has_more:bool -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?executions:Execution.t list -> ?count:int -> ?total:int -> ?has_more:bool -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ListExecutionsResponse
    let name () = ".protocol.ListExecutionsResponse"
    type t = {
      executions:Execution.t list;
      count:int;
      total:int;
      has_more:bool;
    }
    type make_t = ?executions:Execution.t list -> ?count:int -> ?total:int -> ?has_more:bool -> unit -> t
    let make ?(executions = []) ?(count = 0) ?(total = 0) ?(has_more = false) () = { executions; count; total; has_more }
    let merge =
    let merge_executions = Runtime'.Merge.merge Runtime'.Spec.( repeated ((1, "executions", "executions"), (message (module Execution)), not_packed) ) in
    let merge_count = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "count", "count"), int32_int, (0)) ) in
    let merge_total = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "total", "total"), int32_int, (0)) ) in
    let merge_has_more = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "has_more", "hasMore"), bool, (false)) ) in
    fun t1 t2 -> {
    	executions = (merge_executions t1.executions t2.executions);
    	count = (merge_count t1.count t2.count);
    	total = (merge_total t1.total t2.total);
    	has_more = (merge_has_more t1.has_more t2.has_more);
     }
    let spec () = Runtime'.Spec.( repeated ((1, "executions", "executions"), (message (module Execution)), not_packed) ^:: basic ((2, "count", "count"), int32_int, (0)) ^:: basic ((3, "total", "total"), int32_int, (0)) ^:: basic ((4, "has_more", "hasMore"), bool, (false)) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { executions; count; total; has_more } -> serialize writer executions count total has_more

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor executions count total has_more = { executions; count; total; has_more } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { executions; count; total; has_more } -> serialize executions count total has_more
    let from_json_exn =
      let constructor executions count total has_more = { executions; count; total; has_more } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and JobStagesResponse : sig
    type t = {
      job_id:string;
      stages:StageInfo.t list;
      plugin_version:string option;
    }
    val make: ?job_id:string -> ?stages:StageInfo.t list -> ?plugin_version:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?job_id:string -> ?stages:StageInfo.t list -> ?plugin_version:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = JobStagesResponse
    let name () = ".protocol.JobStagesResponse"
    type t = {
      job_id:string;
      stages:StageInfo.t list;
      plugin_version:string option;
    }
    type make_t = ?job_id:string -> ?stages:StageInfo.t list -> ?plugin_version:string -> unit -> t
    let make ?(job_id = {||}) ?(stages = []) ?plugin_version () = { job_id; stages; plugin_version }
    let merge =
    let merge_job_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "job_id", "jobId"), string, ({||})) ) in
    let merge_stages = Runtime'.Merge.merge Runtime'.Spec.( repeated ((2, "stages", "stages"), (message (module StageInfo)), not_packed) ) in
    let merge_plugin_version = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((3, "plugin_version", "pluginVersion"), string) ) in
    fun t1 t2 -> {
    	job_id = (merge_job_id t1.job_id t2.job_id);
    	stages = (merge_stages t1.stages t2.stages);
    	plugin_version = (merge_plugin_version t1.plugin_version t2.plugin_version);
     }
    let spec () = Runtime'.Spec.( basic ((1, "job_id", "jobId"), string, ({||})) ^:: repeated ((2, "stages", "stages"), (message (module StageInfo)), not_packed) ^:: basic_opt ((3, "plugin_version", "pluginVersion"), string) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { job_id; stages; plugin_version } -> serialize writer job_id stages plugin_version

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor job_id stages plugin_version = { job_id; stages; plugin_version } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { job_id; stages; plugin_version } -> serialize job_id stages plugin_version
    let from_json_exn =
      let constructor job_id stages plugin_version = { job_id; stages; plugin_version } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and TaskLogsResponse : sig
    type t = {
      task_id:string;
      logs:LogEntry.t list;
    }
    val make: ?task_id:string -> ?logs:LogEntry.t list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?task_id:string -> ?logs:LogEntry.t list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = TaskLogsResponse
    let name () = ".protocol.TaskLogsResponse"
    type t = {
      task_id:string;
      logs:LogEntry.t list;
    }
    type make_t = ?task_id:string -> ?logs:LogEntry.t list -> unit -> t
    let make ?(task_id = {||}) ?(logs = []) () = { task_id; logs }
    let merge =
    let merge_task_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "task_id", "taskId"), string, ({||})) ) in
    let merge_logs = Runtime'.Merge.merge Runtime'.Spec.( repeated ((2, "logs", "logs"), (message (module LogEntry)), not_packed) ) in
    fun t1 t2 -> {
    	task_id = (merge_task_id t1.task_id t2.task_id);
    	logs = (merge_logs t1.logs t2.logs);
     }
    let spec () = Runtime'.Spec.( basic ((1, "task_id", "taskId"), string, ({||})) ^:: repeated ((2, "logs", "logs"), (message (module LogEntry)), not_packed) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { task_id; logs } -> serialize writer task_id logs

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor task_id logs = { task_id; logs } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { task_id; logs } -> serialize task_id logs
    let from_json_exn =
      let constructor task_id logs = { task_id; logs } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and LogEntry : sig
    type t = {
      timestamp:string;
      level:string;
      message:string;
      metadata:Imported'modules.Struct.Google.Protobuf.Struct.t option;
    }
    val make: ?timestamp:string -> ?level:string -> ?message:string -> ?metadata:Imported'modules.Struct.Google.Protobuf.Struct.t -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?timestamp:string -> ?level:string -> ?message:string -> ?metadata:Imported'modules.Struct.Google.Protobuf.Struct.t -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = LogEntry
    let name () = ".protocol.LogEntry"
    type t = {
      timestamp:string;
      level:string;
      message:string;
      metadata:Imported'modules.Struct.Google.Protobuf.Struct.t option;
    }
    type make_t = ?timestamp:string -> ?level:string -> ?message:string -> ?metadata:Imported'modules.Struct.Google.Protobuf.Struct.t -> unit -> t
    let make ?(timestamp = {||}) ?(level = {||}) ?(message = {||}) ?metadata () = { timestamp; level; message; metadata }
    let merge =
    let merge_timestamp = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "timestamp", "timestamp"), string, ({||})) ) in
    let merge_level = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "level", "level"), string, ({||})) ) in
    let merge_message = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "message", "message"), string, ({||})) ) in
    let merge_metadata = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((4, "metadata", "metadata"), (message (module Imported'modules.Struct.Google.Protobuf.Struct))) ) in
    fun t1 t2 -> {
    	timestamp = (merge_timestamp t1.timestamp t2.timestamp);
    	level = (merge_level t1.level t2.level);
    	message = (merge_message t1.message t2.message);
    	metadata = (merge_metadata t1.metadata t2.metadata);
     }
    let spec () = Runtime'.Spec.( basic ((1, "timestamp", "timestamp"), string, ({||})) ^:: basic ((2, "level", "level"), string, ({||})) ^:: basic ((3, "message", "message"), string, ({||})) ^:: basic_opt ((4, "metadata", "metadata"), (message (module Imported'modules.Struct.Google.Protobuf.Struct))) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { timestamp; level; message; metadata } -> serialize writer timestamp level message metadata

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor timestamp level message metadata = { timestamp; level; message; metadata } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { timestamp; level; message; metadata } -> serialize timestamp level message metadata
    let from_json_exn =
      let constructor timestamp level message metadata = { timestamp; level; message; metadata } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ForceTriggerParams : sig
    type t = {
      handler_name:string;
      payload:bytes;
      source_url:string;
      async_job_id:string;
    }
    val make: ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?async_job_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?async_job_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ForceTriggerParams
    let name () = ".protocol.ForceTriggerParams"
    type t = {
      handler_name:string;
      payload:bytes;
      source_url:string;
      async_job_id:string;
    }
    type make_t = ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?async_job_id:string -> unit -> t
    let make ?(handler_name = {||}) ?(payload = (Bytes.of_string {||})) ?(source_url = {||}) ?(async_job_id = {||}) () = { handler_name; payload; source_url; async_job_id }
    let merge =
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_payload = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ) in
    let merge_source_url = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "source_url", "sourceUrl"), string, ({||})) ) in
    let merge_async_job_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((5, "async_job_id", "asyncJobId"), string, ({||})) ) in
    fun t1 t2 -> {
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	payload = (merge_payload t1.payload t2.payload);
    	source_url = (merge_source_url t1.source_url t2.source_url);
    	async_job_id = (merge_async_job_id t1.async_job_id t2.async_job_id);
     }
    let spec () = Runtime'.Spec.( basic ((2, "handler_name", "handlerName"), string, ({||})) ^:: basic ((3, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ^:: basic ((4, "source_url", "sourceUrl"), string, ({||})) ^:: basic ((5, "async_job_id", "asyncJobId"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { handler_name; payload; source_url; async_job_id } -> serialize writer handler_name payload source_url async_job_id

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor handler_name payload source_url async_job_id = { handler_name; payload; source_url; async_job_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { handler_name; payload; source_url; async_job_id } -> serialize handler_name payload source_url async_job_id
    let from_json_exn =
      let constructor handler_name payload source_url async_job_id = { handler_name; payload; source_url; async_job_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ForceTriggerResult : sig
    type t = {
      scheduled_job_id:string;
      execution_id:string;
      created_new_job:bool;
    }
    val make: ?scheduled_job_id:string -> ?execution_id:string -> ?created_new_job:bool -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?scheduled_job_id:string -> ?execution_id:string -> ?created_new_job:bool -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ForceTriggerResult
    let name () = ".protocol.ForceTriggerResult"
    type t = {
      scheduled_job_id:string;
      execution_id:string;
      created_new_job:bool;
    }
    type make_t = ?scheduled_job_id:string -> ?execution_id:string -> ?created_new_job:bool -> unit -> t
    let make ?(scheduled_job_id = {||}) ?(execution_id = {||}) ?(created_new_job = false) () = { scheduled_job_id; execution_id; created_new_job }
    let merge =
    let merge_scheduled_job_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "scheduled_job_id", "scheduledJobId"), string, ({||})) ) in
    let merge_execution_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "execution_id", "executionId"), string, ({||})) ) in
    let merge_created_new_job = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "created_new_job", "createdNewJob"), bool, (false)) ) in
    fun t1 t2 -> {
    	scheduled_job_id = (merge_scheduled_job_id t1.scheduled_job_id t2.scheduled_job_id);
    	execution_id = (merge_execution_id t1.execution_id t2.execution_id);
    	created_new_job = (merge_created_new_job t1.created_new_job t2.created_new_job);
     }
    let spec () = Runtime'.Spec.( basic ((1, "scheduled_job_id", "scheduledJobId"), string, ({||})) ^:: basic ((2, "execution_id", "executionId"), string, ({||})) ^:: basic ((3, "created_new_job", "createdNewJob"), bool, (false)) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { scheduled_job_id; execution_id; created_new_job } -> serialize writer scheduled_job_id execution_id created_new_job

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor scheduled_job_id execution_id created_new_job = { scheduled_job_id; execution_id; created_new_job } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { scheduled_job_id; execution_id; created_new_job } -> serialize scheduled_job_id execution_id created_new_job
    let from_json_exn =
      let constructor scheduled_job_id execution_id created_new_job = { scheduled_job_id; execution_id; created_new_job } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ScheduleProgress : sig
    type t = {
      run_count:int;
      last_run_at_ms:int;
      (**
{%html:
<p>0 if never run</p>
%}
      *)

      last_execution_id:string;
      next_run_at_ms:int;
      (**
{%html:
<p>0 if the declaration's first run stands</p>
%}
      *)

    }
    val make: ?run_count:int -> ?last_run_at_ms:int -> ?last_execution_id:string -> ?next_run_at_ms:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?run_count:int -> ?last_run_at_ms:int -> ?last_execution_id:string -> ?next_run_at_ms:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ScheduleProgress
    let name () = ".protocol.ScheduleProgress"
    type t = {
      run_count:int;
      last_run_at_ms:int;
      last_execution_id:string;
      next_run_at_ms:int;
    }
    type make_t = ?run_count:int -> ?last_run_at_ms:int -> ?last_execution_id:string -> ?next_run_at_ms:int -> unit -> t
    let make ?(run_count = 0) ?(last_run_at_ms = 0) ?(last_execution_id = {||}) ?(next_run_at_ms = 0) () = { run_count; last_run_at_ms; last_execution_id; next_run_at_ms }
    let merge =
    let merge_run_count = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "run_count", "runCount"), int64_int, (0)) ) in
    let merge_last_run_at_ms = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "last_run_at_ms", "lastRunAtMs"), int64_int, (0)) ) in
    let merge_last_execution_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "last_execution_id", "lastExecutionId"), string, ({||})) ) in
    let merge_next_run_at_ms = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "next_run_at_ms", "nextRunAtMs"), int64_int, (0)) ) in
    fun t1 t2 -> {
    	run_count = (merge_run_count t1.run_count t2.run_count);
    	last_run_at_ms = (merge_last_run_at_ms t1.last_run_at_ms t2.last_run_at_ms);
    	last_execution_id = (merge_last_execution_id t1.last_execution_id t2.last_execution_id);
    	next_run_at_ms = (merge_next_run_at_ms t1.next_run_at_ms t2.next_run_at_ms);
     }
    let spec () = Runtime'.Spec.( basic ((1, "run_count", "runCount"), int64_int, (0)) ^:: basic ((2, "last_run_at_ms", "lastRunAtMs"), int64_int, (0)) ^:: basic ((3, "last_execution_id", "lastExecutionId"), string, ({||})) ^:: basic ((4, "next_run_at_ms", "nextRunAtMs"), int64_int, (0)) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { run_count; last_run_at_ms; last_execution_id; next_run_at_ms } -> serialize writer run_count last_run_at_ms last_execution_id next_run_at_ms

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor run_count last_run_at_ms last_execution_id next_run_at_ms = { run_count; last_run_at_ms; last_execution_id; next_run_at_ms } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { run_count; last_run_at_ms; last_execution_id; next_run_at_ms } -> serialize run_count last_run_at_ms last_execution_id next_run_at_ms
    let from_json_exn =
      let constructor run_count last_run_at_ms last_execution_id next_run_at_ms = { run_count; last_run_at_ms; last_execution_id; next_run_at_ms } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ScheduledJob : sig
    type t = {
      id:string;
      handler_name:string;
      payload:bytes;
      source_url:string;
      interval_seconds:int;
      next_run_at:string;
      (**
{%html:
<p>RFC3339 timestamp</p>
%}
      *)

      last_run_at:string;
      (**
{%html:
<p>RFC3339 timestamp (empty if never run)</p>
%}
      *)

      last_execution_id:string;
      state:string;
      (**
{%html:
<p>one of ScheduleState</p>
%}
      *)

      metadata:string;
      created_at:string;
      (**
{%html:
<p>RFC3339 timestamp</p>
%}
      *)

      updated_at:string;
      (**
{%html:
<p>RFC3339 timestamp</p>
%}
      *)

      created_from_doc:string;
    }
    val make: ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> ?created_from_doc:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> ?created_from_doc:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ScheduledJob
    let name () = ".protocol.ScheduledJob"
    type t = {
      id:string;
      handler_name:string;
      payload:bytes;
      source_url:string;
      interval_seconds:int;
      next_run_at:string;
      last_run_at:string;
      last_execution_id:string;
      state:string;
      metadata:string;
      created_at:string;
      updated_at:string;
      created_from_doc:string;
    }
    type make_t = ?id:string -> ?handler_name:string -> ?payload:bytes -> ?source_url:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> ?created_from_doc:string -> unit -> t
    let make ?(id = {||}) ?(handler_name = {||}) ?(payload = (Bytes.of_string {||})) ?(source_url = {||}) ?(interval_seconds = 0) ?(next_run_at = {||}) ?(last_run_at = {||}) ?(last_execution_id = {||}) ?(state = {||}) ?(metadata = {||}) ?(created_at = {||}) ?(updated_at = {||}) ?(created_from_doc = {||}) () = { id; handler_name; payload; source_url; interval_seconds; next_run_at; last_run_at; last_execution_id; state; metadata; created_at; updated_at; created_from_doc }
    let merge =
    let merge_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ) in
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_payload = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ) in
    let merge_source_url = Runtime'.Merge.merge Runtime'.Spec.( basic ((5, "source_url", "sourceUrl"), string, ({||})) ) in
    let merge_interval_seconds = Runtime'.Merge.merge Runtime'.Spec.( basic ((6, "interval_seconds", "intervalSeconds"), int32_int, (0)) ) in
    let merge_next_run_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((7, "next_run_at", "nextRunAt"), string, ({||})) ) in
    let merge_last_run_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((8, "last_run_at", "lastRunAt"), string, ({||})) ) in
    let merge_last_execution_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((9, "last_execution_id", "lastExecutionId"), string, ({||})) ) in
    let merge_state = Runtime'.Merge.merge Runtime'.Spec.( basic ((10, "state", "state"), string, ({||})) ) in
    let merge_metadata = Runtime'.Merge.merge Runtime'.Spec.( basic ((11, "metadata", "metadata"), string, ({||})) ) in
    let merge_created_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((12, "created_at", "createdAt"), string, ({||})) ) in
    let merge_updated_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((13, "updated_at", "updatedAt"), string, ({||})) ) in
    let merge_created_from_doc = Runtime'.Merge.merge Runtime'.Spec.( basic ((14, "created_from_doc", "createdFromDoc"), string, ({||})) ) in
    fun t1 t2 -> {
    	id = (merge_id t1.id t2.id);
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	payload = (merge_payload t1.payload t2.payload);
    	source_url = (merge_source_url t1.source_url t2.source_url);
    	interval_seconds = (merge_interval_seconds t1.interval_seconds t2.interval_seconds);
    	next_run_at = (merge_next_run_at t1.next_run_at t2.next_run_at);
    	last_run_at = (merge_last_run_at t1.last_run_at t2.last_run_at);
    	last_execution_id = (merge_last_execution_id t1.last_execution_id t2.last_execution_id);
    	state = (merge_state t1.state t2.state);
    	metadata = (merge_metadata t1.metadata t2.metadata);
    	created_at = (merge_created_at t1.created_at t2.created_at);
    	updated_at = (merge_updated_at t1.updated_at t2.updated_at);
    	created_from_doc = (merge_created_from_doc t1.created_from_doc t2.created_from_doc);
     }
    let spec () = Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ^:: basic ((3, "handler_name", "handlerName"), string, ({||})) ^:: basic ((4, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ^:: basic ((5, "source_url", "sourceUrl"), string, ({||})) ^:: basic ((6, "interval_seconds", "intervalSeconds"), int32_int, (0)) ^:: basic ((7, "next_run_at", "nextRunAt"), string, ({||})) ^:: basic ((8, "last_run_at", "lastRunAt"), string, ({||})) ^:: basic ((9, "last_execution_id", "lastExecutionId"), string, ({||})) ^:: basic ((10, "state", "state"), string, ({||})) ^:: basic ((11, "metadata", "metadata"), string, ({||})) ^:: basic ((12, "created_at", "createdAt"), string, ({||})) ^:: basic ((13, "updated_at", "updatedAt"), string, ({||})) ^:: basic ((14, "created_from_doc", "createdFromDoc"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { id; handler_name; payload; source_url; interval_seconds; next_run_at; last_run_at; last_execution_id; state; metadata; created_at; updated_at; created_from_doc } -> serialize writer id handler_name payload source_url interval_seconds next_run_at last_run_at last_execution_id state metadata created_at updated_at created_from_doc

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor id handler_name payload source_url interval_seconds next_run_at last_run_at last_execution_id state metadata created_at updated_at created_from_doc = { id; handler_name; payload; source_url; interval_seconds; next_run_at; last_run_at; last_execution_id; state; metadata; created_at; updated_at; created_from_doc } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { id; handler_name; payload; source_url; interval_seconds; next_run_at; last_run_at; last_execution_id; state; metadata; created_at; updated_at; created_from_doc } -> serialize id handler_name payload source_url interval_seconds next_run_at last_run_at last_execution_id state metadata created_at updated_at created_from_doc
    let from_json_exn =
      let constructor id handler_name payload source_url interval_seconds next_run_at last_run_at last_execution_id state metadata created_at updated_at created_from_doc = { id; handler_name; payload; source_url; interval_seconds; next_run_at; last_run_at; last_execution_id; state; metadata; created_at; updated_at; created_from_doc } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and CreateScheduleRequest : sig
    type t = {
      auth_token:string;
      handler_name:string;
      (**
{%html:
<p>Handler to invoke on each tick</p>
%}
      *)

      interval_seconds:int;
      (**
{%html:
<p>Execution interval</p>
%}
      *)

      payload:bytes;
      (**
{%html:
<p>Handler-specific data (JSON)</p>
%}
      *)

      metadata:(string * string) list;
      (**
{%html:
<p>Optional metadata (glyph_id, plugin, etc.)</p>
%}
      *)

    }
    val make: ?auth_token:string -> ?handler_name:string -> ?interval_seconds:int -> ?payload:bytes -> ?metadata:(string * string) list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?handler_name:string -> ?interval_seconds:int -> ?payload:bytes -> ?metadata:(string * string) list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = CreateScheduleRequest
    let name () = ".protocol.CreateScheduleRequest"
    type t = {
      auth_token:string;
      handler_name:string;
      interval_seconds:int;
      payload:bytes;
      metadata:(string * string) list;
    }
    type make_t = ?auth_token:string -> ?handler_name:string -> ?interval_seconds:int -> ?payload:bytes -> ?metadata:(string * string) list -> unit -> t
    let make ?(auth_token = {||}) ?(handler_name = {||}) ?(interval_seconds = 0) ?(payload = (Bytes.of_string {||})) ?(metadata = []) () = { auth_token; handler_name; interval_seconds; payload; metadata }
    let merge =
    let merge_auth_token = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ) in
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_interval_seconds = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "interval_seconds", "intervalSeconds"), int32_int, (0)) ) in
    let merge_payload = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ) in
    let merge_metadata = Runtime'.Merge.merge Runtime'.Spec.( map ((5, "metadata", "metadata"), (string, basic ((2, "value", "value"), string, ({||})))) ) in
    fun t1 t2 -> {
    	auth_token = (merge_auth_token t1.auth_token t2.auth_token);
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	interval_seconds = (merge_interval_seconds t1.interval_seconds t2.interval_seconds);
    	payload = (merge_payload t1.payload t2.payload);
    	metadata = (merge_metadata t1.metadata t2.metadata);
     }
    let spec () = Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ^:: basic ((2, "handler_name", "handlerName"), string, ({||})) ^:: basic ((3, "interval_seconds", "intervalSeconds"), int32_int, (0)) ^:: basic ((4, "payload", "payload"), bytes, ((Bytes.of_string {||}))) ^:: map ((5, "metadata", "metadata"), (string, basic ((2, "value", "value"), string, ({||})))) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { auth_token; handler_name; interval_seconds; payload; metadata } -> serialize writer auth_token handler_name interval_seconds payload metadata

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor auth_token handler_name interval_seconds payload metadata = { auth_token; handler_name; interval_seconds; payload; metadata } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { auth_token; handler_name; interval_seconds; payload; metadata } -> serialize auth_token handler_name interval_seconds payload metadata
    let from_json_exn =
      let constructor auth_token handler_name interval_seconds payload metadata = { auth_token; handler_name; interval_seconds; payload; metadata } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and CreateScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
      schedule_id:string;
      (**
{%html:
<p>The created schedule ID</p>
%}
      *)

    }
    val make: ?success:bool -> ?error:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = CreateScheduleResponse
    let name () = ".protocol.CreateScheduleResponse"
    type t = {
      success:bool;
      error:string;
      schedule_id:string;
    }
    type make_t = ?success:bool -> ?error:string -> ?schedule_id:string -> unit -> t
    let make ?(success = false) ?(error = {||}) ?(schedule_id = {||}) () = { success; error; schedule_id }
    let merge =
    let merge_success = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ) in
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "error", "error"), string, ({||})) ) in
    let merge_schedule_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "schedule_id", "scheduleId"), string, ({||})) ) in
    fun t1 t2 -> {
    	success = (merge_success t1.success t2.success);
    	error = (merge_error t1.error t2.error);
    	schedule_id = (merge_schedule_id t1.schedule_id t2.schedule_id);
     }
    let spec () = Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ^:: basic ((2, "error", "error"), string, ({||})) ^:: basic ((3, "schedule_id", "scheduleId"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { success; error; schedule_id } -> serialize writer success error schedule_id

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor success error schedule_id = { success; error; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { success; error; schedule_id } -> serialize success error schedule_id
    let from_json_exn =
      let constructor success error schedule_id = { success; error; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and PauseScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = PauseScheduleRequest
    let name () = ".protocol.PauseScheduleRequest"
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    let make ?(auth_token = {||}) ?(schedule_id = {||}) () = { auth_token; schedule_id }
    let merge =
    let merge_auth_token = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ) in
    let merge_schedule_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "schedule_id", "scheduleId"), string, ({||})) ) in
    fun t1 t2 -> {
    	auth_token = (merge_auth_token t1.auth_token t2.auth_token);
    	schedule_id = (merge_schedule_id t1.schedule_id t2.schedule_id);
     }
    let spec () = Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ^:: basic ((2, "schedule_id", "scheduleId"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { auth_token; schedule_id } -> serialize writer auth_token schedule_id

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { auth_token; schedule_id } -> serialize auth_token schedule_id
    let from_json_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and PauseScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
    }
    val make: ?success:bool -> ?error:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = PauseScheduleResponse
    let name () = ".protocol.PauseScheduleResponse"
    type t = {
      success:bool;
      error:string;
    }
    type make_t = ?success:bool -> ?error:string -> unit -> t
    let make ?(success = false) ?(error = {||}) () = { success; error }
    let merge =
    let merge_success = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ) in
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "error", "error"), string, ({||})) ) in
    fun t1 t2 -> {
    	success = (merge_success t1.success t2.success);
    	error = (merge_error t1.error t2.error);
     }
    let spec () = Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ^:: basic ((2, "error", "error"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { success; error } -> serialize writer success error

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor success error = { success; error } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { success; error } -> serialize success error
    let from_json_exn =
      let constructor success error = { success; error } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ResumeScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ResumeScheduleRequest
    let name () = ".protocol.ResumeScheduleRequest"
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    let make ?(auth_token = {||}) ?(schedule_id = {||}) () = { auth_token; schedule_id }
    let merge =
    let merge_auth_token = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ) in
    let merge_schedule_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "schedule_id", "scheduleId"), string, ({||})) ) in
    fun t1 t2 -> {
    	auth_token = (merge_auth_token t1.auth_token t2.auth_token);
    	schedule_id = (merge_schedule_id t1.schedule_id t2.schedule_id);
     }
    let spec () = Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ^:: basic ((2, "schedule_id", "scheduleId"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { auth_token; schedule_id } -> serialize writer auth_token schedule_id

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { auth_token; schedule_id } -> serialize auth_token schedule_id
    let from_json_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ResumeScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
    }
    val make: ?success:bool -> ?error:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ResumeScheduleResponse
    let name () = ".protocol.ResumeScheduleResponse"
    type t = {
      success:bool;
      error:string;
    }
    type make_t = ?success:bool -> ?error:string -> unit -> t
    let make ?(success = false) ?(error = {||}) () = { success; error }
    let merge =
    let merge_success = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ) in
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "error", "error"), string, ({||})) ) in
    fun t1 t2 -> {
    	success = (merge_success t1.success t2.success);
    	error = (merge_error t1.error t2.error);
     }
    let spec () = Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ^:: basic ((2, "error", "error"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { success; error } -> serialize writer success error

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor success error = { success; error } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { success; error } -> serialize success error
    let from_json_exn =
      let constructor success error = { success; error } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and DeleteScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = DeleteScheduleRequest
    let name () = ".protocol.DeleteScheduleRequest"
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    let make ?(auth_token = {||}) ?(schedule_id = {||}) () = { auth_token; schedule_id }
    let merge =
    let merge_auth_token = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ) in
    let merge_schedule_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "schedule_id", "scheduleId"), string, ({||})) ) in
    fun t1 t2 -> {
    	auth_token = (merge_auth_token t1.auth_token t2.auth_token);
    	schedule_id = (merge_schedule_id t1.schedule_id t2.schedule_id);
     }
    let spec () = Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ^:: basic ((2, "schedule_id", "scheduleId"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { auth_token; schedule_id } -> serialize writer auth_token schedule_id

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { auth_token; schedule_id } -> serialize auth_token schedule_id
    let from_json_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and DeleteScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
    }
    val make: ?success:bool -> ?error:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = DeleteScheduleResponse
    let name () = ".protocol.DeleteScheduleResponse"
    type t = {
      success:bool;
      error:string;
    }
    type make_t = ?success:bool -> ?error:string -> unit -> t
    let make ?(success = false) ?(error = {||}) () = { success; error }
    let merge =
    let merge_success = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ) in
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "error", "error"), string, ({||})) ) in
    fun t1 t2 -> {
    	success = (merge_success t1.success t2.success);
    	error = (merge_error t1.error t2.error);
     }
    let spec () = Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ^:: basic ((2, "error", "error"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { success; error } -> serialize writer success error

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor success error = { success; error } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { success; error } -> serialize success error
    let from_json_exn =
      let constructor success error = { success; error } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and GetScheduleRequest : sig
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    val make: ?auth_token:string -> ?schedule_id:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = GetScheduleRequest
    let name () = ".protocol.GetScheduleRequest"
    type t = {
      auth_token:string;
      schedule_id:string;
    }
    type make_t = ?auth_token:string -> ?schedule_id:string -> unit -> t
    let make ?(auth_token = {||}) ?(schedule_id = {||}) () = { auth_token; schedule_id }
    let merge =
    let merge_auth_token = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ) in
    let merge_schedule_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "schedule_id", "scheduleId"), string, ({||})) ) in
    fun t1 t2 -> {
    	auth_token = (merge_auth_token t1.auth_token t2.auth_token);
    	schedule_id = (merge_schedule_id t1.schedule_id t2.schedule_id);
     }
    let spec () = Runtime'.Spec.( basic ((1, "auth_token", "authToken"), string, ({||})) ^:: basic ((2, "schedule_id", "scheduleId"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { auth_token; schedule_id } -> serialize writer auth_token schedule_id

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { auth_token; schedule_id } -> serialize auth_token schedule_id
    let from_json_exn =
      let constructor auth_token schedule_id = { auth_token; schedule_id } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and GetScheduleResponse : sig
    type t = {
      success:bool;
      error:string;
      job:ScheduledJob.t option;
    }
    val make: ?success:bool -> ?error:string -> ?job:ScheduledJob.t -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?success:bool -> ?error:string -> ?job:ScheduledJob.t -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = GetScheduleResponse
    let name () = ".protocol.GetScheduleResponse"
    type t = {
      success:bool;
      error:string;
      job:ScheduledJob.t option;
    }
    type make_t = ?success:bool -> ?error:string -> ?job:ScheduledJob.t -> unit -> t
    let make ?(success = false) ?(error = {||}) ?job () = { success; error; job }
    let merge =
    let merge_success = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ) in
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "error", "error"), string, ({||})) ) in
    let merge_job = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((3, "job", "job"), (message (module ScheduledJob))) ) in
    fun t1 t2 -> {
    	success = (merge_success t1.success t2.success);
    	error = (merge_error t1.error t2.error);
    	job = (merge_job t1.job t2.job);
     }
    let spec () = Runtime'.Spec.( basic ((1, "success", "success"), bool, (false)) ^:: basic ((2, "error", "error"), string, ({||})) ^:: basic_opt ((3, "job", "job"), (message (module ScheduledJob))) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { success; error; job } -> serialize writer success error job

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor success error job = { success; error; job } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { success; error; job } -> serialize success error job
    let from_json_exn =
      let constructor success error job = { success; error; job } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ScheduledJobResponse : sig
    type t = {
      id:string;
      handler_name:string;
      interval_seconds:int option;
      next_run_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      last_run_at:string option;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      last_execution_id:string option;
      (**
{%html:
<p>Last async job it started</p>
%}
      *)

      state:string;
      (**
{%html:
<p>active, paused, stopping, inactive</p>
%}
      *)

      created_from_doc:string option;
      (**
{%html:
<p>ProseMirror document it came from</p>
%}
      *)

      metadata:string option;
      (**
{%html:
<p>JSON</p>
%}
      *)

      created_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      updated_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

    }
    val make: ?id:string -> ?handler_name:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ScheduledJobResponse
    let name () = ".protocol.ScheduledJobResponse"
    type t = {
      id:string;
      handler_name:string;
      interval_seconds:int option;
      next_run_at:string;
      last_run_at:string option;
      last_execution_id:string option;
      state:string;
      created_from_doc:string option;
      metadata:string option;
      created_at:string;
      updated_at:string;
    }
    type make_t = ?id:string -> ?handler_name:string -> ?interval_seconds:int -> ?next_run_at:string -> ?last_run_at:string -> ?last_execution_id:string -> ?state:string -> ?created_from_doc:string -> ?metadata:string -> ?created_at:string -> ?updated_at:string -> unit -> t
    let make ?(id = {||}) ?(handler_name = {||}) ?interval_seconds ?(next_run_at = {||}) ?last_run_at ?last_execution_id ?(state = {||}) ?created_from_doc ?metadata ?(created_at = {||}) ?(updated_at = {||}) () = { id; handler_name; interval_seconds; next_run_at; last_run_at; last_execution_id; state; created_from_doc; metadata; created_at; updated_at }
    let merge =
    let merge_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ) in
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_interval_seconds = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((3, "interval_seconds", "intervalSeconds"), int32_int) ) in
    let merge_next_run_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "next_run_at", "nextRunAt"), string, ({||})) ) in
    let merge_last_run_at = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((5, "last_run_at", "lastRunAt"), string) ) in
    let merge_last_execution_id = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((6, "last_execution_id", "lastExecutionId"), string) ) in
    let merge_state = Runtime'.Merge.merge Runtime'.Spec.( basic ((7, "state", "state"), string, ({||})) ) in
    let merge_created_from_doc = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((8, "created_from_doc", "createdFromDoc"), string) ) in
    let merge_metadata = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((9, "metadata", "metadata"), string) ) in
    let merge_created_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((10, "created_at", "createdAt"), string, ({||})) ) in
    let merge_updated_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((11, "updated_at", "updatedAt"), string, ({||})) ) in
    fun t1 t2 -> {
    	id = (merge_id t1.id t2.id);
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	interval_seconds = (merge_interval_seconds t1.interval_seconds t2.interval_seconds);
    	next_run_at = (merge_next_run_at t1.next_run_at t2.next_run_at);
    	last_run_at = (merge_last_run_at t1.last_run_at t2.last_run_at);
    	last_execution_id = (merge_last_execution_id t1.last_execution_id t2.last_execution_id);
    	state = (merge_state t1.state t2.state);
    	created_from_doc = (merge_created_from_doc t1.created_from_doc t2.created_from_doc);
    	metadata = (merge_metadata t1.metadata t2.metadata);
    	created_at = (merge_created_at t1.created_at t2.created_at);
    	updated_at = (merge_updated_at t1.updated_at t2.updated_at);
     }
    let spec () = Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ^:: basic ((2, "handler_name", "handlerName"), string, ({||})) ^:: basic_opt ((3, "interval_seconds", "intervalSeconds"), int32_int) ^:: basic ((4, "next_run_at", "nextRunAt"), string, ({||})) ^:: basic_opt ((5, "last_run_at", "lastRunAt"), string) ^:: basic_opt ((6, "last_execution_id", "lastExecutionId"), string) ^:: basic ((7, "state", "state"), string, ({||})) ^:: basic_opt ((8, "created_from_doc", "createdFromDoc"), string) ^:: basic_opt ((9, "metadata", "metadata"), string) ^:: basic ((10, "created_at", "createdAt"), string, ({||})) ^:: basic ((11, "updated_at", "updatedAt"), string, ({||})) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { id; handler_name; interval_seconds; next_run_at; last_run_at; last_execution_id; state; created_from_doc; metadata; created_at; updated_at } -> serialize writer id handler_name interval_seconds next_run_at last_run_at last_execution_id state created_from_doc metadata created_at updated_at

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor id handler_name interval_seconds next_run_at last_run_at last_execution_id state created_from_doc metadata created_at updated_at = { id; handler_name; interval_seconds; next_run_at; last_run_at; last_execution_id; state; created_from_doc; metadata; created_at; updated_at } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { id; handler_name; interval_seconds; next_run_at; last_run_at; last_execution_id; state; created_from_doc; metadata; created_at; updated_at } -> serialize id handler_name interval_seconds next_run_at last_run_at last_execution_id state created_from_doc metadata created_at updated_at
    let from_json_exn =
      let constructor id handler_name interval_seconds next_run_at last_run_at last_execution_id state created_from_doc metadata created_at updated_at = { id; handler_name; interval_seconds; next_run_at; last_run_at; last_execution_id; state; created_from_doc; metadata; created_at; updated_at } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and CreateScheduledJobRequest : sig
    type t = {
      handler_name:string;
      interval_seconds:int;
      created_from_doc:string option;
      metadata:string option;
      (**
{%html:
<p>JSON</p>
%}
      *)

      force:bool option;
      (**
{%html:
<p>Bypass deduplication</p>
%}
      *)

    }
    val make: ?handler_name:string -> ?interval_seconds:int -> ?created_from_doc:string -> ?metadata:string -> ?force:bool -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?handler_name:string -> ?interval_seconds:int -> ?created_from_doc:string -> ?metadata:string -> ?force:bool -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = CreateScheduledJobRequest
    let name () = ".protocol.CreateScheduledJobRequest"
    type t = {
      handler_name:string;
      interval_seconds:int;
      created_from_doc:string option;
      metadata:string option;
      force:bool option;
    }
    type make_t = ?handler_name:string -> ?interval_seconds:int -> ?created_from_doc:string -> ?metadata:string -> ?force:bool -> unit -> t
    let make ?(handler_name = {||}) ?(interval_seconds = 0) ?created_from_doc ?metadata ?force () = { handler_name; interval_seconds; created_from_doc; metadata; force }
    let merge =
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_interval_seconds = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "interval_seconds", "intervalSeconds"), int32_int, (0)) ) in
    let merge_created_from_doc = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((3, "created_from_doc", "createdFromDoc"), string) ) in
    let merge_metadata = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((4, "metadata", "metadata"), string) ) in
    let merge_force = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((5, "force", "force"), bool) ) in
    fun t1 t2 -> {
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	interval_seconds = (merge_interval_seconds t1.interval_seconds t2.interval_seconds);
    	created_from_doc = (merge_created_from_doc t1.created_from_doc t2.created_from_doc);
    	metadata = (merge_metadata t1.metadata t2.metadata);
    	force = (merge_force t1.force t2.force);
     }
    let spec () = Runtime'.Spec.( basic ((1, "handler_name", "handlerName"), string, ({||})) ^:: basic ((2, "interval_seconds", "intervalSeconds"), int32_int, (0)) ^:: basic_opt ((3, "created_from_doc", "createdFromDoc"), string) ^:: basic_opt ((4, "metadata", "metadata"), string) ^:: basic_opt ((5, "force", "force"), bool) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { handler_name; interval_seconds; created_from_doc; metadata; force } -> serialize writer handler_name interval_seconds created_from_doc metadata force

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor handler_name interval_seconds created_from_doc metadata force = { handler_name; interval_seconds; created_from_doc; metadata; force } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { handler_name; interval_seconds; created_from_doc; metadata; force } -> serialize handler_name interval_seconds created_from_doc metadata force
    let from_json_exn =
      let constructor handler_name interval_seconds created_from_doc metadata force = { handler_name; interval_seconds; created_from_doc; metadata; force } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and UpdateScheduledJobRequest : sig
    type t = {
      state:string option;
      (**
{%html:
<p>active, paused, stopping, inactive</p>
%}
      *)

      interval_seconds:int option;
    }
    val make: ?state:string -> ?interval_seconds:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?state:string -> ?interval_seconds:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = UpdateScheduledJobRequest
    let name () = ".protocol.UpdateScheduledJobRequest"
    type t = {
      state:string option;
      interval_seconds:int option;
    }
    type make_t = ?state:string -> ?interval_seconds:int -> unit -> t
    let make ?state ?interval_seconds () = { state; interval_seconds }
    let merge =
    let merge_state = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((1, "state", "state"), string) ) in
    let merge_interval_seconds = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((2, "interval_seconds", "intervalSeconds"), int32_int) ) in
    fun t1 t2 -> {
    	state = (merge_state t1.state t2.state);
    	interval_seconds = (merge_interval_seconds t1.interval_seconds t2.interval_seconds);
     }
    let spec () = Runtime'.Spec.( basic_opt ((1, "state", "state"), string) ^:: basic_opt ((2, "interval_seconds", "intervalSeconds"), int32_int) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { state; interval_seconds } -> serialize writer state interval_seconds

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor state interval_seconds = { state; interval_seconds } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { state; interval_seconds } -> serialize state interval_seconds
    let from_json_exn =
      let constructor state interval_seconds = { state; interval_seconds } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ListScheduledJobsResponse : sig
    type t = {
      jobs:ScheduledJobResponse.t list;
      count:int option;
    }
    val make: ?jobs:ScheduledJobResponse.t list -> ?count:int -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?jobs:ScheduledJobResponse.t list -> ?count:int -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ListScheduledJobsResponse
    let name () = ".protocol.ListScheduledJobsResponse"
    type t = {
      jobs:ScheduledJobResponse.t list;
      count:int option;
    }
    type make_t = ?jobs:ScheduledJobResponse.t list -> ?count:int -> unit -> t
    let make ?(jobs = []) ?count () = { jobs; count }
    let merge =
    let merge_jobs = Runtime'.Merge.merge Runtime'.Spec.( repeated ((1, "jobs", "jobs"), (message (module ScheduledJobResponse)), not_packed) ) in
    let merge_count = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((2, "count", "count"), int32_int) ) in
    fun t1 t2 -> {
    	jobs = (merge_jobs t1.jobs t2.jobs);
    	count = (merge_count t1.count t2.count);
     }
    let spec () = Runtime'.Spec.( repeated ((1, "jobs", "jobs"), (message (module ScheduledJobResponse)), not_packed) ^:: basic_opt ((2, "count", "count"), int32_int) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { jobs; count } -> serialize writer jobs count

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor jobs count = { jobs; count } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { jobs; count } -> serialize jobs count
    let from_json_exn =
      let constructor jobs count = { jobs; count } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ChildJobInfo : sig
    type t = {
      id:string;
      handler_name:string;
      source:string;
      status:string;
      progress_pct:float option;
      cost_estimate:float option;
      cost_actual:float option;
      error:string option;
      created_at:string;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      started_at:string option;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

      completed_at:string option;
      (**
{%html:
<p>RFC3339</p>
%}
      *)

    }
    val make: ?id:string -> ?handler_name:string -> ?source:string -> ?status:string -> ?progress_pct:float -> ?cost_estimate:float -> ?cost_actual:float -> ?error:string -> ?created_at:string -> ?started_at:string -> ?completed_at:string -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?id:string -> ?handler_name:string -> ?source:string -> ?status:string -> ?progress_pct:float -> ?cost_estimate:float -> ?cost_actual:float -> ?error:string -> ?created_at:string -> ?started_at:string -> ?completed_at:string -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ChildJobInfo
    let name () = ".protocol.ChildJobInfo"
    type t = {
      id:string;
      handler_name:string;
      source:string;
      status:string;
      progress_pct:float option;
      cost_estimate:float option;
      cost_actual:float option;
      error:string option;
      created_at:string;
      started_at:string option;
      completed_at:string option;
    }
    type make_t = ?id:string -> ?handler_name:string -> ?source:string -> ?status:string -> ?progress_pct:float -> ?cost_estimate:float -> ?cost_actual:float -> ?error:string -> ?created_at:string -> ?started_at:string -> ?completed_at:string -> unit -> t
    let make ?(id = {||}) ?(handler_name = {||}) ?(source = {||}) ?(status = {||}) ?progress_pct ?cost_estimate ?cost_actual ?error ?(created_at = {||}) ?started_at ?completed_at () = { id; handler_name; source; status; progress_pct; cost_estimate; cost_actual; error; created_at; started_at; completed_at }
    let merge =
    let merge_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ) in
    let merge_handler_name = Runtime'.Merge.merge Runtime'.Spec.( basic ((2, "handler_name", "handlerName"), string, ({||})) ) in
    let merge_source = Runtime'.Merge.merge Runtime'.Spec.( basic ((3, "source", "source"), string, ({||})) ) in
    let merge_status = Runtime'.Merge.merge Runtime'.Spec.( basic ((4, "status", "status"), string, ({||})) ) in
    let merge_progress_pct = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((5, "progress_pct", "progressPct"), double) ) in
    let merge_cost_estimate = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((6, "cost_estimate", "costEstimate"), double) ) in
    let merge_cost_actual = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((7, "cost_actual", "costActual"), double) ) in
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((8, "error", "error"), string) ) in
    let merge_created_at = Runtime'.Merge.merge Runtime'.Spec.( basic ((9, "created_at", "createdAt"), string, ({||})) ) in
    let merge_started_at = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((10, "started_at", "startedAt"), string) ) in
    let merge_completed_at = Runtime'.Merge.merge Runtime'.Spec.( basic_opt ((11, "completed_at", "completedAt"), string) ) in
    fun t1 t2 -> {
    	id = (merge_id t1.id t2.id);
    	handler_name = (merge_handler_name t1.handler_name t2.handler_name);
    	source = (merge_source t1.source t2.source);
    	status = (merge_status t1.status t2.status);
    	progress_pct = (merge_progress_pct t1.progress_pct t2.progress_pct);
    	cost_estimate = (merge_cost_estimate t1.cost_estimate t2.cost_estimate);
    	cost_actual = (merge_cost_actual t1.cost_actual t2.cost_actual);
    	error = (merge_error t1.error t2.error);
    	created_at = (merge_created_at t1.created_at t2.created_at);
    	started_at = (merge_started_at t1.started_at t2.started_at);
    	completed_at = (merge_completed_at t1.completed_at t2.completed_at);
     }
    let spec () = Runtime'.Spec.( basic ((1, "id", "id"), string, ({||})) ^:: basic ((2, "handler_name", "handlerName"), string, ({||})) ^:: basic ((3, "source", "source"), string, ({||})) ^:: basic ((4, "status", "status"), string, ({||})) ^:: basic_opt ((5, "progress_pct", "progressPct"), double) ^:: basic_opt ((6, "cost_estimate", "costEstimate"), double) ^:: basic_opt ((7, "cost_actual", "costActual"), double) ^:: basic_opt ((8, "error", "error"), string) ^:: basic ((9, "created_at", "createdAt"), string, ({||})) ^:: basic_opt ((10, "started_at", "startedAt"), string) ^:: basic_opt ((11, "completed_at", "completedAt"), string) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { id; handler_name; source; status; progress_pct; cost_estimate; cost_actual; error; created_at; started_at; completed_at } -> serialize writer id handler_name source status progress_pct cost_estimate cost_actual error created_at started_at completed_at

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor id handler_name source status progress_pct cost_estimate cost_actual error created_at started_at completed_at = { id; handler_name; source; status; progress_pct; cost_estimate; cost_actual; error; created_at; started_at; completed_at } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { id; handler_name; source; status; progress_pct; cost_estimate; cost_actual; error; created_at; started_at; completed_at } -> serialize id handler_name source status progress_pct cost_estimate cost_actual error created_at started_at completed_at
    let from_json_exn =
      let constructor id handler_name source status progress_pct cost_estimate cost_actual error created_at started_at completed_at = { id; handler_name; source; status; progress_pct; cost_estimate; cost_actual; error; created_at; started_at; completed_at } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and JobChildrenResponse : sig
    type t = {
      parent_job_id:string;
      children:ChildJobInfo.t list;
    }
    val make: ?parent_job_id:string -> ?children:ChildJobInfo.t list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?parent_job_id:string -> ?children:ChildJobInfo.t list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = JobChildrenResponse
    let name () = ".protocol.JobChildrenResponse"
    type t = {
      parent_job_id:string;
      children:ChildJobInfo.t list;
    }
    type make_t = ?parent_job_id:string -> ?children:ChildJobInfo.t list -> unit -> t
    let make ?(parent_job_id = {||}) ?(children = []) () = { parent_job_id; children }
    let merge =
    let merge_parent_job_id = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "parent_job_id", "parentJobId"), string, ({||})) ) in
    let merge_children = Runtime'.Merge.merge Runtime'.Spec.( repeated ((2, "children", "children"), (message (module ChildJobInfo)), not_packed) ) in
    fun t1 t2 -> {
    	parent_job_id = (merge_parent_job_id t1.parent_job_id t2.parent_job_id);
    	children = (merge_children t1.children t2.children);
     }
    let spec () = Runtime'.Spec.( basic ((1, "parent_job_id", "parentJobId"), string, ({||})) ^:: repeated ((2, "children", "children"), (message (module ChildJobInfo)), not_packed) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { parent_job_id; children } -> serialize writer parent_job_id children

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor parent_job_id children = { parent_job_id; children } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { parent_job_id; children } -> serialize parent_job_id children
    let from_json_exn =
      let constructor parent_job_id children = { parent_job_id; children } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  and ErrorResponse : sig
    type t = {
      error:string;
      details:string list;
      (**
{%html:
<p>From the error chain</p>
%}
      *)

    }
    val make: ?error:string -> ?details:string list -> unit -> t
    (** Helper function to generate a message using default values *)

    val to_proto: t -> Runtime'.Writer.t
    (** Serialize the message to binary format *)

    val from_proto: Runtime'.Reader.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from binary format *)

    val to_json: Runtime'.Json_options.t -> t -> Runtime'.Json.t
    (** Serialize to Json (compatible with Yojson.Basic.t) *)

    val from_json: Runtime'.Json.t -> (t, [> Runtime'.Result.error]) result
    (** Deserialize from Json (compatible with Yojson.Basic.t) *)

    val name: unit -> string
    (** Fully qualified protobuf name of this message *)

    (**/**)
    type make_t = ?error:string -> ?details:string list -> unit -> t
    val merge: t -> t -> t
    val to_proto': Runtime'.Writer.t -> t -> unit
    val from_proto_exn: Runtime'.Reader.t -> t
    val from_json_exn: Runtime'.Json.t -> t
    (**/**)
  end = struct
    module This'_ = ErrorResponse
    let name () = ".protocol.ErrorResponse"
    type t = {
      error:string;
      details:string list;
    }
    type make_t = ?error:string -> ?details:string list -> unit -> t
    let make ?(error = {||}) ?(details = []) () = { error; details }
    let merge =
    let merge_error = Runtime'.Merge.merge Runtime'.Spec.( basic ((1, "error", "error"), string, ({||})) ) in
    let merge_details = Runtime'.Merge.merge Runtime'.Spec.( repeated ((2, "details", "details"), string, not_packed) ) in
    fun t1 t2 -> {
    	error = (merge_error t1.error t2.error);
    	details = (merge_details t1.details t2.details);
     }
    let spec () = Runtime'.Spec.( basic ((1, "error", "error"), string, ({||})) ^:: repeated ((2, "details", "details"), string, not_packed) ^:: nil )
    let to_proto' =
      let serialize = Runtime'.apply_lazy (fun () -> Runtime'.Serialize.serialize (spec ())) in
      fun writer { error; details } -> serialize writer error details

    let to_proto t = let writer = Runtime'.Writer.init () in to_proto' writer t; writer
    let from_proto_exn =
      let constructor error details = { error; details } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize.deserialize (spec ()) constructor)
    let from_proto writer = Runtime'.Result.catch (fun () -> from_proto_exn writer)
    let to_json options =
      let serialize = Runtime'.Serialize_json.serialize ~message_name:(name ()) (spec ()) options in
      fun { error; details } -> serialize error details
    let from_json_exn =
      let constructor error details = { error; details } in
      Runtime'.apply_lazy (fun () -> Runtime'.Deserialize_json.deserialize ~message_name:(name ()) (spec ()) constructor)
    let from_json json = Runtime'.Result.catch (fun () -> from_json_exn json)
  end

  module ScheduleService = struct
    module CreateSchedule = struct
      let package_name = Some "protocol"
      let service_name = "ScheduleService"
      let method_name = "CreateSchedule"
      let name = "/protocol.ScheduleService/CreateSchedule"
      module Request = CreateScheduleRequest
      module Response = CreateScheduleResponse
    end

    let createSchedule =
      (module CreateScheduleRequest : Runtime'.Spec.Message with type t = CreateScheduleRequest.t ),
      (module CreateScheduleResponse : Runtime'.Spec.Message with type t = CreateScheduleResponse.t )

    module PauseSchedule = struct
      let package_name = Some "protocol"
      let service_name = "ScheduleService"
      let method_name = "PauseSchedule"
      let name = "/protocol.ScheduleService/PauseSchedule"
      module Request = PauseScheduleRequest
      module Response = PauseScheduleResponse
    end

    let pauseSchedule =
      (module PauseScheduleRequest : Runtime'.Spec.Message with type t = PauseScheduleRequest.t ),
      (module PauseScheduleResponse : Runtime'.Spec.Message with type t = PauseScheduleResponse.t )

    module ResumeSchedule = struct
      let package_name = Some "protocol"
      let service_name = "ScheduleService"
      let method_name = "ResumeSchedule"
      let name = "/protocol.ScheduleService/ResumeSchedule"
      module Request = ResumeScheduleRequest
      module Response = ResumeScheduleResponse
    end

    let resumeSchedule =
      (module ResumeScheduleRequest : Runtime'.Spec.Message with type t = ResumeScheduleRequest.t ),
      (module ResumeScheduleResponse : Runtime'.Spec.Message with type t = ResumeScheduleResponse.t )

    module DeleteSchedule = struct
      let package_name = Some "protocol"
      let service_name = "ScheduleService"
      let method_name = "DeleteSchedule"
      let name = "/protocol.ScheduleService/DeleteSchedule"
      module Request = DeleteScheduleRequest
      module Response = DeleteScheduleResponse
    end

    let deleteSchedule =
      (module DeleteScheduleRequest : Runtime'.Spec.Message with type t = DeleteScheduleRequest.t ),
      (module DeleteScheduleResponse : Runtime'.Spec.Message with type t = DeleteScheduleResponse.t )

    module GetSchedule = struct
      let package_name = Some "protocol"
      let service_name = "ScheduleService"
      let method_name = "GetSchedule"
      let name = "/protocol.ScheduleService/GetSchedule"
      module Request = GetScheduleRequest
      module Response = GetScheduleResponse
    end

    let getSchedule =
      (module GetScheduleRequest : Runtime'.Spec.Message with type t = GetScheduleRequest.t ),
      (module GetScheduleResponse : Runtime'.Spec.Message with type t = GetScheduleResponse.t )

  end

end

