...
Individual clusters can appear as their own [glyph](https://github.com/teranos/QNTX/blob/main/packages/glyphs/VISION.md), [melded](./glyph-melding.md) with the semantic search glyph. When you select a cluster in ⊨, the cluster glyph shows that cluster's shape and where the semantic matches land within it — so you can see how reliable a match actually is.
