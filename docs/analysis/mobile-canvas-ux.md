...
Tauri mobile (WKWebView on iOS, WebView on Android). A QNTX mobile app is a node in a decentralised mesh network — it can operate offline via WASM (query parsing, attestation storage in IndexedDB) and gains more capabilities by connecting to other QNTX nodes. The canvas is the primary workspace; the element tray is the main navigation surface.

## Element Tray: Touch Browse

The tray's desktop interaction (mouse proximity morphing) has been extended to touch.

### How it works

Touch near the tray enters browse mode. Thumb slides up/down through elements — the same proximity morphing pipeline (easing, baseline boost, text fade) drives the visual feedback. Lifting the thumb opens the element with highest proximity.

```
touchstart near tray → browse mode, preventDefault (block scroll)
touchmove            → feed coordinates into proximity system
touchend             → find peaked element → morphToWindow/Canvas → collapse rest
```
...
- Element DOM axiom fully preserved — no element creation, only coordinate feeding.

## Window Manifestation: Already Touch-Aware

`window/drag.ts` in [`teranos/elements`](https://github.com/teranos/elements) handles both mouse and touch for window dragging.

## Canvas Manifestation

### Canvas Pan — Single finger drag (mobile/touch)

`canvas/canvas-pan.ts` implements touch-based panning for mobile and responsive design mode. Single finger drag anywhere on the canvas (including on elements) pans the viewport. Desktop uses two-finger trackpad scroll and middle mouse button drag.
...
**Element drag, resize, spawn menu, meld** — all use `mousedown`/`mousemove`/`mouseup` exclusively. On mobile/touch devices, these interactions are not available. Elements can be viewed, tapped to select, and the canvas panned and zoomed.
...
| Touch-based element editing | Low | Element manipulation currently desktop-only |
