...
Element melding in QNTX creates spatial compositions through proximity. Phase 1 implemented N-element compositions using flat arrays (`glyphIds: string[]`) for horizontal chains: `[ax|py|prompt]`.
...
- `config ↓ element` (runtime configuration)
...
Migrate from flat `elementIds: string[]` to edge-based composition DAG structure.
...
message Composition {
  ...
  reserved 3;  // formerly element_ids (removed for DAG-native approach)
  double x = 4;
  double y = 5;
}
```
...
- Defined in `element/proto/canvas.proto`
...
- Traverse edges to find elements (e.g., `composition.edges.some(e => e.from === glyphId)`)
...
- MELDABILITY registry restructured: each element class maps to `PortRule[]` with direction + targets
...
- Result elements auto-meld below py on execution (bottom port)
...
- Drag-to-extend: standalone element melds into existing composition (append to leaf / prepend to root)
...
- 3+ element chains in browser, composition persistence across page reload
...
Edge-based composition uses a port-based model (à la GoFlow).
...
- Unmeld granularity: splitting a composition at a middle element
...
- `docs/development/multi-element-meld.md`: Implementation checklist
