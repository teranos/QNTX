...
## Decision
Refactor LLM access into a first-class service on `ServiceRegistry`. Plugins call `services.LLM()` instead of going through Pulse or HTTP. Provider plugins (OpenRouter, Ollama, others) register as LLM backends. Core routes calls to the appropriate provider.
