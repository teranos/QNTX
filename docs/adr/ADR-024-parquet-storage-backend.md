...
- **Point lookup by ID** (`storage_get(id)`) is a scan of the buffer and the files, and the existence check before every `put` is that lookup. After compaction it is a scan of one file, which is what makes a write cheap enough to pay at boot. A write through a landing file ([ADR-037](ADR-037-operational-db.md)) does not `put`: the landing file's primary key already refused a duplicate, and a send hands its rows to `write_file`, which looks nothing up. Before that change `qntx.boot.subsystem.took` measured the `store-proof` write at 4.4 s on average and 266 s at worst, over 137 boots.
...
- **Write durability window.** Since [ADR-037](ADR-037-operational-db.md) a write lands in the namespace's landing file and nowhere else, and the landing file sends what it holds to the record every six hours (`sendInterval` in `cmd/qntx/commands/database_parquet.go`). A crashed process loses nothing: the rows wait in the landing file and the next open sends them. Losing the host loses what was not yet sent, so the send interval is the RPO for host loss, and `qntx.store.unsent` says how much that is, minute by minute.
