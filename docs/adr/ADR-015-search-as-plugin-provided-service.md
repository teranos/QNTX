...
See `plugin/grpc/protocol/search.proto` ([SearchService gRPC API](https://github.com/teranos/QNTX/blob/main/plugin/grpc/protocol/search.proto)). Four RPCs: `Search`, `IndexDocuments`, `DeleteDocuments`, `ConfigureIndex`. Documents and filters flow as JSON bytes — the proto is engine-agnostic.
