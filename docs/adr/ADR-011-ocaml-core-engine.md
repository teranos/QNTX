...
1. **[ATSStoreService](https://github.com/teranos/QNTX/blob/main/plugin/grpc/protocol/atsstore.proto) expands massively** — kern calls back into Go for every DB operation. More gRPC surface, more latency, but the plugin boundary stays clean. This is the incremental path.
