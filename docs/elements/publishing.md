...
An element's UI is an attestation. You publish one, and every page in that
...
qntx element list --to https://api.q.abcd.nl
```

## What a published element is

Subject `element-<name>`, predicate `element:module`, the module's source under the
...
`/g/` is `of ANYONE`. An element module is UI, and UI is not a boundary — every
call it makes is gated against whoever made it. What is served is what was
published; an element written and not yet public is an attestation this route does
not read.
...
export const render = (item, ui) => {
  const { element, content } = ui.element({
    defaults: { x: item.x ?? 200, y: item.y ?? 200, width: 960, height: 640 },
...
`ui.element()` builds the frame: position, drag, resize, the title bar, and the ⬆
...
compiled and bundled by `bun build` into a single `dist/element-module.js`, which
...
2. `standing-element-published` matches on the predicate. It is a watcher every
...
5. What is already drawn is redrawn: a canvas element in place, keeping its
...
An element is published into the namespace the caller acts in, and served to that
namespace alone. Namespaces are their own universes and nothing crosses
(ADR-026) — an element published in one is not on another's canvas.

## When it does not appear

The element draws its own absence. Where it should be, you get a frame that says
...
to open a console to find out why an element is missing.
...
- `server/element_module_handlers.go` — what `/g/` serves
