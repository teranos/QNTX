# git Types

<!-- Code generated by typegen from Go source. DO NOT EDIT. -->
<!-- Regenerate with: make types -->

This document shows Go type definitions alongside their TypeScript equivalents.
Use this as a reference when working with QNTX types in different contexts.

## CargoLock

**Source**: [`qntx-code/ixgest/git/deps.go:404`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L404)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type CargoLock struct {
	Package []unknown `toml:"package"`
}
```

</td>
<td>

```typescript
export interface CargoLock {
  Package: unknown[];
}
```

</td>
</tr>
</table>

## CargoToml

**Source**: [`qntx-code/ixgest/git/deps.go:331`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L331)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type CargoToml struct {
	Package unknown `toml:"package"`
	Dependencies map[string]interface{} `toml:"dependencies"`
	DevDependencies map[string]interface{} `toml:"dev-dependencies"`
}
```

</td>
<td>

```typescript
export interface CargoToml {
  Package: unknown;
  Dependencies: Record<string, unknown>;
  DevDependencies: Record<string, unknown>;
}
```

</td>
</tr>
</table>

## DepsIngestionResult

**Source**: [`qntx-code/ixgest/git/deps.go:33`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L33)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type DepsIngestionResult struct {
	FilesDetected int `json:"files_detected"`
	FilesProcessed int `json:"files_processed"`
	TotalAttestations int `json:"total_attestations"`
	ProjectFiles []ProjectFileResult `json:"project_files,omitempty"`
}
```

</td>
<td>

```typescript
export interface DepsIngestionResult {
  files_detected: number;
  files_processed: number;
  total_attestations: number;
  project_files?: ProjectFileResult[];
}
```

</td>
</tr>
</table>

## DepsIxProcessor

**Source**: [`qntx-code/ixgest/git/deps.go:50`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L50)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type DepsIxProcessor struct {
}
```

</td>
<td>

```typescript
export interface DepsIxProcessor {
}
```

</td>
</tr>
</table>

## FlakeLock

**Source**: [`qntx-code/ixgest/git/deps.go:539`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L539)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type FlakeLock struct {
	Nodes map[string]unknown `json:"nodes"`
}
```

</td>
<td>

```typescript
export interface FlakeLock {
  nodes: Record<string, unknown>;
}
```

</td>
</tr>
</table>

## GitBranchResult

**Source**: [`qntx-code/ixgest/git/ingest.go:85`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/ingest.go#L85)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GitBranchResult struct {
	Name string `json:"name"`
	CommitHash string `json:"commit_hash"`
	AttestationCount int `json:"attestation_count"`
	Attestations []string `json:"attestations,omitempty"`
}
```

</td>
<td>

```typescript
export interface GitBranchResult {
  name: string;
  commit_hash: string;
  attestation_count: number;
  attestations?: string[];
}
```

</td>
</tr>
</table>

## GitCommitResult

**Source**: [`qntx-code/ixgest/git/ingest.go:73`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/ingest.go#L73)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GitCommitResult struct {
	Hash string `json:"hash"`
	ShortHash string `json:"short_hash"`
	Message string `json:"message"`
	Author string `json:"author"`
	Timestamp string `json:"timestamp"`
	ParentCount int `json:"parent_count"`
	AttestationCount int `json:"attestation_count"`
	Attestations []string `json:"attestations,omitempty"`
}
```

</td>
<td>

```typescript
export interface GitCommitResult {
  hash: string;
  short_hash: string;
  message: string;
  author: string;
  timestamp: string;
  parent_count: number;
  attestation_count: number;
  attestations?: string[];
}
```

</td>
</tr>
</table>

## GitIngestionHandler

**Source**: [`qntx-code/ixgest/git/async.go:14`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/async.go#L14)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GitIngestionHandler struct {
}
```

</td>
<td>

```typescript
export interface GitIngestionHandler {
}
```

</td>
</tr>
</table>

## GitIngestionPayload

**Source**: [`qntx-code/ixgest/git/async.go:28`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/async.go#L28)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GitIngestionPayload struct {
	RepositorySource string `json:"repository_source"`
	Actor string `json:"actor,omitempty"`
	Verbosity int `json:"verbosity"`
	NoDeps bool `json:"no_deps,omitempty"`
	Since string `json:"since,omitempty"`
}
```

</td>
<td>

```typescript
export interface GitIngestionPayload {
  /**
   * RepositorySource can be a local path OR a URL (handler will clone if URL)
   */
  repository_source: string;
  actor?: string;
  verbosity: number;
  no_deps?: boolean;
  /**
   * Since filters commits to only those after this timestamp or commit hash
   */
  since?: string;
}
```

</td>
</tr>
</table>

## GitIxProcessor

**Source**: [`qntx-code/ixgest/git/ingest.go:44`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/ingest.go#L44)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GitIxProcessor struct {
}
```

</td>
<td>

```typescript
export interface GitIxProcessor {
}
```

</td>
</tr>
</table>

## GitProcessingResult

**Source**: [`qntx-code/ixgest/git/ingest.go:57`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/ingest.go#L57)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GitProcessingResult struct {
	RepositoryPath string `json:"repository_path"`
	DryRun bool `json:"dry_run"`
	Actor string `json:"actor"`
	CommitsProcessed int `json:"commits_processed"`
	BranchesProcessed int `json:"branches_processed"`
	TotalAttestations int `json:"total_attestations"`
	Commits []GitCommitResult `json:"commits,omitempty"`
	Branches []GitBranchResult `json:"branches,omitempty"`
	Success bool `json:"success"`
	Message string `json:"message"`
	StartTime time.Time `json:"start_time"`
	EndTime time.Time `json:"end_time"`
}
```

</td>
<td>

```typescript
export interface GitProcessingResult {
  repository_path: string;
  dry_run: boolean;
  actor: string;
  commits_processed: number;
  branches_processed: number;
  total_attestations: number;
  commits?: GitCommitResult[];
  branches?: GitBranchResult[];
  success: boolean;
  message: string;
  start_time: string;
  end_time: string;
}
```

</td>
</tr>
</table>

## GoPackageInfo

**Source**: [`qntx-code/ixgest/git/ingest.go:709`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/ingest.go#L709)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type GoPackageInfo struct {
	PackageName string
	Imports []string
}
```

</td>
<td>

```typescript
export interface GoPackageInfo {
  PackageName: string;
  Imports: string[];
}
```

</td>
</tr>
</table>

## PackageJSON

**Source**: [`qntx-code/ixgest/git/deps.go:437`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L437)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type PackageJSON struct {
	Name string `json:"name"`
	Version string `json:"version"`
	Dependencies map[string]string `json:"dependencies"`
	DevDependencies map[string]string `json:"devDependencies"`
}
```

</td>
<td>

```typescript
export interface PackageJSON {
  name: string;
  version: string;
  dependencies: Record<string, string>;
  devDependencies: Record<string, string>;
}
```

</td>
</tr>
</table>

## ProjectFile

**Source**: [`qntx-code/ixgest/git/deps.go:26`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L26)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type ProjectFile struct {
	Type string
	Path string
	Name string
}
```

</td>
<td>

```typescript
export interface ProjectFile {
  /**
   * e.g., "go.mod", "Cargo.toml", "package.json"
   */
  Type: string;
  /**
   * Full path to the file
   */
  Path: string;
  /**
   * Base name
   */
  Name: string;
}
```

</td>
</tr>
</table>

## ProjectFileResult

**Source**: [`qntx-code/ixgest/git/deps.go:41`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L41)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type ProjectFileResult struct {
	Type string `json:"type"`
	Path string `json:"path"`
	AttestationCount int `json:"attestation_count"`
	Attestations []string `json:"attestations,omitempty"`
	Error string `json:"error,omitempty"`
}
```

</td>
<td>

```typescript
export interface ProjectFileResult {
  type: string;
  path: string;
  attestation_count: number;
  attestations?: string[];
  error?: string;
}
```

</td>
</tr>
</table>

## PyprojectToml

**Source**: [`qntx-code/ixgest/git/deps.go:595`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/deps.go#L595)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type PyprojectToml struct {
	Project unknown `toml:"project"`
	Tool unknown `toml:"tool"`
}
```

</td>
<td>

```typescript
export interface PyprojectToml {
  Project: unknown;
  Tool: unknown;
}
```

</td>
</tr>
</table>

## RepoSource

**Source**: [`qntx-code/ixgest/git/repo.go:17`](https://github.com/teranos/QNTX/blob/main/qntx-code/ixgest/git/repo.go#L17)


<table>
<tr>
<th>Go Source</th>
<th>TypeScript</th>
</tr>
<tr>
<td>

```go
type RepoSource struct {
	LocalPath string
	OriginalInput string
	IsCloned bool
	TempDir string
}
```

</td>
<td>

```typescript
export interface RepoSource {
  /**
   * LocalPath is the path to the local repository (either original or cloned)
   */
  LocalPath: string;
  /**
   * OriginalInput is the original input (URL or path)
   */
  OriginalInput: string;
  /**
   * IsCloned indicates if the repo was cloned from a remote URL
   */
  IsCloned: boolean;
  /**
   * TempDir is the temporary directory used for cloning (empty if not cloned)
   */
  TempDir: string;
}
```

</td>
</tr>
</table>
