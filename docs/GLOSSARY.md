...
Namespaces are flat: a namespace is a keypair, and keypairs have no hierarchy. Grouping is attested, not nested.

There is no separate concept of a user. A user is a namespace whose keys are held by biometrics; a node is a namespace holding its own. See [ADR-026](adr/ADR-026-namespaces.md) and [Decentralized Identity](vision/identity.md). Proposed — not yet implemented.
...
See [SYMBOLS.md](SYMBOLS.md) for the complete symbol reference — segments, attestation building blocks, derived types, and system symbols.
...
Superseded by [ADR-026](adr/ADR-026-namespaces.md), which moves per-glyph provenance off the actor slot onto a dedicated origin key so `by` can mean identity. This convention is still what runs.
