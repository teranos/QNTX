...
// simulateReclusterSetup mirrors the idempotent schedule creation logic in
// server.setupEmbeddingReclusterSchedule. It returns the job ID if a schedule
// was created or found, or "" if the interval is disabled.
func simulateReclusterSetup(store *Store, handlerName string, interval int) (string, error) {
	if interval <= 0 {
		return "", nil
	}
...
func TestReclusterScheduleSetup(t *testing.T) {
	const handler = "embeddings.recluster"

	t.Run("ZeroIntervalCreatesNothing", func(t *testing.T) {
		db := qntxtest.CreateTestDB(t)
		store := NewStore(db)

		id, err := simulateReclusterSetup(store, handler, 0)
	...
	t.Run("CreatesSchedule", func(t *testing.T) {
		...
		id, err := simulateReclusterSetup(store, handler, 3600)
		require.NoError(t, err)
	...
	t.Run("IdempotentOnRestart", func(t *testing.T) {
		...
		id1, err := simulateReclusterSetup(store, handler, 3600)
		require.NoError(t, err)

		id2, err := simulateReclusterSetup(store, handler, 3600)
		require.NoError(t, err)
	...
	t.Run("UpdatesIntervalOnConfigChange", func(t *testing.T) {
		...
		_, err = simulateReclusterSetup(store, handler, 1800)
		require.NoError(t, err)
