...
func TestEdgeCursor_AppliedOnReload(t *testing.T) {
	...
	// Create a meld-edge watcher with element_execute action
	...
	w.Filter.Actors = []string{"element:ax-source"}
	...
	oldAttestation := &types.As{
		...
		Actors:    []string{"element:ax-source"},
		Timestamp: cursorTime.Add(-1 * time.Hour),
	}

	// OnAttestationCreated should NOT fire executeElement for old attestation
