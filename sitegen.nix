...
${lib.concatStringsSep "\n" fileLinks}
  ...
  # Generate bash commands directly from file metadata (no runtime parsing needed)
  processFilesScript = lib.concatMapStringsSep "\n" (f: ''
    # Create output directory if needed
    ${if f.dir != "." then ''mkdir -p "$out/${f.dir}"'' else ""}

    # Generate HTML page for ${f.name}
    {
      cat <<'EOF'
${htmlHead "QNTX - ${f.name}" f.prefix}
${docStyles f.prefix}
EOF
      ${pkgs.pulldown-cmark}/bin/pulldown-cmark -T -S -F < "${f.mdPath}"
      echo "    </body>"
      echo "</html>"
    } > "$out/${f.htmlPath}"
  '') fileInfos;
...
pkgs.runCommand "qntx-docs-site"
  ...
  ''
    ...
    # Process markdown files (commands generated by Nix at build time)
    ${processFilesScript}
